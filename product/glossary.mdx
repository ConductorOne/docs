...
#### Annotation
A custom key/value metadata pair attached to a C1 object such as a policy, app, or entitlement. Use annotations to track details such as cost centers, compliance scope, and ownership. Objects managed by an infrastructure-as-code tool display a **Managed by** chip on their detail page.
...
#### Circuit breaker
A rate limit that can be set on an individual automation. When the automation runs more than a configured number of times within a set period, C1 automatically pauses it and queues new trigger events for review rather than running them immediately. See [Automation](/product/glossary#automation).
...
The C1 capability that governs how AI agents (such as Claude and VS Code) reach the MCP servers your organization runs. People authenticate once to C1, and their agents then get short-lived, scoped access to the MCP servers they're entitled to — single sign-on for agents, under the same governance you apply to people. It is built on the open [Cross-App Access (XAA)](/product/glossary#cross-app-access-xaa) standard, and the token C1 issues is an [ID-JAG](/product/glossary#id-jag). See [Enterprise-managed authorization overview](/product/admin/enterprise-managed-authorization/overview).
...
#### ID-JAG
The short-lived token C1 issues under [enterprise-managed authorization](/product/glossary#enterprise-managed-authorization-ema) (via the [Cross-App Access](/product/glossary#cross-app-access-xaa) protocol). Short for Identity Assertion JWT Authorization Grant, an IETF draft standard. Its audience is the MCP server's authorization server, which verifies C1's signature and exchanges it for one of its own access tokens.
