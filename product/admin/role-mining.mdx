...
<Steps>
  ...
  <Step>
    Enter a name for the access profile, then click **Create profile**.
  </Step>
</Steps>

The new access profile appears in **Governance** > **Access profiles**. It's created with no enrolled members — to enroll users, navigate to the profile's details page and add members.
...
ConductorOne analyzes the access held by users matching your cohort definition and surfaces entitlement suggestions with confidence scores. You can accept or dismiss these suggestions the same way as automated suggestions.
...
<AccordionGroup>
  ...
  <Accordion title="What happens to a suggestion after I accept it?">
    Accepting a suggestion creates a new access profile with the recommended entitlements. The profile starts with no enrolled members — you'll need to add members from the profile's details page. The suggestion moves to an accepted state and no longer appears in your active suggestions list.
  </Accordion>
</AccordionGroup>
