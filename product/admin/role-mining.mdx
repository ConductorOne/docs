...
- **Custom analysis**: You define a specific cohort and C1 analyzes the access patterns within that group on demand.
- **Suggestions**: C1 continuously analyzes access patterns across your organization and surfaces recommended access profiles, grouped by cohorts like department, job title, employment status, and manager's team.
...
C1 updates suggestions automatically after each connector sync and when you trigger a manual analysis run.
...
<Tip>
After accepting a suggestion, check **Create requests for currently enrolled users** on the access profile if you want C1 to automatically create access request tasks for each new entitlement for each enrolled user.
</Tip>
...
Dismissed suggestions are removed from your active view. C1 may surface a similar suggestion in a future analysis run if the underlying access pattern persists.

### Trigger a new analysis

C1 runs role mining analysis automatically after each connector sync. To run an analysis on demand:
<Steps>
  <Step>
    Navigate to **Governance** > **Role mining**.
  </Step>
