...
<Step>
Enter a **Filter (CEL expression)**. Gate filters read a different variable set than classifier rules — the call's identity and target rather than the risk axes. The field's helper text names `ctx.tool_name` and `ctx.tool_input`; see the [full list](/product/admin/agent-classifiers-reference#gate-filter-variables).
</Step>
<Step>
