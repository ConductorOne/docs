...
Navigate to **Security** > **Findings** to get started.

## Turn on finding types

C1.ai comes with a catalog of built-in finding types, grouped by category. Most are opt-in — you choose which conditions matter to your organization — but a handful are on by default, so C1.ai starts surfacing them without any setup.
...
<Step>
In **Findings settings**, toggle on the finding types you want C1.ai to detect. Each type shows a short description of what it looks for.
</Step>
<Step>
Click **Save changes**.
</Step>
...
C1.ai starts creating findings of the types you turned on the next time each connector syncs.

<Note>
Turning off a finding type stops C1.ai from creating new findings of that type. Findings of that type that are already open stay open — nothing re-checks them, so they won't resolve automatically.
</Note>
...
| Configuration | Connector sync failing | A connector with two consecutive failed full syncs. Auto-resolves on the next successful sync. | On by default |

## Work with a finding

Click a finding from the **Overview** or **All findings** tab to open its detail view, which includes:

- **Evidence** — why C1.ai flagged this finding, including any confidence score.
...
- **Assignee** — the person responsible for triaging the finding. Appears as its own column and filter on the findings list, and as a detail field on the finding itself.

<Note>
The Assignee field was named "Owner" until September 2026. If you consume the Finding object via API or webhook, `computed_owner`/`assigned_owner` are deprecated and no longer populated — use `assignee_identity_user_id` going forward. If you consume the audit log or feed it to a SIEM, the finding event previously logged as `OWNER_CHANGED` (with `old_owner`/`new_owner` keys) is now `ASSIGNEE_CHANGED` (with `old_assignee`/`new_assignee` keys); only newly emitted events use the new name, and historical rows are unaffected.
</Note>
...
Unlike transformation rules, routing rules stop at the first match. C1.ai evaluates rules in ascending order by **Order** and applies only the first one that matches.
