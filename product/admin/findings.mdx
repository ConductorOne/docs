...
<Step>
Create a transformation rule to guarantee severity. Go to **Transformation rules** > **Create rule**. Set **Finding type** to **Decoy credential used**, and leave **App** and the expression unset to match every app. Add a **Set severity** transform and set it to **Critical**. Save the rule.
</Step>
<Step>
Create a routing rule to escalate automatically. Go to **Routing rules** > **Create rule**. Set **Finding type** to **Decoy credential used**. Set **Action** to **No action**, so the finding stays open for review. Add a **Trigger automation** dispatcher to notify your security team immediately.
</Step>
<Step>
