...
og:description: Configure ConductorOne to send notification emails through your own AWS Simple Email Service account so recipients see your company's domain as the sender.
description: Configure ConductorOne to send notification emails through your own AWS Simple Email Service account so recipients see your company's domain as the sender.
...
- **Domain (recommended)**: Enter your domain (e.g., `yourcompany.com`). SES displays three DKIM CNAME records. Publish all three at your DNS provider.
...
<Step>
Name the role (e.g., `c1-ses-sender`) and click **Create role**.
</Step>
<Step>
...
<Step>
Name the policy (e.g., `c1-ses-send`) and save.
</Step>
<Step>
Copy the role's ARN (e.g., `arn:aws:iam::123456789012:role/c1-ses-sender`). You'll paste this into C1 in the next step.
</Step>
</Steps>
...
- **Region**: The AWS region where your SES identity is verified (e.g., `us-east-1`).
