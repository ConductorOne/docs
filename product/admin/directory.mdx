...
- **To change core details** (email, name, employee ID, department, job title, and other synced fields): edit the value in the source app that supplies the attribute (for example, your IdP or HRIS). The change propagates to C1 on the next directory sync. Which app supplies each field is configured under **Directory** > **Directory sources** > **Attribute manager** — see [Map user attributes](/product/admin/attributes).
- **To change a user's role in C1**: go to **Directory** > **Users**, click the **...** (more actions) menu next to the user, then select **Change role**. See [User roles](/product/admin/user-roles).
- **To add or override profile attributes on a user**: use **Directory** > **Users** > *user* > **Profile attributes**. These are the custom attributes described in [Map user attributes](/product/admin/attributes#create-and-map-custom-user-attributes).
