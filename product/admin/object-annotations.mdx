...
A set of reserved keys mark an object as managed by an IaC tool.

| Key | What it means |
| :--- | :--- |
| `managed_by` | The tool managing this object — `terraform`, `opentofu`, `pulumi`, `crossplane`, `ansible`, or any lowercase value |
| `iac_workspace` | Workspace or project identifier, for example `prod/c1-config` |
| `iac_resource_address` | Tool-native resource address, for example `conductorone_policy.approval_default` |
| `iac_tool_version` | IaC tool or provider version |
