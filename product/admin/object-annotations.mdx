...
Keys can be anything that passes the [shape rules](#limits), but prefixing with a short namespace avoids collisions with other tools' keys. Use your team name, company domain, or a runtime identifier as the namespace — for example, `myteam/cost-center`, `acme.com/compliance`, or `runtime/region`. Within the namespace, lowercase with `-` or `_` as separators is conventional.
