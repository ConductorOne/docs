...
CEL expressions in policies, approver rules, and entitlement routing rules can read annotations on entitlements, app resources, and other supported objects. This lets you fork logic based on metadata your team has already applied — without adding new fields or changing your object configuration.

### Read an annotation value

Use bracket notation to read a specific key:

```cel
entitlement.annotations["team"] == "infrastructure"
```

The same field is available on other objects where annotations are supported:
...
- **Scope automations by compliance tag**: In an automation condition, check `entitlement.annotations["compliance"] == "sox"` to limit a workflow to entitlements tagged for SOX review.
