...
Keys can be anything that passes the [shape rules](#limits), but prefixing with a short namespace avoids collisions with other tools' keys:
...
Use your team name, company domain, or a runtime identifier as the namespace. Within the namespace, lowercase with `-` or `_` as separators is conventional.

**Common uses:**

- **Cost reporting** — tag apps and access profiles with a cost center, then query the API or export to your dashboard
...
When you edit any field on an IaC-managed object in the C1 UI — other than the annotations themselves — C1 atomically clears the IaC keys in the same operation and shows a confirmation toast. This signals that C1, not your IaC tool, now manages the object.
...
Every annotation change is recorded in the object's history alongside other field changes. Changes to IaC management keys — setting or clearing `managed_by` — are called out with a distinct badge, so you can see at a glance when an object moved in or out of IaC management and who made the change.
