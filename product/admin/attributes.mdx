...
<Tip>
Creating a custom attribute mapping is just the first step. For the attribute to appear on a ConductorOne user's profile, you also need to bind it to a profile type and assign users to that profile type. See [How attributes flow to users](/product/admin/profile-types#how-attributes-flow-to-users) for the full lifecycle.
</Tip>
