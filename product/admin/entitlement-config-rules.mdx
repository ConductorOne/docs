...
<Warning>
**For cloud infrastructure apps, entitlement configuration rules are required for entitlements to expire.** A cloud infrastructure entitlement only expires and is automatically removed if a matching configuration rule sets a maximum grant duration for it. Without a matching rule, these entitlements don't expire.
</Warning>
