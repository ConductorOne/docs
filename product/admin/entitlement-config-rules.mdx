...
## Manage configuration rules with the C1 API

In addition to the web app, you can create and manage an app's entitlement configuration rules programmatically with the C1 API. Each rule belongs to a single app, so every operation is scoped to an `app_id`:

| Action | Method and path |
|--------|-----------------|
| List the rules for an app | `GET /api/v1/apps/{app_id}/entitlement_configuration_rules` |
| Get a single rule | `GET /api/v1/apps/{app_id}/entitlement_configuration_rules/{id}` |
| Create a rule | `POST /api/v1/apps/{app_id}/entitlement_configuration_rules` |
| Update a rule | `POST /api/v1/apps/{app_id}/entitlement_configuration_rules/{id}` |
| Delete a rule | `DELETE /api/v1/apps/{app_id}/entitlement_configuration_rules/{id}` |
| Reorder an app's rules | `POST /api/v1/apps/{app_id}/entitlement_configuration_rules/reorder` |

Reordering replaces the full priority order in a single call: supply the complete, ordered list of rule IDs, and the server assigns priorities (1 through N) in that order. The request must be a permutation of every active rule in the app—missing or extra IDs are rejected.

Browse these endpoints under the **App Entitlement Configuration Rule** group in the [API reference](/conductorone-api/api) for full request and response schemas. To call the API, first [create a personal API key](/conductorone-api/api#create-a-personal-api-key).
