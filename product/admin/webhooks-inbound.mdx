...
1. When you configure the webhook trigger, ConductorOne generates a 256-bit secret (base64url-encoded).
...
The signature input is the string `{timestamp}.{event_id}.{body}` (the three values joined with periods). Compute HMAC-SHA256 using your secret, then base64url-encode the result (without padding).
