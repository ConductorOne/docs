...
- Governing resource access at a granular level — admins review, approve, or disable each static resource and URI template, then grant read access through its linked entitlement
...
| **MCP resource** | A static resource or URI template exposed by an MCP server. C1 discovers it automatically, but a user can read it only after an admin approves it and the user has its linked read entitlement. |
...
4. **Admin reviews and approves resources**, then manages their linked read entitlements independently of toolsets.
5. **Admin binds the toolset or resource entitlement to an access profile** with an approval policy.
6. **End user registers their AI client** with C1.
7. **End user requests the applicable access profile** from the C1 catalog (web, Slack, or from their AI client).
8. **Approver approves**, and the authorized tool or resource becomes available to the user's AI client.
