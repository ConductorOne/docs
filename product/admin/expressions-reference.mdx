...
**Available as:** `entitlement` (in policy expressions and [entitlement configuration rules](/product/admin/entitlement-routing-rules); in configuration rules for cloud infrastructure apps, `entitlement.role` and `entitlement.scope` are also populated)
