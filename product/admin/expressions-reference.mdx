...
**Available as:** `entitlement` (in policy expressions and [entitlement routing rules](/product/admin/entitlement-routing-rules); in routing rules for cloud infrastructure apps, `entitlement.role` and `entitlement.scope` are also populated)
