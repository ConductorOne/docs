...
og:description: Set up webhooks in C1 to extend workflows across multiple tools, such as creating a ticket or making an API call. 
description: Set up webhooks in C1 to extend workflows across multiple tools, such as creating a ticket or making an API call.
...
| `c1.webhooks.v1.PayloadProvisionStep` | The same payload that is returned from the [GetTask API endpoint](api-reference/task/get). |
...
You can configure external systems to trigger ConductorOne automations by sending authenticated HTTP requests to an inbound webhook endpoint. ConductorOne supports both HMAC and JWT authentication methods.
