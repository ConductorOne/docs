...
- Guardrail policy decision (block, curated hook dispatch, or an observe-mode rule that would have blocked) — logged as its own event linked to the tool call, not as a field on the tool call entry above
