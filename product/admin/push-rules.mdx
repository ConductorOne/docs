...
description: "Automatically sync user attributes from C1 to connected applications to keep user data consistent across your integrated systems."
og:description: "Automatically sync user attributes from C1 to connected applications to keep user data consistent across your integrated systems."
...
- Sync C1 attributes to application user profiles
...
Attribute push keeps user attributes synchronized between C1 and your connected applications.

In broad strokes, here's how to set up a push rule:

1. Select the connector you want to push attributes to
2. Map C1 attributes to the target application's user attributes
...
Once enabled, C1 automatically pushes attributes to the target application whenever:

- The push configuration is created or updated
- A user's attribute values change in C1
...
- User attribute mappings defined in C1 (**Identities** > **Directory sources**)
...
For each attribute you want to sync:
   ...
   - Select a C1 attribute to map, or click the **Change to expression** icon and write a CEL expression for custom logic
...
Select a C1 attribute from the dropdown to sync its value directly to the target application.

**Example**: Map the `Department` attribute in C1 to the `department` field in the target application.
...
Syncs attributes for all users in your C1 tenant.
