...
For example, if a resource's visibility is set to **Members**, only users who have been granted an entitlement on that resource can see the resource and any of its entitlements. Users who don't meet the visibility criteria will not see the entitlements in search results, the access catalog, or other areas of the ConductorOne interface.
