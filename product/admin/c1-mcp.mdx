---
title: C1.ai MCP
description: Connect AI assistants like Claude Desktop, Codex, Cursor, and VS Code to query your C1.ai data using the Model Context Protocol (MCP).
og:title: C1.ai MCP - C1.ai docs
og:description: Connect AI assistants like Claude Desktop, Codex, Cursor, and VS Code to query your C1.ai data using the Model Context Protocol (MCP).
sidebarTitle: Connect to the C1.ai MCP
---

{/* Editor Refresh: 2026-04-28 */}

Connect AI assistants like Claude Desktop, Codex, Cursor, and VS Code to query your C1.ai data using the Model Context Protocol (MCP).

<Note>
This page covers querying C1.ai's own identity governance data from your AI client. If you're looking to govern AI access to external tools like Salesforce or GitHub, see [AI access management](/product/admin/aiam-overview).
</Note>

<Note>
AI Connections are read-only. Connected AI assistants can view your C1.ai data but cannot create, modify, or delete any configuration.
</Note>
...
Before users can connect AI assistants, a C1.ai administrator must complete the following setup.
...
<Steps>
<Step>
Navigate to **AI > C1.ai Gateway** and click **Settings**.
</Step>
<Step>
Find **C1 Gateway**, click **Edit**, and toggle **Enable the C1.ai gateway** to on.
</Step>
<Step>
Click **Save**.
</Step>
...
Follow these steps to connect your AI assistant to C1.ai via MCP.

<Steps>
<Step>
**Get the MCP server URL**

In C1.ai, navigate to your user profile menu and click **AI & API** > **AI connections**. Copy the MCP server URL displayed at the top of the page:
...
<Tabs>
  <Tab title="Claude Desktop">
    ...
    3. Add a new server with the C1.ai MCP URL.
    4. Save and restart Claude Desktop.
  </Tab>
  <Tab title="Claude Code">
    1. From your terminal, add the C1.ai MCP server using the `claude mcp add` command:

       ```bash
       claude mcp add --transport http c1 https://<your-tenant>-mcp.conductor.one/v1
       ```

       Replace `<your-tenant>` with your C1.ai tenant subdomain.
    2. Start Claude Code and run the `/mcp` slash command.
    3. Select the **c1** server and choose **Authenticate**. Your browser opens to C1.ai to complete the authorization step described below.
  ...
  <Tab title="Codex">
    1. From your terminal, add the C1.ai MCP server using the `codex mcp add` command:
    ...
    2. Verify the server is configured:
       ...
       codex mcp list
       ```
    3. Start Codex in the CLI or IDE extension and connect to the **c1** MCP server. Your browser opens to C1.ai to complete the authorization step described below.
  ...
  <Tab title="Cursor">
    ...
    3. Add the C1.ai MCP server URL in the JSON editor.
...
When your AI assistant first connects, you'll be redirected to C1.ai to authorize the connection:
...
Once authorized, your AI assistant can query C1.ai data. The connection appears in your user profile under **AI & API** > **AI connections**.
</Step>
</Steps>
...
AI assistants can query 20 different object types across your C1.ai tenant.
...
| `App` | Applications and directories connected to C1.ai |
...
| `RoleBinding` | C1.ai role assignments |
...
<Note>
Revoking a connection immediately prevents the AI assistant from accessing your C1.ai data. You can re-authorize the connection later if needed.
</Note>
...
| User | The C1.ai user who authorized the connection |
...
<AccordionGroup>
  <Accordion title="Can the AI modify my C1.ai configuration?">
  ...
  <Accordion title="What AI tools are compatible?">
    Any tool that supports the Model Context Protocol (MCP) can connect to C1.ai, including:
  ...
  <Accordion title="Can I see what queries the AI is making?">
    Yes. AI connection activity is logged in your C1.ai system log. See [System log events](#system-log-events) for details.
  </Accordion>

  <Accordion title="What happens if I revoke a connection?">
    The AI assistant immediately loses access to your C1.ai data. Any ongoing queries will fail. You can re-authorize the same AI assistant later by going through the connection flow again.
  </Accordion>
