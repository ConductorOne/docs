---
title: C1 MCP
description: Connect AI assistants like Claude Desktop, Cursor, and VS Code to query your ConductorOne data using the Model Context Protocol (MCP).
og:title: C1 MCP - C1
og:description: Connect AI assistants like Claude Desktop, Cursor, and VS Code to query your ConductorOne data using the Model Context Protocol (MCP).
sidebarTitle: C1 MCP
---

{/* Editor Refresh: 2026-04-27 */}

<Warning>
**Early access.** This feature is in early access, which means it's undergoing ongoing testing and development while we gather feedback, validate functionality, and improve outputs. Contact the C1 Support team if you'd like to try it out or share feedback.
</Warning>

Connect AI assistants like Claude Desktop, Cursor, and VS Code to query your ConductorOne data using the Model Context Protocol (MCP).

<Note>
AI Connections are read-only. Connected AI assistants can view your ConductorOne data but cannot create, modify, or delete any configuration.
</Note>

## Prerequisites

Before users can connect AI assistants, a ConductorOne administrator must complete the following setup.

### Turn on AI connections

A Super Admin must enable AI connections in your tenant settings:

<Steps>
<Step>
Navigate to **Settings** in the left sidebar.
</Step>
<Step>
Under the **Developers** sub-section, select **AI Connections**.
</Step>
<Step>
Edit and toggle **Enable AI connections** to on.
</Step>
<Step>
Click **Save**.
</Step>
</Steps>

<Tip>
Once enabled, the MCP server URL is not shown on this settings page. Each user must navigate to **Profile** → **AI & API** to find their MCP server URL.
</Tip>

### Configure IP restrictions (optional)

For additional security, you can restrict which IP addresses can use AI connections:

<Steps>
<Step>
Navigate to **Settings** in the left sidebar.
</Step>
<Step>
Under the **Security** sub-section, select **SSO & sessions**.
</Step>
<Step>
Scroll down to the **Global IP allow list configuration**.
</Step>
<Step>
Find the **AI connections** section.
</Step>
<Step>
Add allowed CIDR ranges.
</Step>
<Step>
Click **Save**.
</Step>
</Steps>

<Warning>
If you configure IP restrictions, AI assistants will only work from the specified IP ranges. This is useful for restricting usage to corporate networks or VPNs.
</Warning>

## Connect an AI assistant

### Who can connect

Currently only users with Super Admin or Read Only Admin roles can authorize AI connections during this phase of the early access program.

### Setup

<Steps>
<Step>
**Get the MCP server URL**

Navigate to **Profile** → **AI & API** in ConductorOne. Copy the MCP server URL displayed at the top of the page:

```
https://<your-tenant>.conductor.one/api/v1alpha/mcp
```
</Step>
<Step>
**Configure your AI assistant**

Add the MCP server URL to your AI assistant's configuration:

**Claude Desktop:**
1. Open Claude Desktop settings.
2. Navigate to the MCP servers section.
3. Add a new server with the ConductorOne MCP URL.
4. Save and restart Claude Desktop.

**Cursor:**
1. Open Cursor settings.
2. Navigate to the **Tools and MCP** configuration.
3. Add the ConductorOne MCP server URL in the JSON editor.
4. Save your configuration.
</Step>
<Step>
**Authorize the connection**

When your AI assistant first connects, you'll be redirected to ConductorOne to authorize the connection:

1. Review the connection details.
2. Optionally customize the connection name.
3. The connection is granted Read Only Admin access.
4. Click **Allow** to authorize.
</Step>
<Step>
**Start querying**

Once authorized, your AI assistant can query ConductorOne data. The connection appears in your **Profile** → **AI & API** page.
</Step>
</Steps>

## Available tools

Connected AI assistants have access to the following query tools:

| Tool | Description |
| :--- | :--- |
| `find_api_objects` | Query objects by identifiers, search text, or filters |
| `count_api_objects` | Count objects matching specific criteria |
| `list_object_types` | List all available object types |
| `describe_object_filters` | Get filterable fields for an object type |
| `get_object_schema` | Get the JSON schema for an object type |

## Queryable objects

AI assistants can query 20 different object types across your ConductorOne tenant.

### Identity and applications

| Object | Description |
| :--- | :--- |
| `User` | Identity accounts in your directory |
| `App` | Applications and directories connected to ConductorOne |
| `Connector` | Data sync connectors for applications |
| `Task` | Access requests, reviews, and other tickets |
| `Policy` | Approval workflows and access policies |

### Application data model

| Object | Description |
| :--- | :--- |
| `AppUser` | Application accounts linked to identities |
| `AppResourceType` | Categories of resources within applications |
| `AppResource` | Specific resources within applications |
| `AppEntitlement` | Permissions and roles within applications |
| `Grant` | Entitlement assignments to users |

### Access reviews

| Object | Description |
| :--- | :--- |
| `AccessReview` | Access review campaigns |
| `AccessReviewTemplate` | Templates for recurring reviews |
| `AccessReviewSelection` | Individual items within a review |

### Access conflicts and separation of duties

| Object | Description |
| :--- | :--- |
| `ConflictMonitor` | Separation of duties policies |
| `AccessViolation` | Detected SoD violations |

### Supporting objects

| Object | Description |
| :--- | :--- |
| `RequestCatalog` | Collections of requestable entitlements in an Access Profile |
| `Webhook` | Event notification configurations |
| `Directory` | Account sync configurations |
| `ProfileType` | Attribute mapping configurations |
| `RoleBinding` | ConductorOne role assignments |

## Example queries

**Access analysis**

- "Who has access to Salesforce?"
- "What apps does jane@company.com have access to?"
- "Show me all users with admin access to AWS"
- "List all entitlements for the Engineering group"

**Access requests**

- "Show me all pending access requests"
- "How many access requests were created this week?"
- "Find all denied access requests for the Finance app"
- "What requests are waiting for my approval?"

**Access reviews**

- "List access review campaigns that are in progress"
- "How many items are in the Q4 access review?"
- "Show me completed access reviews from this month"
- "Which access reviews are past their due date?"

**Compliance**

- "Find all separation of duties violations"
- "Which users have conflicting access?"
- "Show me all active conflict monitors"
- "List users who violate the Finance-Engineering SoD policy"

## Managing connections

### View connections

Navigate to **Profile** → **AI & API** to see all AI connections you've authorized. Each connection shows:

- AI assistant name and icon
- Client ID
- Connection date
- Last used date

### Revoke a connection

<Steps>
<Step>
Navigate to **Profile** → **AI & API**.
</Step>
<Step>
Find the connection you want to revoke.
</Step>
<Step>
Click **Revoke**.
</Step>
<Step>
Type the connection name to confirm.
</Step>
<Step>
Click **Confirm**.
</Step>
</Steps>

<Warning>
Revoking a connection immediately prevents the AI assistant from accessing your ConductorOne data. You can re-authorize the connection later if needed.
</Warning>

### Admin management

Super Admins can view and manage all AI connections across the tenant:

<Steps>
<Step>
Navigate to **Settings** → **Developers** → **AI connections**.
</Step>
<Step>
View the list of all authorized connections.
</Step>
<Step>
Revoke connections as needed.
</Step>
</Steps>

## Security considerations

AI Connections is designed with security as a priority:

- **Read-only access:** AI assistants cannot modify any data or configuration.
- **User-scoped authorization:** Each user authorizes their own connections.
- **IP restrictions:** Optional IP allow listing for network-level control.
- **Audit logging:** All AI queries are logged in the system log.
- **Revocable access:** Connections can be revoked at any time.

## System log events

All MCP activity is recorded in the ConductorOne system log. You can use these event types to monitor and alert on AI connection activity:

| Event | Activity Name | Description |
| :--- | :--- | :--- |
| MCP Session Start | `mcp_session_start` | Logged when an AI assistant establishes a connection |
| MCP Session End | `mcp_session_end` | Logged when an AI assistant disconnects (includes session statistics) |
| MCP Tool Call | `mcp_tool_call` | Logged for each tool invocation (query, count, etc.) |

### System log matching rules

Use these filters in **Settings** → **System Log** to monitor AI connection activity:

All MCP activity:
```
activity_name starts with "mcp_"
```

Session connections only:
```
activity_name = "mcp_session_start"
```

Tool calls only:
```
activity_name = "mcp_tool_call"
```

Failed tool calls:
```
activity_name = "mcp_tool_call" AND outcome = "FAILURE"
```

Activity by specific user:
```
activity_name starts with "mcp_" AND actor.user.email = "user@example.com"
```

### Session event details

Each MCP session event includes:

- **Session ID:** Unique identifier for the connection
- **User:** The ConductorOne user who authorized the connection
- **Source IP:** IP address of the AI assistant
- **User Agent:** Client identifier (e.g., Claude Desktop, Cursor)
- **Duration:** Session length (in session end events)
- **Tool call counts:** Success/failure statistics (in session end events)

### Tool call event details

Each tool call event includes:

- **Tool name:** Which tool was invoked (e.g., `find_api_objects`)
- **Object type:** The API object being queried (e.g., `User`, `App`)
- **Result count:** Number of objects returned
- **Duration:** Query execution time
- **Outcome:** Success or failure status

## FAQ

### Can the AI modify my ConductorOne configuration?

No. AI Connections are granted **Read Only Admin** access. Connected AI assistants can view and query your data, but they cannot create, update, or delete any configuration, users, or access assignments.

### Who can authorize AI connections?

Only users with **Super Admin** or **Read Only Admin** roles can authorize AI connections for their own use at this time.

### What AI tools are compatible?

Any tool that supports the Model Context Protocol (MCP) can connect to ConductorOne, including:

- Claude Desktop
- Cursor
- VS Code (with MCP extensions)
- Other MCP-compatible AI assistants

At this time the experience is best with Cursor, since it has more built-in features to deal with large context tool call results. Some results, for example finding Task objects, can result in very large uses of the context window. We are investigating how to improve context window management for all clients.

### How do I restrict where AI connections can be used?

Administrators can configure IP allow lists in **Settings** → **IP Allowlist** → **AI connections** to restrict access to specific IP ranges. This is useful for limiting usage to corporate networks or VPNs.

### Can I see what queries the AI is making?

Yes. AI connection activity is logged in your ConductorOne system log. See [System log events](#system-log-events) for details.

### What happens if I revoke a connection?

The AI assistant immediately loses access to your ConductorOne data. Any ongoing queries will fail. You can re-authorize the same AI assistant later by going through the connection flow again.

### Is my data sent to AI providers?

When you query data through an AI assistant, the query results are sent to the AI provider (e.g., Anthropic for Claude) to generate responses. Only the data specifically requested by your queries is transmitted. ConductorOne does not send your data to any AI providers in the MCP flow — it is sent by clients under your control.

For questions about AI Connections security or compliance, contact your ConductorOne account team.
