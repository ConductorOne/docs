...
og:title: Delegate a user's tasks - C1 docs
og:description: Set a delegate for a user who should not or cannot be assigned C1 tasks. Tasks will be automatically reassigned to the delegated user unless the task's policy doesn't allow delegation.
description: Set a delegate for a user who should not or cannot be assigned C1 tasks. Tasks will be automatically reassigned to the delegated user unless the task's policy doesn't allow delegation.
...
<Note>
Delegation applies only to tasks created **after** the delegate is set. C1 does not reassign tasks that were already open when the delegate was added. To move an existing task, reassign it manually from the task's details page (subject to the governing policy's reassignment rules).
</Note>
