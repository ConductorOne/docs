...
description: Review MCP resources and URI templates discovered by C1, approve them, and manage the grants that control who can read them.
og:title: Govern MCP resources - C1 docs
og:description: Review MCP resources and URI templates discovered by C1, approve them, and manage the grants that control who can read them.
---
...
Each discovered MCP resource has its own read entitlement. A user can read a resource only when all of the following are true:
...
A tool grant does not grant access to a resource. Resources are not added to toolsets. Use the resource's linked entitlement to manage its grants.
...
After you approve a resource, use its linked entitlement to give users read access. In the resource details panel, select the linked entitlement or its grant count to open the standard entitlement details page. From there, use your normal entitlement and [access profile](/product/admin/profiles) workflow to grant access.
...
If an upstream server stops advertising a resource, C1 marks it **Removed**. If the server later advertises the same resource identity again, review its current state before granting access.
