...
sidebarTitle: Service principals
---
...
| :--- | :--- | :--- |
| **How** | Client ID + secret sent to token endpoint | External OIDC JWT exchanged for ConductorOne token |
...
| **Security** | Secret can leak; mitigate with CIDR + expiration + scoped roles | No secret to leak; token valid only for a single CI/CD run |
...
| CIDR ranges per credential or trust | 32 |
| Workload federation provider requirement | Publicly accessible JWKS endpoint |
