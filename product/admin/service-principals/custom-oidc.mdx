...
og:title: Custom OIDC providers - C1.ai docs
...
Send the external OIDC JWT to C1.ai's token exchange endpoint:
...
| **Audience (`aud`)** | Must contain your C1.ai tenant domain (for example `yourcompany.conductor.one`) |
...
Once you have the access token, set the `CONDUCTORONE_ACCESS_TOKEN` environment variable and all C1.ai tools pick it up automatically:
