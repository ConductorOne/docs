...
og:title: GitLab CI integration - C1.ai docs
og:description: Set up secretless authentication from GitLab CI to C1.ai using OIDC id_tokens.
description: Set up secretless authentication from GitLab CI to C1.ai using OIDC id_tokens.
sidebarTitle: GitLab CI
---

{/* Editor Refresh: 2026-02-14 */}

GitLab CI/CD can issue OIDC tokens for each job via the `id_tokens` keyword. You exchange this token for a C1.ai access token using a curl command in your pipeline.
...
Add `id_tokens` to your job to request a GitLab OIDC token, then exchange it for a C1.ai access token:
...
The `id_tokens` block tells GitLab to generate a signed JWT with your C1.ai tenant domain as the audience. The token is available as the `C1_TOKEN` environment variable within the job.
