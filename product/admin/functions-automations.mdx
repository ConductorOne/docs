...
Functions run as steps in automation workflows, triggered by:
...
Run functions on-demand from the UI:
...
Call functions programmatically via the ConductorOne API for administrative tasks, batch processing, or integration with other systems.
...
- Your **return value** becomes available to subsequent automation steps via the step name (e.g., `check_training.approved`).
