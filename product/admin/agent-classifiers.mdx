---
title: "Agent classifiers"
og:title: "Agent classifiers - C1.ai docs"
description: "Classifiers are named, reusable rule cascades that decide what an AI agent may do on each tool call, based on how much private data, untrusted content, and exfiltration capability is in play."
og:description: "Classifiers are named, reusable rule cascades that decide what an AI agent may do on each tool call, based on how much private data, untrusted content, and exfiltration capability is in play."
sidebarTitle: "Agent classifiers"
---
...
Every classifier rule is a CEL expression over the same small set of variables. Three of them are the risk axes C1.ai scores per call, each `LOW`, `MEDIUM`, or `HIGH`:
