...
<Warning>
**This agent is in beta.** This means it's undergoing ongoing testing and development while we gather feedback, validate functionality, and improve outputs. Please contact the ConductorOne Support team with any issues or feedback.
</Warning>
...
The more specific your prompt, the more accurate the generated automation.

<Tip>
Include the trigger event, the apps or entitlements involved, and the outcome you want — not the individual steps. For example: "When a user is deactivated in Okta, revoke their access to Salesforce and GitHub, then send a Slack message to #it-ops."

Other details that help:
- The role or group the automation applies to
- Any conditions or exceptions (for example, skip contractors, or only run for users in the US)
- The timing of each action (immediately, after a delay, or on a schedule)
</Tip>

### Review what the agent generates

After the agent creates your automation, review it before testing:

- **Trigger**: Confirm the trigger type and any filter conditions match your intent. Refer to the [automation triggers reference](/product/admin/automations-triggers-reference) for details.
- **Steps**: Check that the steps are in the right order and target the correct apps and entitlements. Refer to the [automation steps reference](/product/admin/automations-steps-reference) for details.
- **CEL conditions**: If any steps include skip conditions, verify the logic reflects your requirements.
- **Automation toggle**: Decide whether you want the automation to run automatically once published. You can leave the toggle off and trigger it manually while you're still evaluating.

You can edit any step directly or adjust the trigger without leaving the page.

### Try it in draft mode first

New automations — whether created manually or by the agent — start in draft state. They don't run automatically until you publish them, so you can safely experiment without affecting production.

Use this workflow to build confidence before going live:

<Steps>
  <Step>
    Click **Run draft** to test the automation with real data. You'll be prompted to provide context for the test run, and a panel will show the execution details as it proceeds.
  </Step>
  <Step>
    Review the results. If something isn't right, edit the steps or re-prompt the agent to refine the automation.
  </Step>
  <Step>
    Re-run the draft until the automation behaves as expected.
  </Step>
  <Step>
    When you're confident it works correctly, click **Publish** to put it into use.
  </Step>
</Steps>
