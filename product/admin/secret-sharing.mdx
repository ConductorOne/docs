...
Copy the generated **share URL** and send it to recipients via email, Slack, Teams, or another preferred channel.
...
Use the filters to switch between **Active** secrets and **All** secrets (which includes expired and revoked secrets).
</Step>
<Step>
...
| Role | Capabilities |
| :--- | :--- |
| **Basic user** | Create secrets, view their own secrets, revoke their own secrets. |
| **Super admin** | All basic user capabilities, plus: view all tenant secret metadata (not content), revoke any secret, access audit logs. |
