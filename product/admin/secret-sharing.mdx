...
og:title: Secret sharing - ConductorOne docs
...
Secret sharing lets you securely share credentials, files, and sensitive notes with internal team members or external contacts. Content is encrypted in your browser before upload — ConductorOne never stores or logs plaintext.

## How it works

Secret sharing is designed so that ConductorOne never has access to your plaintext — encryption happens in your browser, before anything leaves your device. Here's how a secret moves from creation to delivery:
...
3. **Share** — copy the generated link and send it to recipients through whatever channel works — email, Slack, a ticket, or anything else.
...
<Steps>
<Step>
Navigate to **Secrets** in the left sidebar, then click **Share a secret**.
</Step>
<Step>
...
| :--- | :--- |
| **Internal** | Share with ConductorOne users in your organization. Recipients authenticate via SSO. |
| **External** | Share with anyone via email address. Recipients verify their identity with a one-time magic link. |
</Step>
<Step>
**Add recipients.**
...
| **Key-value** | Environment variables — supports paste from `.env` files. |
</Step>
<Step>
**Set access limits.**
...
Copy the generated share URL and send it to recipients via your preferred channel (email, Slack, Teams, and so on).

<Note>
ConductorOne does not notify recipients. The share URL is the only way to access the secret — you must distribute it yourself.
</Note>
</Step>
</Steps>
...
You can view and manage secrets you've created from the **Secrets** page. Administrators can manage all secrets across the tenant from **Settings** > **Security** > **Secret Sharing**.
...
<Step>
**Optional.** Click **Revoke** to immediately and permanently delete the encrypted content and block further access.
</Step>
</Steps>
...
<Steps>
<Step>
Navigate to **Settings** > **Security** > **Secret Sharing**.
</Step>
<Step>
Use the **Secrets** tab to view all secrets created across your tenant.
</Step>
<Step>
Use the **Audit Log** tab to view all secret-related activity. Filter by creator, sharing mode, status, or date range.
</Step>
<Step>
**Optional.** Click **Revoke** on any active secret to permanently delete it.
</Step>
</Steps>

## Permissions

Secret sharing is available to all users. Administrators have additional visibility and control across the tenant.

| Role | Capabilities |
| :--- | :--- |
| **Basic user** | Create secrets, view own secrets, revoke own secrets. |
| **Owner / Super admin** | All basic user capabilities, plus: view all tenant secret metadata (not content), revoke any secret, access audit logs. |
...
<AccordionGroup>
  <Accordion title="Can ConductorOne employees see my secrets?">
    No. ConductorOne stores only encrypted blobs. Decryption occurs in an isolated vault service and plaintext is never stored or logged.
  </Accordion>
  ...
  <Accordion title="Can I extend a secret's expiration?">
    No. Create a new secret with a longer expiration if needed.
  </Accordion>
...
- Expiration cannot be extended after creation.
- ConductorOne does not notify recipients — you must send the share URL yourself.
- External recipients are limited to 64 per secret; internal recipients to 128.
- Text content is capped at 64 KB; files at 1 GB.
