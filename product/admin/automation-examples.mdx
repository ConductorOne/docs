...
4. Add a **Create Account** step. Select the connector for a department-specific tool (for example, Jira for Engineering, Salesforce for Sales). Use the "From C1.ai user data" creation method to map the user's profile fields automatically.
...
**Use case:** When a user's status changes to disabled, revoke their access, update their C1.ai status, and notify the security team.
...
1. Navigate to the Salesforce application page and find the **Unused access** section on the **Overview** page. Click **Add automation** and select **Custom usage-based automation**.

   Alternatively, create a global automation with the **Unused Access** trigger and select Salesforce as the app.

2. Configure the trigger:
   - **App name**: Salesforce
   - **Trigger after account not logged in for**: 30
   - **Cold start behavior**: **Run automation after (30) days for initially matched accounts**. This prevents the automation from immediately acting on all currently inactive accounts when first published. Only accounts that cross the 30-day threshold after the automation goes live will trigger it.
...
- An [outbound webhook](/product/admin/webhooks) configured to call the environment cleanup API.
...
<Note>
The **Set credential** step used in this example is in early access and is only supported for self-hosted connectors.
</Note>
