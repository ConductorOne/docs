...
1. **Connector syncs data from the source system.** When a connector syncs with a source application (like Workday, Active Directory, or Okta), it pulls user account data into ConductorOne. This data is stored on the **app user** (the account record within the connected application). Custom fields from the source system are included in the app user's profile as key-value pairs.

2. **You create an attribute mapping.** In the [Attribute manager](/product/admin/attributes), you create a custom attribute and tell ConductorOne which application and which field to pull the data from. This is how ConductorOne knows, for example, that "Employment Type" should come from your Workday app user's `employmentType` field.
...
5. **The attribute appears on the ConductorOne user.** After the next directory sync, the attribute value flows from the app user through the mapping and profile type, and appears in the **Profile attributes** section of the ConductorOne user's page. From here, it can be used in policies, access review campaigns, CEL expressions, and [account correlation](#example-using-a-custom-ad-attribute-for-account-correlation).
...
### Example: making a custom Workday field available for policy routing

Suppose you want to use a "Cost Center" field from Workday to route access requests to the right approver.

<Steps>
<Step>
Make sure your Workday connector is set up and syncing. After a sync completes, the Cost Center value is stored on each Workday app user's profile.
</Step>
<Step>
...
### Example: using a custom AD attribute for account correlation
...
<Steps>
<Step>
Make sure your Active Directory connector is set up and syncing. After sync, the `githubUserName` value is available on each AD app user's profile.
</Step>
<Step>
...
**Custom attributes vs. standard attributes.** Only custom user attributes can be used in CEL expressions, policies, and profile types. If you need to reference data that's captured in a standard user attribute (like Department or Job Title) in a CEL expression, you must [create a custom attribute](/product/admin/attributes#create-and-map-custom-user-attributes) that maps to the same source data.
