...
**Available data.** On demand triggers populate `ctx.trigger.user_id` for the subject user selected at execution time. If triggered via **Run automation** step, any JSON context passed by the parent automation is available in `ctx.trigger`. For requestable automations, form field values are available in `ctx.trigger.form_fields`. See [workflow expressions](/product/admin/expressions-workflows).
...
**Available data.** Populates `ctx.trigger.oldAccount` (AppUser object with previous values), `ctx.trigger.newAccount` (AppUser object with new values), `ctx.trigger.app_id`, and `ctx.trigger.user_id`. Use old/new to detect changes: `ctx.trigger.oldAccount.status.status != ctx.trigger.newAccount.status.status`. See [workflow expressions](/product/admin/expressions-workflows).
...
- **Grant source**: Where the grant came from (for example, a specific policy, manual assignment, or external system).
- **Grant type**: Whether the grant is permanent or time-limited.
- **Grant origin**: How the grant was created (for example, discovered during sync, provisioned through ConductorOne, or found via an external connector).
