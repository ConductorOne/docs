...
**Available data.** Populates `ctx.trigger.oldUser` (User object with previous values), `ctx.trigger.newUser` (User object with new values), and `ctx.trigger.user_id`. Use `ctx.trigger.oldUser` and `ctx.trigger.newUser` to detect state transitions, e.g.: `ctx.trigger.oldUser.department != ctx.trigger.newUser.department`
...
- **Grant source**: Where the grant came from (e.g., a specific policy, manual assignment, or external system).
- **Grant type**: Whether the grant is permanent or time-limited.
- **Grant origin**: How the grant was created (e.g., discovered during sync, provisioned through ConductorOne, or found via an external connector).
