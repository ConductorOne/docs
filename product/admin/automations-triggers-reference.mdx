...
**Available data.** On demand triggers populate `ctx.trigger.user_id` for the subject user selected at execution time. If triggered via the **Run automation** step, any JSON context passed by the parent automation is available in `ctx.trigger`. For requestable automations, form field values are available in `ctx.trigger.form_fields`. See [workflow expressions](/product/admin/expressions-workflows).
