...
| Authorization flow fails after the user consents | The redirect URI doesn't match. | Set it to exactly whichever matches your C1 tenant's domain (default instance: `https://accounts.conductor.one/auth/callback`; EU data residency instance: `https://accounts.c1eu.ai/auth/callback`) — see [Create the client credentials](#create-the-client-credentials). |
