...
<Step>
In the app settings sidebar, open **Agents**. Under **Slack Model Context Protocol (MCP) Server**, confirm the toggle is on — the manifest's `is_mcp_enabled` sets it, but check it here, because Slack's hosted MCP server rejects an app without it. If you created the app from scratch, turn it on now.
...
oauth_config:
  ...
  scopes:
    user:
      ...
      - pins:read
      - search:read
      # Optional — uncomment to enable write and admin tools, then add the
      # matching scopes in C1's authentication settings too. See the optional
      # scopes table below for the full set.
      # - chat:write
      # - reactions:read
settings:
  org_deploy_enabled: false
