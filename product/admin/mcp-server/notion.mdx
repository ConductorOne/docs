---
title: Set up the Notion MCP source
description: Connect Notion to C1 through the Notion API or Notion's own hosted MCP server, then add the Catalog MCP source and govern its tools.
og:title: Set up the Notion MCP source
og:description: Connect Notion to C1 through the Notion API or Notion's own hosted MCP server, then add the Catalog MCP source and govern its tools.
sidebarTitle: Notion
---
...
| **Setup effort** | Add the source in C1 and authorize — nothing to create in Notion first | Create a Notion integration (public or internal) first, then add the source in C1 |
...
<Note>
In your MCP source catalog, this option is listed as **Notion** — distinct from the **Notion API** entry, which connects through C1's own MCP server. If you don't see either, [contact the C1 support team](mailto:support@c1.ai) to enable it for your tenant.
</Note>
...
<Steps>
<Step>
Follow [Add an MCP source](/product/admin/mcp-servers#add-an-mcp-source) and select **Notion** from the catalog.
</Step>
<Step>
...
After you add the source, C1 runs tool discovery against Notion's MCP server. Discovered tools appear on the source's **Tools** tab and include Notion's own search and fetch tools, page and database creation and editing, page duplication, comments, teamspaces, users, and database views.
...
Notion supports two ways to authenticate, and you choose one when you add the source:
...
<Note>
In your MCP source catalog, this option is listed as **Notion API** — distinct from the **Notion** entry, which connects to Notion's own hosted MCP server. If you don't see either, [contact the C1 support team](mailto:support@c1.ai) to enable it for your tenant.
</Note>
...
With your public integration ready, add the source and provide its credentials to C1:

<Steps>
<Step>
Follow [Add an MCP source](/product/admin/mcp-servers#add-an-mcp-source) and select **Notion API** from the catalog.
</Step>
<Step>
...
With your internal integration secret ready, add the source and provide it to C1:
...
After you add the source, C1 runs tool discovery against Notion. Discovered tools appear on the source's **Tools** tab.
...
- **Rotate the OAuth client secret** on your public integration's **Configuration** tab in Notion, then update the secret on the source's authentication settings in C1.
