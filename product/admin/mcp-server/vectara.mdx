---
title: Set up the Vectara MCP source
description: Create a Vectara API key or OAuth app credentials, then add Vectara as a Catalog MCP source in C1 and govern its tools.
og:title: Set up the Vectara MCP source
og:description: Create a Vectara API key or OAuth app credentials, then add Vectara as a Catalog MCP source in C1 and govern its tools.
...
The Vectara Catalog MCP source lets you govern access to Vectara — corpora, documents, queries, and account data — as tools your AI clients can call through C1.

Vectara supports two ways to authenticate, and you choose one when you add the source:
...
<Note>
If you don't see **Vectara** in your MCP source catalog, [contact the C1 support team](mailto:support@c1.ai) to enable it for your tenant.
</Note>
...
### Add the source with an API key

With your API key ready, add the source and provide your credentials.

<Steps>
<Step>
Follow [Add an MCP source](/product/admin/mcp-servers#add-an-mcp-source) and select **Vectara** from the catalog.
</Step>
<Step>
...
### Add the source with OAuth2 client credentials

With your app credentials ready, add the source and provide them.
...
After you add the source, C1 runs tool discovery against Vectara. Discovered tools appear on the source's **Tools** tab.
