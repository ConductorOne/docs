---
title: Set up the Wiz MCP source
description: Create a Wiz service account, then add Wiz as a Catalog MCP source in C1 and govern the tools your AI clients can call.
og:title: Set up the Wiz MCP source
og:description: Create a Wiz service account, then add Wiz as a Catalog MCP source in C1 and govern the tools your AI clients can call.
sidebarTitle: Wiz
---
...
The Wiz Catalog MCP source lets you govern access to the Wiz cloud security platform — issues, vulnerabilities, cloud resources, and other data exposed by the Wiz GraphQL API — as tools your AI clients can call through C1.
...
<Note>
If you don't see **Wiz** in your MCP source catalog, [contact the C1 support team](mailto:support@c1.ai) to enable it for your tenant.
</Note>
...
## Add the Wiz Catalog MCP source in C1

With your service account credentials ready, add the source and provide them.

<Steps>
<Step>
Follow [Add an MCP source](/product/admin/mcp-servers#add-an-mcp-source) and select **Wiz** from the catalog.
</Step>
<Step>
...
After you add the source, C1 runs tool discovery against Wiz. Discovered tools appear on the source's **Tools** tab.
...
- **Rotate the client secret** by rotating the service account's secret in Wiz, then update the secret on the source's authentication settings in C1.
