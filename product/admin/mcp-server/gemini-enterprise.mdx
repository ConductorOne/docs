...
You now have the MCP server URL for your tenant.
...
gcloud agent-registry services create c1-mcp-gateway \
  ...
  --interfaces="url=https://YOUR_TENANT-mcp.conductor.one/v1,protocolBinding=jsonrpc"
```
...
The gateway routes and governs outbound agent traffic for the registry you just populated.
...
```text
projects/<your-project-id>/locations/us-central1/agentGateways/c1-agent-gateway
```
</Step>
<Step>
Select **Save**.
...
| The form rejects an empty **Client Secret** | **Use HTTP Basic Authentication** is still selected. | Clear that checkbox. See [Create the data store](#create-the-data-store). |
...
In C1, open **AI access management** > **AI clients**, find the Gemini Enterprise client, and use the **kill switch**. It revokes all tokens for that client immediately. See [Manage AI clients](/product/admin/ai-clients).
</Accordion>
