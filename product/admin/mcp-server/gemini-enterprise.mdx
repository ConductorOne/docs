...
Google is the party that should publish that document, since Gemini Enterprise is the client it describes. Today it does not: `/.well-known/oauth-client-metadata` returns `404` on `vertexaisearch.cloud.google.com`, `cloud.google.com`, `discoveryengine.googleapis.com`, and `accounts.google.com`. Google's instructions are to register the MCP server as an OAuth client with your identity provider and take a client ID from it.

Until Google publishes one, somebody else has to host it on Google's behalf.

| Who hosts it | Client ID | Status |
| :--- | :--- | :--- |
| Google | A Google-published URL | Not published. This is the outcome to prefer |
