...
<Note>
Use the **Custom MCP Server** card, not a server imported from Agent Registry. In C1 testing, a connector created from a registry import failed tool discovery with `Failed to reload custom actions` and a `FAILED_PRECONDITION` error, while an otherwise identical connector created from the Custom MCP Server card completed without error.
</Note>
...
<Warning>
Every user does this once, for themselves. Enabling actions as an administrator does not authorize anyone. Until a user authorizes here, the assistant answers that it has no ConductorOne integration, even though the actions are enabled and the connector is **Active**.
</Warning>
</Step>
<Step>
Ask a question that needs a C1 tool, naming the connector, such as "Using the C1 connector, list the access reviews in ConductorOne."
</Step>
<Step>
...
| **Failed to reload custom actions** on a connector imported from Agent Registry | Registry-imported connectors failed discovery in C1 testing with `FAILED_PRECONDITION`. | Recreate the data store from the **Custom MCP Server** card instead. See [Create the data store](#create-the-data-store). |
...
| The assistant answers that it has no ConductorOne integration | The user has not authorized the connector in the web app. Enabling actions does not authorize anyone. | In the message box, open **Connectors** and select **Authorize** beside your connector. See [Verify the Gemini Enterprise connection](#verify-the-gemini-enterprise-connection). |
