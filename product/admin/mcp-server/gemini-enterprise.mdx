...
| **Reload custom actions** succeeds with no error but no actions appear | C1 returned an empty tool list for the authorizing user, which happens when that user has no toolset. Authentication is fine: the connection appears in C1 under **AI & API** > **AI connections**. | Assign the authorizing user an access profile that includes the tools you expect, then reload again. See [Tools and toolsets](/product/admin/tools-and-toolsets). |
