...
- Someone who holds `roles/orgpolicy.policyAdmin` at the **organization** level. This is not grantable at the project or folder level, and project **Owner** does not include it.
...
| `roles/orgpolicy.policyAdmin` | **Organization** | Allow custom MCP data connectors |
...
The override is applied at the project level, but the person applying it needs `roles/orgpolicy.policyAdmin` at the organization level.
...
| **Client Secret** | Any placeholder, such as `unused` | The console treats this as required. C1 never reads it, because this client has no secret. |
...
Gemini Enterprise calls MCP tools *actions*, and imports every one of them turned off. Turn on only the actions your users need: Google recommends no more than 100 per data store, and a shorter list makes the agent's tool selection more accurate.
