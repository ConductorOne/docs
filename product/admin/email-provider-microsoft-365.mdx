...
og:description: Configure ConductorOne to send notification emails through your Microsoft 365 tenant using the Microsoft Graph sendMail API so recipients see your company's domain as the sender.
description: Configure ConductorOne to send notification emails through your Microsoft 365 tenant using the Microsoft Graph sendMail API so recipients see your company's domain as the sender.
...
- A licensed mailbox to send from (e.g., `governance@yourcompany.com`).
...
<Step>
Enter a description (e.g., C1 Email Sender) and choose an expiration. 24 months is the maximum; a shorter expiration is better practice.
</Step>
<Step>
Click **Add**.
</Step>
...
<Step>
Create the access policy, replacing `<APP_CLIENT_ID>` with the Application (client) ID from Step 1 and `<SENDER_MAILBOX>` with the address you'll use as the sender (e.g., `governance@yourcompany.com`):
...
If you want the app to send from multiple mailboxes (e.g., `governance@` and `security@`), create a mail-enabled security group containing those mailboxes and use the group's primary SMTP address for `-PolicyScopeGroupId` — one policy covers the whole group.
</Info>
...
- **Sender email address**: The mailbox you scoped in Step 4 (e.g., `governance@yourcompany.com`).
