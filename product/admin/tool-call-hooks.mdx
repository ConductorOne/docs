...
og:title: Tool call hooks - C1.ai docs
...
| :--- | :--- | :--- |
| **Pre-tool use** | Before C1.ai forwards the call to the MCP server | Inspect or rewrite the input, or deny the call |
...
C1.ai rejects a mismatch between the selected event and pattern when you save the hook.
...
When the built-in patterns don't fit, write a [function](/product/admin/functions) and attach it to a hook. C1.ai invokes the function with a JSON payload describing the call and uses the return value to decide whether to allow, modify, or deny.
