...
**Error behavior:** Fails if the channel doesn't exist or the C1 Slack app doesn't have access. Requires the [Slack application](/product/admin/slack-application) to be configured.
...
- **From ConductorOne user data**: Automatically maps the C1 user's profile fields (name, email, department, and so on) to the target app's account schema. Use this when the target app's fields align with standard user attributes.
