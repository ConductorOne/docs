...
After adding an MCP source, every tool it exposes starts life as **Unset**. This page covers reviewing those tools, approving the safe ones, classifying them, bundling them into toolsets, and binding toolsets to access profiles so end users can request them.
...
Discovered tools live under each MCP source's own **Tools** tab.

<Steps>
<Step>
In **AI > MCP Sources**, open an MCP source.
</Step>
<Step>
Click the **Tools** tab.
</Step>
...
C1 periodically re-runs tool discovery against each MCP source. When the inventory changes:
...
C1 ships and auto-maintains two toolsets per MCP source:

- **All approved tools** — every tool in **Approved** state on that source.
- **Read tools** — every **Approved** tool on that source with Classification = Read.

Their display names are suffixed with the source name — for example, `All approved tools - Slack` and `Read tools - Slack`.
...
<Step>
Add the toolset as an entitlement — both C1-maintained toolsets (**All approved tools**, **Read tools**) and custom toolsets appear as options.
</Step>
<Step>
