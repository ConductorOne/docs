...
description: "Technical reference for C1 functions, including SDK namespaces, runtime constraints, configuration options, and troubleshooting."
og:description: "Technical reference for C1 functions, including SDK namespaces, runtime constraints, configuration options, and troubleshooting."
...
Configure secrets, network access, and scopes in the C1 UI:
...
The config drawer has **Secrets** and **Outbound network access** sections. API access is configured separately by linking a service principal — see [API access](#api-access) below. On legacy tenants, the drawer also includes a **Scopes** section — see [Scopes (legacy)](#scopes-legacy).
...
<Warning>
**Legacy.** Scopes are being replaced by linked service principals. New functions should use [API access](#api-access) above. This section documents the legacy path for tenants that haven't been migrated yet.
</Warning>

On legacy tenants, functions are granted API access by assigning roles in the **Scopes** section of the config drawer. The function's SDK credentials are created with only the permissions those roles allow.

Currently available on legacy:

- **Read-Only Administrator**: Read-only access to all C1 APIs (such as users, apps, entitlements, tasks). The function can query data but cannot create, update, or delete resources.
...
When your tenant is migrated to the service principal model, existing functions are automatically linked to a service principal with the same effective access; no action is required.
...
- Check the linked service principal's roles — the function authenticates as that SP and is authorized against its roles
- If the function isn't linked to a service principal yet, link one (see [API access](#api-access))
- On legacy tenants, check the function's **Scopes** configuration instead (see [Scopes (legacy)](#scopes-legacy))
- Verify your user has permissions for the API operation
