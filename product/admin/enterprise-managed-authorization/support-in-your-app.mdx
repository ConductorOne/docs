...
This is the protocol-level page. Enterprise-managed authorization is built on the open Cross-App Access (XAA) standard, and the token C1 issues is an ID-JAG; both are used directly below because they're what you implement against.
...
- **`scope` omitted (or blank):** ConductorOne grants the subject's **default full set** at the resource server — every *enabled* scope the subject is actually entitled to (`enabled-at-RS ∩ subject-held`). This is the path most silent clients use (e.g. Claude Code's silent exchange sends no `scope`).
