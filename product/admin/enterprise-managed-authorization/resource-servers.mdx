...
Registering an MCP server is the core admin task in enterprise-managed authorization. It tells C1 which server to issue tokens for and how those tokens are addressed. This page also carries the full onboarding path, so you can follow a single server from enablement through to a user requesting access.
...
2. Register the MCP server (this page).
3. [Enable the server's scopes and bundle them into access profiles](/product/admin/enterprise-managed-authorization/scopes-and-profiles) so users can request them. That page covers both steps: discovering or declaring scopes, and grouping them into profiles.
4. Users request access through the standard C1 catalog, and approvers grant it. Once a profile exists and your access request settings are configured, there's no further step here.
...
The MCP server is registered on the C1 application that represents it. Open the application (**Apps** in the left navigation), then open its **Cross-App Access** tab and click **Register resource server**. The **Cross-App Access** tab only appears once enterprise-managed authorization is enabled for your tenant (see [Enable enterprise-managed authorization](/product/admin/enterprise-managed-authorization/enable)). Clicking **Register resource server** opens the **Register resource server** drawer, where you fill in the fields below.

<Steps>
<Step>
On the application, open the **Cross-App Access** tab and click **Register resource server**.
...
<Step>
Optionally set a **Maximum grant lifetime** and a **Signing algorithm**.
...
If a token request is denied, or a server rejects a token C1 issued, match the symptom below.
