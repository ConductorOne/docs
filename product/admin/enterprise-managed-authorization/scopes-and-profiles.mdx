...
Scopes and access profiles move through the standard C1 workflow. Users request access, approvers grant it, and reviewers certify it on a schedule.

- Configure how access is requested and approved in [Access requests](/product/admin/access-requests).
- Curate the bundles users request in [Access profiles](/product/admin/profiles).
- Certify access over time with [Campaigns](/product/admin/campaigns).

Once a profile exists and your access request settings are configured, users can request it. There's no further setup step.
