...
og:title: c1i command reference | Docs - C1.ai
...
c1i interacts with the C1.ai API to manage users, apps, accounts, entitlements, tasks, and access requests.
...
| `users` | `list` | Search and list C1.ai users. |
...
| `mcp` | `gateway list-tools` `gateway call` | Call the C1.ai MCP gateway directly (list and invoke tools end to end). |
| `api` | | Make raw C1.ai API requests. |
...
| Flag | Description |
| :--- | :--- |
| `--url string` | C1.ai tenant URL. |
| `-h`, `--help` | Help for any command. |

## Docs

Explore C1.ai API documentation and schemas. These commands do **not** require authentication.
...
| :--- | :--- |
| `search` | Search C1.ai documentation by keyword. |
...
Search C1.ai documentation by keyword.
...
Authenticate to C1.ai. Opens a browser for OAuth device flow authorization, or accepts credentials directly.
...
Check whether valid C1.ai credentials are stored and working. Reports which source (environment, keyring, or file) served the active credentials.
...
The token is audience-scoped to the C1.ai API host.
...
Search and list C1.ai users. Outputs NDJSON.
...
| Flag | Description |
| :--- | :--- |
| `--user-id strings` | C1.ai user ID to set as owner (repeatable; replaces the full owner list). (Required.) |
...
| `--user-id string` | C1.ai user ID to set as owner. (Required.) |
...
Manage policies — the objects that describe how C1.ai processes a task (an access request, a certification, a provisioning action): who approves it, what happens on escalation or timeout, and how the underlying resource gets provisioned.
...
Call the C1.ai MCP gateway directly over its MCP transport — the same handshake an MCP host performs — to verify what a registered server actually exposes. This closes the configure-then-verify loop: register a server, approve its tools, then list or call them here.
...
Make raw C1.ai API requests. This is an escape hatch for accessing any API endpoint not covered by the built-in commands.
...
<Note>
c1i sends `--body` to the API verbatim — it does not wrap or reshape the payload. If the endpoint's schema nests fields under a top-level key (for example, `POST /api/v1/iam/personal_clients/{id}` expects `{"client": {"displayName": "...", "scopedRoles": ["..."]}, "updateMask": "displayName,scopedRoles"}`), your `--body` must include that wrapper. Use `c1i docs endpoint <path>` to see the exact request shape before calling.
</Note>
...
| Flag | Effect |
| :--- | :--- |
| `--page-size N` | Per-call batch size. The C1.ai API caps this at 100; c1i clamps client-side, so `--page-size 500` is treated as 100. Default is 50. |
