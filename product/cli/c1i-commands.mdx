...
<Note>
c1i sends `--body` to the API verbatim — it does not wrap or reshape the payload. If the endpoint's schema nests fields under a top-level key (for example, `Update Service Principal` expects `{"servicePrincipal": {"displayName": "...", "user": {"roleIds": [...]}}}`), your `--body` must include that wrapper. Use `c1i docs endpoint <path>` to see the exact request shape before calling.
</Note>
