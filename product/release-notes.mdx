...
- **Data lineage for agentic reports**: When the C1 AI assistant generates a report, you can now open its **Data lineage** view to see the exact queries behind each number, confirm each source was verified against the execution log, and copy the lineage for audit workpapers. See [Verify report sources with Data lineage](/product/admin/ai-assistant#verify-report-sources-with-data-lineage).

- **Multi-chart dashboards and composed reports**: Ask C1 AI for multiple charts and they now render together as an aligned grid, each with its own export and sources. C1 AI can also answer reporting questions with a composed report — stat cards, a trend chart, and a data table in one response, each showing the sources behind it. See [Generate visual reports](/product/admin/ai-assistant#generate-visual-reports).

- **Generate CEL conditions from plain English**: Writing a CEL condition previously meant working through an AI chat to iterate toward the right expression. Describe the condition you need in plain English and the CEL editor's **Generate** action drops the expression directly into the field in one step. See [Generate an expression with AI](/product/admin/expressions#generate-an-expression-with-ai).
...
- **Register MCP servers past relaxable OAuth discovery issues**: External and hosted MCP servers that failed a relaxable OAuth discovery check — for example, an issuer mismatch or a cross-host authorization server — previously couldn't be registered at all. An admin can now review the flagged findings and register the server anyway. See [Configure authentication](/product/admin/mcp-servers#configure-authentication).

- **Hosted MCP servers use their own declared tool prefix by default**: Hosted MCP servers without an admin-set tool prefix previously fell back to a generic derived name. They now use the prefix the connector itself declares, so tool names stay recognizable to AI clients by default. See [Configure server settings](/product/admin/mcp-servers#configure-server-settings).
