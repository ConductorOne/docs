...
- **Customizable columns for app account grants**: The **Grants** tab on an application account's page now supports configurable columns, including resource type, description, grant count, and grant origins (when grant reasons are enabled), alongside the entitlement and revoke controls.
