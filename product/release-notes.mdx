...
---
{/* Editor Refresh: 2026-03-27 */}

<Update label="March 27, 2026">

### Role mining

Role mining is now available as a dedicated page at **Governance** > **Role mining**. ConductorOne continuously analyzes access patterns across your organization and surfaces suggested access profiles, grouped by cohorts like department, job title, and manager's team. Each suggestion includes a confidence score for every recommended entitlement, so you can quickly assess fit before accepting. You can also define a custom cohort and run a targeted analysis on demand.

Accepting a suggestion creates a new access profile with the recommended entitlements, ready for you to name and start enrolling members.

To learn more, see [Use role mining](/product/admin/role-mining).

### External insights with CrowdStrike Falcon Identity Protection (early access)

ConductorOne can now sync identity risk scores from CrowdStrike Falcon Identity Protection and surface them in context during access decisions. Reviewers see an identity's risk score and contributing risk factors directly on access review tasks, in the task log, and on access request approval tasks — without leaving ConductorOne.

External insights are enabled automatically once a supported connector is configured and syncing. No additional setup is required.

To learn more, see [External insights](/product/admin/external-insights).


### Usability improvements

- Spotlight search results for groups and access profiles now link directly to their dedicated detail pages, and appear under separate **Groups** and **Access Profiles** headings.
...
- URLs in entitlement descriptions are now automatically clickable wherever they appear, including detail pages, tables, tooltips, and access request forms.
- The entitlement grants table now displays account status (Enabled/Disabled) as a dedicated column instead of inline on the account name.
- App entitlements can now be filtered by requestable status using a new **Requestable** / **Not requestable** filter.
- The entitlement grants tab now supports filtering by account status.
- A **Cancel** button is now available when editing entitlement attributes, so you can discard changes without saving.
- Editing resource names in the new app details page is supported again.
- Clearing an expiration date field on a grant no longer causes an error.
- Uploading an avatar, app icon, or profile icon no longer fails if you don't interact with the crop controls before saving.
- Resource counts on app detail page tabs now respect visibility settings — hidden resources are no longer included in counts for users without access.
- System-only API roles are no longer shown in the entitlements list.
- Tooltip text and form helper text now have better contrast in dark mode.
