...
You can now assign a workflow step in a request or review policy to an **Agent**. The agent evaluates each request that reaches the step using C1's built-in guidance plus any custom instructions you provide.

When adding an **Agent** step, configure:

- **Agent mode** — whether the agent can comment only, take full control of the step, or change policy steps only
- **Agent failure action** — what happens if the agent can't reach a decision: reassign to users you select, reassign to super admins, or skip the step
- **Agent instructions** — free-text guidance describing the conditions under which the agent should approve or deny

{/* Docs update needed: /product/admin/policies and /product/admin/audit-ai-tool-usage don't yet document the System tab or the C1 System identity attribution for these evaluations. */}

Each agent evaluation is attributed to the built-in **C1 System** agent identity and labeled as an AI-agent action in the audit log. Every evaluation is also recorded as a conversation you can review under **AI > Conversations**, **System** tab.

See [Policies](/product/admin/policies#assign-for-review).

{/* Screenshot needed: the Agent step configuration panel in the policy rule editor (Agent mode / Agent failure action / Agent instructions fields). Placeholder path below. */}

<Frame>
<img src="/images/product/assets/PLACEHOLDER-agent-policy-step.png" alt="The policy rule editor showing an Agent step with Agent mode, Agent failure action, and Agent instructions fields."/>
</Frame>

### Search for what you need, see what grants it

{/* Docs update needed: /product/how-to/create-requests documents App catalog search by entitlement name, not this behavior. Verified in product: search results add an "Entitlements matching '<term>'" section, each row has a "View access graph" link, and the expanded graph shows a GRANTS list with the searched-for item tagged "You searched for this". Seen on both the self-service Requests > App catalog page and the admin "Create a new request" flow. */}

Searching the request catalog for a specific resource — a repo, a folder, a project — previously only matched entitlements by name, so if the resource wasn't in an entitlement's name, you wouldn't find it. Search results now include an **Entitlements matching** section listing entitlements that indirectly grant access to what you searched for — for example, a GitHub team whose repository access includes the repo you need. Click **View access graph** on any result to see the full grant chain, with your match highlighted. You can find the right thing to request without knowing which group owns the resource you're after.

See [Request an app or permission](/product/how-to/create-requests#request-an-app-or-permission).

{/* Screenshot needed: the "View access graph" expansion (see 3b/3d in reference screenshots) is the clearest single shot — shows the requested team entitlement plus its GRANTS list with the searched-for repo highlighted "You searched for this". Placeholder path below. */}

<Frame>
<img src="/images/product/assets/PLACEHOLDER-catalog-search-grants.png" alt="The access graph for a requested entitlement, showing its granted repositories with the searched-for repository highlighted as a match."/>
</Frame>
...
- **Custom scheduling for campaign templates**: Campaign templates previously offered only fixed recurrence presets. You can now set a template to recur on any custom interval — for example, every 2 days or every 3 months — and choose exactly how long each campaign runs. See [Manage campaigns](/product/admin/campaigns).

{/* Docs update needed: /product/how-to/review-tasks doesn't cover resource attributes or an IAM policy viewer. */}

- **Readable IAM policy documents**: Reviewing an AWS resource's access previously meant reading its IAM policy document as an unformatted block of raw JSON. Resource attributes that carry an IAM policy document now show a **View policy** button that opens it as a readable, collapsible JSON tree — available on resource-level access reviews and in task details. See [Review tasks](/product/how-to/review-tasks).

### Access request improvements

{/* Docs update needed: /product/admin/managing-entitlements doesn't cover entitlements created via Terraform ahead of connector sync, or this wait-for-sync behavior. */}

- **Entitlements created ahead of sync no longer block requests**: Requesting access to an entitlement created ahead of a connector sync — for example, through Terraform — previously failed to provision because C1 didn't yet recognize the entitlement. These requests now wait instead, show why they're waiting, and resume automatically once the next sync completes, falling back to manual provisioning after 24 hours. Contact the C1 Support team to enable this feature. See [Managing entitlements](/product/admin/managing-entitlements).

{/* Docs update needed: /product/admin/groups FAQ currently says the opposite ("C1 groups cannot be added to access profiles") and needs updating for this change. */}

- **Requestable C1 groups**: C1 groups can now be added to a request catalog or access profile, so users can request group membership through the same approval, duration, and revocation flow as other requestable access. Members added through a request aren't removed by the group's dynamic membership rules. See [Groups](/product/admin/groups).

### Task approval improvements

{/* Docs update needed: /product/how-to/access-change-tasks doesn't document the Retry provisioning action yet. */}

- **Retry provisioning without restarting the task**: When connector provisioning fails on an access task, you previously had to cancel the task and start over once the issue was fixed, losing any approvals already collected. You can now retry provisioning directly on the task with the new **Retry failed provisioning** action. See [Access and change tasks](/product/how-to/access-change-tasks).

{/* Docs update needed: /product/admin/step-up-auth documents the step-up flow but doesn't mention the comment-required case. */}

- **Step-up approvals that also require a comment**: Approving a task that required both step-up (MFA) authentication and a comment previously failed with an error and couldn't be completed. Approvers can now complete both requirements together — the approval prompts for step-up verification with your identity provider and completes once verified. See [Step-up authentication](/product/admin/step-up-auth).

### Agentic AI and MCP improvements

- **Per-user authentication for external MCP servers**: Admins can now require each user to authenticate with their own credential — OAuth2, bearer token, custom header, or basic auth — instead of sharing a single admin credential. See [Set up an MCP server](/product/admin/mcp-servers).

{/* Docs update needed: /product/admin/ai-assistant has no section on data lineage or report sourcing. */}

- **Data lineage for agentic reports**: When the C1 AI assistant generates a report, you can now open its **Data lineage** view to see the exact queries behind each number, confirm each source was verified against the execution log, and copy the lineage for audit workpapers. See [Ask C1 AI](/product/admin/ai-assistant).

{/* Docs update needed: /product/admin/ai-assistant doesn't document chart/dashboard generation or composed reports. */}

- **Multi-chart dashboards and composed reports**: Ask C1 AI for multiple charts and they now render together as an aligned grid, each with its own export and sources. C1 AI can also answer reporting questions with a composed report — stat cards, a trend chart, and a data table in one response, each showing the sources behind it. See [Ask C1 AI](/product/admin/ai-assistant).

{/* Docs update needed: /product/admin/expressions doesn't document an AI-assisted generate action. */}

- **Generate CEL conditions from plain English**: Writing a CEL condition previously meant working through an AI chat to iterate toward the right expression. Describe the condition you need in plain English and the CEL editor's **Generate** action drops the expression directly into the field in one step. See [Write expressions](/product/admin/expressions).

{/* Docs update needed: /product/admin/tools-and-toolsets documents tenant defaults and per-tool overrides, but not a server-level auto-enable setting. */}

- **Tool enablement settings on individual MCP servers**: You can now set whether an MCP server's tools require enablement or are auto-enabled directly on the server, independent of your tenant's default AI governance setting. See [Govern tools and toolsets](/product/admin/tools-and-toolsets).

{/* Docs update needed: /product/admin/mcp-servers doesn't cover OAuth discovery review/acknowledgment for issuer mismatches or cross-host authorization servers. */}

- **Register MCP servers past relaxable OAuth discovery issues**: External and hosted MCP servers that failed a relaxable OAuth discovery check — for example, an issuer mismatch or a cross-host authorization server — previously couldn't be registered at all. An admin can now review the flagged findings and register the server anyway. See [Set up an MCP server](/product/admin/mcp-servers).

{/* Docs update needed: /product/admin/mcp-servers says C1 generates a default prefix if you don't set one, but doesn't yet mention this new fallback to the connector's own declared prefix. */}

- **Hosted MCP servers use their own declared tool prefix by default**: Hosted MCP servers without an admin-set tool prefix previously fell back to a generic derived name. They now use the prefix the connector itself declares, so tool names stay recognizable to AI clients by default. See [Set up an MCP server](/product/admin/mcp-servers).
