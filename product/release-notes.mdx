...
We've reorganized parts of the C1 left navigation to better reflect how the product has grown:

- **AI** is now a top-level section, consolidating the AI access management surfaces that were previously scattered under Integrations and Settings. MCP server configuration lives under **AI > MCP**, and C1 Gateway settings, connected clients, and hooks are under **AI > C1 Gateway**.
- **Identity security** is a new top-level section. Inventory has moved here from Explore.
- In the **Directory** section, **User data sources** has been renamed to **Directory sources**, and **Groups** has been renamed to **Dynamic groups**.

### Customizable columns on dozens more pages

The column configurator is now available on 23 additional pages across the product — including Secrets, Findings, Request Catalog, Webhooks, Decoys, API Keys, System Log, Bridges, Vaults, Service Principals, Auth Configs, MCP Servers, Shadow Apps, Policies, Conflict Monitors, Automations, and Automation Executions. Show, hide, and reorder columns to match how your team works, with preferences saved across sessions.

### Entitlement owners now visible in the UI and exports

The App Entitlements table now includes an **Owners** column so you can see at a glance who's responsible for each entitlement. Owners are also included in the entitlements CSV export, matching the existing behavior on the App Resources table.

### Navigate between rows without closing drawers

When you open a drawer from a list view — task logs, function invocation history, or access-review campaigns — you can now move to the next or previous row using arrow buttons in the drawer header, without closing and reopening for each item. Drawer headers now always appear consistently, giving the close button a stable location that no longer overlaps page content.

### Richer guidance on requestable form fields

{/* DOCS: Check whether the requestable items / form configuration docs need to mention markdown support. */}

Admins can now use markdown in the helper text on requestable form fields — add hyperlinks, bold text, and lists to give requesters clearer, more useful guidance when filling out a request.

### Task-state filters persist in links

Task-state filters on the Task log and Requests pages now persist in the URL. Sharing or bookmarking a filtered view keeps the filter applied when the link is opened.

### Bulk revoke access from the Applications area

App owners can now revoke access directly from the Applications area, without navigating to a separate flow. On an account's **Grants** tab, revoke a single entitlement inline or select multiple and revoke them together — the list is also now searchable. On an application's **Accounts** page, select one or more accounts and revoke specific entitlements or remove all access at once, from the bulk action menu or the per-row menu.

### More flexible access conflict conditions

{/* DOCS: Check the conflict monitor docs to see if the Is not in condition option needs to be documented. */}

Access conflicts can now trigger when a user is in Group A but **not** in Group B — useful for flagging users who have sensitive access without the compensating control that's supposed to go with it. Previously, conflicts only triggered when a user held both entitlements simultaneously.

### Search for what you need, find the role that grants it

{/* TODO: This feature is SKU_MANUAL on the Platform SKU (promoted July 2). Confirm how to qualify it — e.g. "available on [plan name]" or a separate callout. */}
{/* DOCS: New feature — request catalog indirect access search likely needs a docs page or section. */}

Searching the request catalog for a specific resource — a repo, a folder, a project — now surfaces the roles, teams, or groups that grant access to it, even when the search term doesn't match those entitlements by name. Instead of needing to know which team owns the resource they want, users can search for the resource itself and see the requestable entitlement that covers it, with a link to the full access graph showing everything it grants.

### Condition builder for Directory source imports

{/* DOCS: New feature — Directory source import conditions need to be documented. The engineer provided these steps as a starting point: go to Directory sources settings, select the three-dot menu to the right of the directory, select Condition, build the condition, then use Preview to verify the count and spot-check individual users. */}

Controlling which accounts from a Directory source get imported into C1 previously required writing a raw CEL expression — error-prone and hard to validate before saving. The import configuration now uses the same visual condition builder available elsewhere in C1, so admins can define import rules using UI controls instead of hand-written expressions. Advanced users can still switch to Custom CEL for conditions the builder doesn't cover.

A built-in **Data preview** panel lets you verify your condition before saving: see how many source accounts will be imported, search for specific accounts and check whether they'd be included, and hover over a match result to see exactly why it matched. Existing CEL conditions that are builder-compatible will open in the builder automatically; unsupported expressions stay editable in Custom CEL.

### Edit post actions on built-in certification policies in the UI

{/* DOCS: Check the policy configuration docs to see if UI steps for editing post actions need to be added. */}

Admins can now edit the follow-up steps (post actions) on the built-in "App owner certification" policy directly in the UI. Previously this was only possible through the API.

### Disable connector anomaly pauses tenant-wide

{/* DOCS: This likely needs a new entry or section in the connector management / system settings docs. */}

A new **System management** setting lets admins turn off connector anomaly pauses across all connectors at once, instead of managing this per connector.

### Expiring API credentials in the daily digest

{/* DOCS: Check the daily digest / notifications docs to see if expiring credential alerts need to be documented. */}

System Owners now receive a notice in their daily digest email when service principal API credentials are approaching expiration or have already expired. Credentials expiring within 30 days are listed with a countdown and a direct link to the credential in the admin directory.

### Agentic AI and MCP improvements

**Richer AI assistant responses in Slack** — The C1 AI assistant in Slack now presents buttons, forms, and pickers directly in the conversation when the situation calls for them. Users can fill out forms, select from options, and confirm actions without leaving Slack, bringing the Slack experience in line with the web app.

**Slack conversations in the admin console** — The Conversations page under Agentic AI now has separate **Web** and **Slack** tabs, giving admins visibility into agent activity across both surfaces. The Slack tab supports search by title and read-only conversation transcripts.

**MCP server setup without client credentials** — Hosted and external MCP servers that support OAuth Dynamic Client Registration (DCR) can now be connected without entering a client ID or secret. Enable **Use dynamic client registration** when adding the server and C1 registers itself with the authorization server automatically.

{/* DOCS: The MCP server setup guides will need to cover the DCR option for both hosted and external servers. */}

**Per-user authentication for external MCP servers** — Admins can now require each user to authenticate to an external MCP server with their own credential — OAuth2, bearer token, custom header, or basic auth — instead of sharing a single admin credential.

{/* TODO: This feature is SKU_MANUAL. Confirm with dev team how to qualify it — e.g. "available on [plan name]" or behind a separate callout. */}

**Toolset tools visible on hover** — Reviewers can now see which tools are included in a toolset by hovering over it in the task view, without navigating into entitlement detail pages.

{/* TODO: PR #20754 removed the Toolsets column from the MCP servers list and server Tools tab. No release note added since it's a removal, but check if the MCP server docs reference this column and need updating. */}

### Access profile membership changes now apply continuously

Access profile grants and removals now stream through to your connected apps and infrastructure as they happen, instead of waiting for the next hourly sync. Access changes reflect faster across the board — no configuration needed, and existing automation configurations carry over as-is.

We also strengthened the safety cap that pauses an automation when it's about to remove an unusually large number of members at once. The cap now tracks removals cumulatively over a rolling hour rather than evaluating one update at a time, so it catches situations where several smaller removals add up to something risky. You can now view and adjust this threshold directly in your automation settings — previously it was only configurable by the support team.

{/* DOCS: Check the automations docs to see if the rolling-hour safety cap and the new threshold setting need to be documented. */}

### Bug fixes

- The AI assistant now correctly surfaces request action tools for super administrators, resolving a "request actions tools aren't available" error they could encounter.
...
Contact the C1 support team at support@c1.ai if you have questions or run into issues during migration.
