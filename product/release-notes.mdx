...
Searching the request catalog for a specific resource — a repo, a folder, a project — now surfaces the roles, teams, or groups that grant access to it, even when those entitlements don't match the search term by name. Users can find the right thing to request without knowing which group owns the resource they're after.

### Bulk revoke access from the Applications area

App owners can now revoke access directly from the Applications area. On an account's **Grants** tab, revoke entitlements individually or in bulk — the list is also searchable. On the **Accounts** page, select one or more accounts and revoke specific entitlements or remove all of their access at once.

### Access profile changes now apply in real time
...
We also strengthened the safety cap that pauses automations before removing an unusually large number of members. The cap now tracks removals over a rolling hour rather than evaluating one update at a time, catching situations where smaller removals add up to something risky. You can now view and adjust the threshold in your automation settings — previously this required contacting support.
...
Configuring which accounts from a Directory source get imported into C1 previously required a raw CEL expression — difficult to write correctly and impossible to validate before saving. The import configuration now uses the same visual condition builder available elsewhere in C1. A built-in **Data preview** panel shows how many accounts match and lets you spot-check individual accounts before saving. Custom CEL remains available for advanced cases.

### Agentic AI and MCP improvements

- **Richer AI assistant responses in Slack**: The C1 AI assistant in Slack now uses rich interactive responses — buttons, forms, and pickers — so users can complete tasks without leaving the conversation, matching the web app experience.

- **Slack conversations in the admin console**: The Conversations page under Agentic AI now has separate **Web** and **Slack** tabs with full visibility into agent activity across both surfaces. The Slack tab supports title search and read-only transcripts.

- **MCP server setup without client credentials**: Hosted and external MCP servers that support OAuth Dynamic Client Registration (DCR) can now be connected without entering a client ID or secret. Enable **Use dynamic client registration** when adding the server and C1 handles registration automatically.
...
- **Toolset contents visible on hover**: Reviewers can see which tools a toolset includes by hovering over it in the task view, without navigating into entitlement detail pages.
...
- **Entitlement owners in the table and exports**: The App Entitlements table now includes an **Owners** column, also reflected in the CSV export.

- **Row-by-row drawer navigation**: Drawers opened from list views now include next/previous controls in the header, so you can move through rows without reopening.

- **Markdown in requestable form field helper text**: Admins can now use markdown — links, bold, lists — in the guidance shown to requesters on form fields.

{/* DOCS: Check whether the requestable items / form configuration docs need to mention markdown support. */}

- **Task-state filters persist in links**: Filters on the Task log and Requests pages now persist in the URL, so shared or bookmarked links keep the filter applied.

- **More flexible access conflict conditions**: Conflict monitors now support an **Is not in** condition for Group B — flag users who hold sensitive access without the compensating entitlement that should go with it.

{/* DOCS: Check the conflict monitor docs to see if the Is not in condition option needs to be documented. */}

- **Disable connector anomaly pauses tenant-wide**: A new **System management** setting lets admins disable connector anomaly pauses across all connectors at once.

{/* DOCS: This likely needs a new entry or section in the connector management / system settings docs. */}

- **Expiring API credentials in the daily digest**: System Owners now see service principal credentials expiring within 30 days — or already expired — in their daily digest email, with a countdown and a direct link.

{/* DOCS: Check the daily digest / notifications docs to see if expiring credential alerts need to be documented. */}

- **Edit built-in certification policy post actions in the UI**: Admins can now edit the post actions on the built-in "App owner certification" policy in the UI, without using the API.

{/* DOCS: Check the policy configuration docs to see if UI steps for editing post actions need to be added. */}

- **Fixed**: The AI assistant now correctly surfaces request action tools for super administrators.
