...
{/* DRAFT STATUS: Session 1 complete. Built from both PR digest chunks (Aug 21-28 and Aug 28-Sept 3), colleague-provided copy for two docs-update projects (Agent classifiers, Govern MCP resources), a docs update for the guided MCP connect flow (ductone/c1#24062/#24638), and a system-log schema note (ConductorOne/docs#511). All inputs for this cycle are in as of 2026-09-03. Remaining before publish: final section ordering/headline-item selection, and the Session 2 verify/polish pass resolving every inline TODO/QUESTION comment. */}

### Agent classifiers

{/* SOURCE: colleague docs-update copy, not from PR digest. Doc page (product/admin/agent-classifiers.mdx) carries its own "Early access" warning banner - matches precedent for EA framing elsewhere in this file. Could not confirm a single matching feature-flag ID in ductone/c1 (closest candidates: AIGOV_AGENT_GUARDRAILS = SKU_ALL/GA, AI_GOVERNANCE_FEATURES_PREVIEW = SKU_MANUAL); flagging for a tighter check in Session 2 if needed. */}
...
**Classifiers** evaluate that combination at call time: write rules over private-data sensitivity, untrusted-content exposure, and exfiltration capability, then route matches to a hook, a human approval, or an outright block. Roll out safely with **Observe mode**, which logs what a rule would have caught before it can deny anything.

*Early access.* See [Agent classifiers](/product/admin/agent-classifiers) for how rules, hooks, and Observe mode fit together.

### Govern MCP resources

{/* SOURCE: colleague docs-update copy, not from PR digest. TODO: gated FEATURE_FLAG_ID_MCP_RESOURCE_SURFACE = SKU_MANUAL as of 2026-09-03 ("resource governance is in development"); doc page carries an "Activation required - contact support" note. User is confirming rollout status before publish - remove this section if it hasn't GA'd. */}

Admins can now review and approve MCP resources and URI templates, not just tools. When C1 discovers a server that exposes static resources or URI templates, they appear on a new **Resources** tab with a **Pending** state until an admin approves them. Each approved resource gets its own read entitlement, so access to a resource is governed separately from tool and toolset access — a user needs an explicit grant to read it. C1 also logs every allowed and denied resource read in the audit log.

See [Govern MCP resources](/product/admin/mcp-resources) for how to review, approve, and audit discovered resources.

### C1 MCP improvements

- **A guided flow for connecting MCP servers**: Registering an MCP server now walks through one question at a time — which server, where it's hosted, the URL, who can use it and when, what happens to newly discovered tools, and how it authenticates — instead of a single multi-step form. Auth questions adapt automatically: a self-registering (DCR) server asks nothing extra, since C1 probes its OAuth configuration for you.

  {/* SOURCE: ductone/c1 PR #24638, making the guided V2 flow (PR #24062) the default at both registration entry points. #24062 is gated on AI_GOVERNANCE_FEATURES_PREVIEW (SKU_MANUAL as of 2026-09-03) and its own Release Notes section says "no release notes needed"; #24638 keeps that flag/SKU mapping intact per its description and was still OPEN as of 2026-09-03. Added on the user's PM-sourced info that this ships today - user is re-checking this afternoon and will pull this item if #24638 hasn't landed or the flow isn't actually broadly available. See also the Warning added to product/admin/mcp-servers.mdx about the access-profile-selection gap. */}

### Access requests and reviews

- **Set bulk request limits per tenant**: The number of entitlements a user could request at once in bulk used to be a single fixed limit for every tenant. Admins can now set their own max (1-100, default 10) from the Forms settings drawer, and the limit is enforced server-side so an over-limit request fails instead of silently succeeding.

  {/* PR #23942, #23936, #23932 */}

- **Duration limits apply before submission, not after**: A request could previously select "Indefinite" access, or an invalid duration, even when the entitlement had a max-duration policy — only to fail or get silently adjusted later. Requests now respect the entitlement's duration limit up front: "Indefinite" doesn't appear as an option when a max is set, and invalid existing durations are auto-adjusted.

  {/* PR #23931 */}

- **See account status right on the request**: Access-request flows now show a status label (e.g. "Suspended") next to the target user, and flag when "already granted" access actually belongs to a suspended account.

  {/* PR #23858 */}

- **"Certify All" only counts what's actually certified**: The count next to a campaign's "Certify All" action used to include tasks that were pending but never submitted, overstating how much a reviewer had actually finished. It now reflects only tasks that will actually be certified.

  {/* PR #24027 */}

- **Fewer false "still open" prompts when ending a campaign**: Ending a campaign showed a "not everything is reviewed yet" warning even when every review was already in a terminal state, including cancelled ones. That prompt no longer fires in that case.

  {/* PR #24019 */}

- **No more duplicate review notifications**: Access-review notifications now go only through the newer (V2) dispatch path; reviewers no longer get a duplicate notification from the legacy path alongside it.

  {/* PR #24017 */}

- **Tell apart same-named app accounts on audit reports**: The access review audit report can now include an optional "Subject username" column, so two app accounts sharing a display name don't get confused with each other.

  {/* PR #23838 */}

- **Campaign notifications can go to Microsoft Teams**: Campaign notification templates now show a Microsoft Teams channel option alongside Slack.

  {/* PR #24144 */}
  {/* TODO: check MS Teams campaign-messages flag status in ductone/c1 (features_*.go) before publishing - source material notes this is behind a flag. */}

- **Bulk actions match what single-row actions already did**: "Select all X in filter" on a campaign's task list no longer offers a bulk Cancel or Skip step when the filtered set includes closed tasks — matching the row-level "Select 100" behavior. When a bulk action finishes with some failures, the banner now says "N succeeded, M failed" instead of a generic "Bulk action completed," and progress bars and task counts refresh automatically afterward.

  {/* PR #24707 */}

- **Mid-campaign reassignments show up in submissions**: When a certifier resolves a task directly from the task execution plan — for example, after a mid-campaign reassignment — that action now appears in the campaign report's Submissions tab and export, alongside batch submissions, with the certifier's name, date, and approve/deny counts. Previously these resolutions were recorded but invisible in Submissions.

  {/* PR #24651 */}

- **Bulk review actions keep your place**: Completing a bulk certify, deny, or discard in a campaign no longer sends reviewers back to the first page of the task list — they stay where they were working.

  {/* PR #23801 */}

- **Request access for an already-enrolled user without touching automation**: Access Profile administrators can now create a request for a user already enrolled in a profile without first changing the profile's membership automation setting.

  {/* PR #24037 */}
  {/* TODO: verify "Access Profile administrators" is the exact role/permission gate, not a broader admin role - see house rule on permission claims. */}

### Authentication and SSO

- **Fewer silent session reuses on sensitive SSO requests**: SSO applications can now honor OIDC requests for silent or fresh authentication, and SAML passive requests. A request that can't be completed safely — such as SAML forced authentication without verifiable upstream evidence — now returns a standards-based protocol error instead of silently reusing an existing session. An expired upstream sign-in can recover once without losing the original request context.

  {/* PR #23464 */}

### Automation and workflows

- **Push rule history opens without leaving the page**: Push rule execution history now opens in a drawer, so you can review and filter results while keeping the list of push rules visible.

  {/* PR #24553 */}

- **Breaking change: automation expressions must reference workflow context directly**: Automation expressions that reached the workflow context indirectly — rather than naming the step and field directly — no longer validate. If an automation's expressions were written this way, they'll need to be rewritten to a static path (naming the step and field explicitly) before they'll save or run.

  {/* PR #24042 */}
  {/* TODO: confirm effective/enforcement date and whether existing automations with indirect expressions are grandfathered, blocked on save, or blocked on run - source PR text doesn't specify. Also confirm whether this needs its own customer broadcast (like the OCSF change) rather than just a digest mention. */}

### Connector management

- **Add an account attribute the app hasn't discovered yet**: The Account attribute field previously only offered app-user profile keys the app had already discovered, and disabled itself entirely for apps with none. It now accepts any key typed by hand.

  {/* PR #24289 */}
  {/* TODO: confirm exact nav path and field label (source says Settings -> User data sources -> Attributes -> Add attribute) against current UI/screenshot before publishing. */}

- **"Last push" now means a push actually happened**: The "Last push" timestamp on a push-rule configuration now only populates when a push rule actually executes during a sync, so the field is trustworthy evidence the rule is working.

  {/* PR #24143 */}

- **AI-suggested CSV mappings confirm when applied**: Clicking "Apply mappings" on the AI agent's suggested column mapping used to leave the button sitting there, looking like the click had failed. It now shows a green "Mappings applied" confirmation and hides the button.

  {/* PR #24139 */}

- **Custom entitlement slugs stick on V2 connectors**: Custom entitlement slugs (e.g. on Azure DevOps) used to silently reset to the connector's default on the next sync. They no longer do, and the access-request catalog picker shows the entitlement's display name instead of a generic slug when none is set.
...
- **Connector error digests reach the right people**: The daily "Connector Errors" digest used to go to every Application Admin and System Owner. It now reaches only the actual owners of the affected connector.
...
- **MCP server URLs are locked after first sync**: An MCP server connector's server URL can no longer be changed after its first successful sync — register a new MCP server to point at a different endpoint instead.

  {/* PR #24086 */}

- **No more redundant daily attribute pushes**: Connector attribute pushes no longer re-send an update when every pushed attribute already matches the value from the last sync — previously this ran thousands of no-op pushes a day for users whose attributes hadn't changed. Pushes still fire whenever the previous value is unknown or actually differs.
...
- **See which rows a file-import sync skipped**: A file-import connector's Audit trail now records how many rows a sync skipped because their User ID was empty, with the row numbers of the first ten. Previously those rows — and any access they granted — were dropped without a trace, and the sync still reported success.
...
- **A finding when a connector's sync keeps failing**: Two consecutive failed full syncs on a connector now raise a "Connector sync failing" finding, which auto-resolves on the next successful sync. Can be turned off per tenant.

  {/* PR #22952 */}

- **Findings get an "Assignee," not an "Owner"**: The "Owner" column, filter, and detail field on findings are now labeled "Assignee" — how findings actually get assigned hasn't changed. If you consume the Finding object via API or webhook, `computed_owner`/`assigned_owner` are deprecated and no longer populated; use `assignee_identity_user_id` going forward. If you consume the audit log or feed it to a SIEM, the finding event previously logged as `OWNER_CHANGED` (with `old_owner`/`new_owner` keys) is now `ASSIGNEE_CHANGED` (with `old_assignee`/`new_assignee` keys) — only newly emitted events use the new name; historical rows are unaffected.

  {/* PR #24617 */}

### Role mining

- **We've improved the role mining experience**: Set a shared-access threshold to focus results on what your cohort actually has in common, fine-tune the entitlement list before acting on it, and optionally route automatic profile enrollments through an access request task for approval.
  ...
  {/* SOURCE: docs PR #522 (ConductorOne/docs), open as of 2026-09-03; user will merge before this release notes PR merges. Rolled up per user request from 5 detailed bullets (shared-access threshold, entitlements-tab fine-tuning, approval-routed enrollment, Done-button behavior, suggested-refinements removal) - see PR history for full detail if needed later. */}

- **Large cohorts load progressively instead of stalling**: Role mining's member list previously could block or time out entirely for large cohorts. It now loads the member list immediately, filling in per-user entitlement coverage as it becomes available.

  {/* PR #24271 */}

- **See up to 1000 shared entitlements, not 100**: Custom role-mining analysis silently capped shared entitlements at 100 per cohort. The cap is now 1000.

  {/* PR #23848 */}

### Task and notification handling

- **Stale Slack buttons fail gracefully**: Clicking Approve/Deny/Provision on an access-request message in Slack after it had gone stale used to fail the workflow outright and page oncall. It now shows the user an "expired" message instead.

  {/* PR #24145 */}

- **Slack stops retrying interactions that can't succeed**: Stale clicks, duplicate submissions, and missing-auth interactions used to be retried by Slack action handling even though they could never succeed. They now fail fast instead.

  {/* PR #24112 */}

- **Slack messages render user text cleanly**: User-submitted text in access-request Slack notifications now renders with a bold label and plain value, fixing artifacts like trailing asterisks showing up in URLs.

  {/* PR #24108 */}

- **Accept-step messages now reach the requester**: A policy accept step's configured message used to be recorded only in ticket history. It's now also sent to the requester via Slack, matching how reject steps already behave.

  {/* PR #24046 */}

- **Revoke tasks link back to the grant that created them**: Revoke tasks created when timed access expires now link back to their originating grant task under "Related Tasks."

  {/* PR #17692 */}

- **Audit views hold up on large histories**: The task audit view now shows accurate totals and page ranges even on tickets with large audit histories.

  {/* PR #24122 */}

### User and access administration

- **Deleting a user cleans up their Slack binding too**: Deleting a C1 user now also removes their Slack user binding, preventing bot failures tied to that account.

  {/* PR #24133 */}

- **See and manage the full owner list**: The "Select owners" picker now has an "All users / Owners" toggle, so admins can see and manage the complete current owner list, including suspended or otherwise filtered-out users.

  {/* PR #23832 */}

- **Owner mutations are scoped to the app they belong to**: Owner-mutation endpoints for app resources are now app-scoped, matching how the equivalent v1 endpoints already worked — only that app's owners or an Application Admin can mutate its owner list.

  {/* PR #24025 */}
  {/* TODO: verify the exact permission gate (app owner vs. Application Admin, and which one if not both) against the current permission check before publishing - see house rule on permission claims. */}

- **Locked notification preferences are now visible**: Personal notification settings now show which preferences your admin has locked, matching how locks already appear in org settings.

  {/* PR #24349 */}

- **External Slack accounts can no longer bind to a C1 user**: Slack accounts from outside your workspace — including Slack Connect members — can no longer be linked to a C1 user account by email match. Previously, an external member whose Slack email happened to match a C1 user's email would have bot interactions and notifications treated as that C1 user. Legitimate workspace members are unaffected; a C1 user whose only Slack match is external now has notifications skipped with a warning instead of silently misdelivered.

  {/* PR #24324 */}

### System log event schema updates

The system log adds four new Identity & Access event classes — Account Change, Entity Management, User Access Management, and Group Management — plus a `groups[]` field naming each actor's platform roles and a shared `correlation_uid` linking the paired events a privilege change now produces. Every event's actor now also lists the platform roles that person held, and machine identities (connectors, tunnels, functions) are reported with the standard "Service" account type instead of "Other." Admin-tier actors also log a new actor type value as of September 3, 2026, distinguishing them from non-admin users for the first time; affected customers were notified separately ahead of this change. See the [system log reference](/product/admin/system-log) for the full event catalog.

{/* SOURCE: docs PR #511 (ConductorOne/docs), merged 2026-08-31, not from the PR digest. Merged in ductone/c1 PR #23955 from the chunk-2 digest per user - same underlying change (actor platform roles, audience, paired standard-type events, Service account type); only the "Service" detail was new, added here rather than as a separate item. Breaking change (actor.user.type_id remap) already covered in a customer broadcast per user - kept brief here, general-awareness/historic-record entry only. */}

### API and developer tools

- **New devices get a working API credential automatically**: Registering a new desktop device now auto-creates a full-access API credential shown right on the success screen, instead of leaving the API credentials page empty.

  {/* PR #24096 */}

- **Mint CLI enrollment codes without interactive approval**: A new RPC lets a CLI mint a one-time, 10-minute, single-use device enrollment code without needing interactive approval.

  {/* PR #24074 */}
  {/* QUESTION: does this warrant its own API/CLI reference doc link, or a mention on an existing enrollment page? */}

- **Entitlement list endpoint gets camelCase params and fuzzy search**: The entitlement list endpoint now accepts camelCase pagination params (pageSize/pageToken) alongside the existing snake_case ones, and supports fuzzy search plus filtering by app-user ID.

  {/* PR #23826 */}
  {/* TODO: confirm endpoint is live in the generated API reference before linking. */}

- **Check a session policy's blast radius before enforcing it**: Three new read-only API endpoints let admins see which session policy applies to a given user and why (direct assignment, group membership, or tenant default), and list every user a policy applies to — direct and group-conferred, with the conferring group named. These are diagnostic reads only; there's no change to session evaluation, enforcement, or assignment behavior.

  {/* PR #24521 */}
  {/* QUESTION: worth a public API reference link once the endpoints are confirmed live? */}

- **Real descriptions for 23 API operations**: The public API reference for 23 operations — spanning hooks, LLM gateway keys and provider credentials, reports, role-mining custom analysis, organization contacts, and app entitlement routing rules — now explains what each call actually does, instead of a placeholder. Generated SDKs and the Terraform provider pick up the same wording.

  {/* PR #24311 */}

### Vaults and secrets

- **See secret activity and vault inventory in Analytics**: The Vaults tab in Analytics now shows Paper secret history, internal/external creation activity, and burned-secret totals. Tenants with Latchkey enabled also see Multipass vault inventory, reveal activity, and device readiness. Analytics failures don't block secret retrieval, and historical Multipass analytics only start accumulating after enablement — earlier periods show no data rather than a misleading zero.

  {/* PR #24696 */}

- **Headless enrollment codes actually redeem now**: Enrollment codes created through the CLI enrollment-code endpoint were bound to the wrong client type and always rejected by the CLI's device-authorization step with an "invalid, expired, or already used" error. Headless (multipass) enrollment now succeeds.

  {/* PR #24508 */}

### Usability improvements

- **See the real group count**: The C1 Groups table now shows the exact total group count instead of capping the display at "more than 25."

  {/* PR #24111 */}

- **Chart legends behave the way you'd expect**: Single-series charts no longer show a redundant legend; on multi-series charts, clicking a legend item now isolates that series instead of removing it.

  {/* PR #24110 */}

- **Accurate AI adoption numbers**: The "Access profile enrollments" chart on the AI & MCP analytics tab previously counted every access profile, overstating AI adoption. It now counts only enrollments in profiles with at least one MCP tool or toolset.
