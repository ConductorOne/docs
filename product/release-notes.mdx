...
See [Interact with C1 via Slack](/product/admin/slack-application#ask-questions-and-take-action-with-the-c1-ai-assistant) for details.

</Update>

<Update label="June 11, 2026">

### Cloud infrastructure access (early access)
...
This initial release supports Azure. Access requests, access reviews, JML automation, and provisioning all work on the hierarchical model from day one.

Cloud Infrastructure Access is an early access capability. Contact the C1 Support team if you'd like to try it out. See [Cloud Infrastructure Access](/product/admin/cloud-infrastructure-access) for details.
...
Entitlement routing rules let you automatically apply request settings — request policy, emergency grant behavior, and maximum grant duration — to entitlements across an entire app, based on conditions you define. Rules are evaluated in priority order; the first match wins. Use the Basic builder for simple field-matching or write a CEL expression for full control. For cloud infrastructure apps, conditions can match on the granted role and the scope it applies to.

See [Apply request settings automatically with entitlement routing rules](/product/admin/entitlement-routing-rules) for details.
