...
- **Reuse a policy instead of copying it**: A policy rule or baseline can now point to another policy of the same type instead of repeating its steps. Define a common flow once — like a security review or a manager-then-owner approval chain — and reuse it across every policy that needs it. Update the shared policy and every policy pointing to it picks up the change automatically, so you're not hunting down and updating copies scattered across your policy library. 
