...
Configuring which accounts from a Directory source get imported into C1 previously required a raw CEL expression — difficult to write correctly and impossible to validate before saving. The import configuration now uses the same visual condition builder available elsewhere in C1. A built-in **Data preview** shows how many accounts match and lets you spot-check individual accounts before saving. Custom CEL is still available for complex conditions.
...
### AI Governance Administrator role

Organizations using AI access management previously needed Super Administrator privileges to configure tenant defaults, register MCP servers, govern tools, manage AI clients, and use kill switches. The new **AI Governance Administrator** role lets you delegate AIAM governance to dedicated security or platform team members without granting full Super Administrator access.

Users with this role can register and manage MCP servers across all applications, review and govern tools, create and manage toolsets, edit access profiles to bind and unbind toolsets, manage AI clients, and activate all kill switches including the tenant-wide emergency switch. The role is scoped to AIAM surfaces only — AI Governance Administrators cannot enable or disable AIAM for the tenant, assign or revoke user roles, or create or delete access profiles.
