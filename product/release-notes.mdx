...
MCP servers in C1 now support HTTP Basic Authentication. Configure it in two modes depending on how credentials should be managed:

- **Admin-shared**: Enter credentials once and all users connect using the same username and password — useful when a shared service account is appropriate.

- **Per-user**: Each user supplies their own credentials, so MCP requests run under their own identity.
...
See [Add applications](/product/admin/applications) and [Manage entitlements](/product/admin/managing-entitlements) for details.
