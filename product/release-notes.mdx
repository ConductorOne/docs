...
<Update label="Digest for June 12, 2026">

## Product digest: June 12, 2026

*A quick note: we're adjusting how we organize and present product updates. Major new features will be posted here on the day they ship; this new biweekly product digest covers all the other updates from the past two weeks.*

### Trigger request actions from the AI assistant

You can now discover and trigger request actions (requestable automations) directly from the AI assistant — on the web and in Slack. Ask the assistant what actions you can request and it lists the ones available to you; pick one and the assistant renders that action's form for you to review and submit. Submission is always a deliberate human click — the assistant never submits on your behalf — and every request runs through the same visibility, form-validation, and approval-policy checks as the web UI.

### AIAM improvements

- **Simplified MCP server authentication**: When you add an MCP server from the catalog, the authentication step now opens pre-filled with the server's recommended configuration — endpoints, scopes, grant type, and other provider-specific settings already populated, so in most cases you only need to enter your credentials. Servers that support multiple authentication options present each as its own selectable recommended choice. Token sharing now defaults to per-user.

- **Connection verification**: When connecting an external MCP server, C1 now verifies the server URL and credentials before saving. A **Test auth config** button lets you check the connection at any time, with a clear error message (including the HTTP status) if it fails.

- **MCP toolset management API**: The C1 public API now supports managing MCP toolsets — create, list, update, and delete toolsets; add, remove, and list tool membership; and view toolset and binding change history per app and connector, with pagination. These operations are also available to AI agents as governed MCP tools.

### Assign a group (or any entitlement) as the owner of a resource

You can now use any entitlement to define resource ownership. When you add an entitlement (such as a group or a role) as an owner, everyone currently assigned that entitlement automatically becomes an owner, and ownership updates dynamically as users have that entitlement granted to or removed from their account.

See [Change resource owners](/product/admin/managing-resources#change-resource-owners) for details.

### Policy and automation expressions can read object annotations

CEL expressions in policies, approver rules, and entitlement routing rules can now read object annotations. Use this to fork logic on customer tags (`entitlement.annotations["team"]`), IaC-management markers (`entitlement.annotations["managed_by"] == "terraform"`), or any other annotation your team uses. The same field is available as `role.annotations` and `scope.annotations` for sparse entitlement routing. Use the `in` operator (`"team" in entitlement.annotations`) to guard against missing keys.

See [Object annotations](/product/admin/object-annotations) for details.

### Customizable columns on Campaigns screens

You can now customize which columns appear on the **Campaigns** screens. On the **Running**, **Drafts**, and **Completed** tabs, use the column picker to show, hide, and reorder columns; your layout is saved per tab. New optional columns include **Created by**, **Owners**, **Policy**, **Type**, **Time remaining**, **Due date**, **Duration**, and **On-time status**.

### Store credentials to multiple recipients

The **Store credential** automation step now supports sharing a credential with multiple recipients when using Paper Vault — specify multiple SSO users, or multiple email addresses for email verification. App Vault continues to target a single user.

### Usability improvements

- **Multi-step approval notifications**: Multi-step access request approvals now notify each step's approvers by email, Slack, or Teams when the request advances to their step.

- **Filter accounts by app**: On **My Access** > **Accounts**, a new **App** filter lets you narrow accounts to one or more apps. Exports respect the active filter.

- **Connector "Updated on" column**: The **Connectors** table now has an **Updated on** column showing when each connector was last modified.

- **Governance pages**: The Governance pages — **Campaigns**, **Findings**, **Access profiles**, **Role mining**, **Policies**, and **Passwords** — now use the updated top-bar layout that matches the rest of the app.

### Connector updates

Learn about the latest connector updates and new connectors in the [connector release notes](/baton/_release-notes).
