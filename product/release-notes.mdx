...
- **Request forms now show up in Slack, not just the web.** Requesting or extending access through the C1 app for Slack now renders the entitlement's configured request form right in the modal, and submits your answers with the ticket — no more finishing the form on the web after starting in Slack. See [Collect additional information from requestors using request forms](/product/admin/customize-requests#collect-additional-information-from-requestors-using-request-forms) for details. {/* PR #24671 */}
{/* QUESTION: re-added 2026-09-21 - FEATURE_FLAG_ID_SLACK_REQUEST_FORMS is still SKU_MANUAL in ductone/c1 as checked this same day (its CUSTOM_DURATION_SELECT dependency is SKU_ALL, not a blocker). Re-added anyway on the developer's word (via Melinda) that it's actually rolled out - per the skill this kind of verbal override has been wrong before, so if anything looks off post-publish (support tickets, a customer saying they don't see it), check the flag again rather than assuming the verbal confirmation still holds. */}
