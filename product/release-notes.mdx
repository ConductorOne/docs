...
- The **Unstructured** access review view now supports optional columns for app user email, username, and profile attributes. Enable them from the column customization menu.
*/}
...
- Three new automation steps — **Generate password**, **Set credential**, and **Store credential** — can be chained to automate onboarding flows that provision access and set initial credentials in a single automation.
...
Cone, the ConductorOne CLI, now supports AWS IAM Identity Center (SSO). Run `cone aws setup` once to generate AWS CLI profiles for every permission set available to you in ConductorOne, then use your normal AWS CLI workflow — Cone handles access requests and credential fetching in the background. See [Use Cone with AWS SSO](/product/how-to/cone-aws-sso-integration) for setup instructions.
...
- The **Submit reviews** button in the end user reviews UI has moved from the top to the bottom of the screen, putting it closer to the review tasks you're working through.

- Confirmation messages and alert notifications now appear at the top center of the screen instead of the top right.
