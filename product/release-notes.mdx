...
[PLACEHOLDER: confirm whether this is early access or GA, and adjust the opening line accordingly.]

You can now connect an MCP server running in your private network — a laptop, a VM, or a cluster — to C1 without opening any inbound firewall rules. Run the `bridge-client` agent next to your server; it dials out to C1 and holds an outbound tunnel open. C1 routes incoming tool calls back through that tunnel, so your server never needs to accept inbound connections. Once connected, the server's tools are governed through the same access request, approval, and review workflow as any other tool in C1.

See [Connect a private MCP server through a bridge](/product/admin/mcp-bridge) for setup instructions.
