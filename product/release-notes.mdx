...
{/* DRAFT STATUS: Session 1 complete, plus a consolidation/tightening pass on 2026-09-03 (cut low-interest items, merged related bullets, promoted Role mining, merged the two MCP sections). Built from both PR digest chunks (Aug 21-28 and Aug 28-Sept 3), colleague-provided copy for two docs-update projects (Agent classifiers, Govern MCP resources), a docs update for the guided MCP connect flow (ductone/c1#24062/#24638), and a system-log schema note (ConductorOne/docs#511). Remaining before publish: the Session 2 verify/polish pass resolving every inline TODO/QUESTION comment. */}
...
- **We've improved the role mining experience**: Set a shared-access threshold to focus results on what your cohort actually has in common, fine-tune the entitlement list before acting on it, and optionally route automatic profile enrollments through an access request task for approval. Large cohorts now also load progressively instead of stalling, and shared-entitlement analysis scales up to 1000 entitlements per cohort instead of 100.
...
- **"Certify All" only counts what's actually certified**: The count next to a campaign's "Certify All" action now reflects only tasks that will actually be certified, not ones still pending submission.
...
- **Campaign bulk actions are more reliable and accurate**: Bulk certify, deny, or discard actions now keep reviewers on the page they were working on instead of resetting to the first page. "Select all in filter" no longer offers a bulk Cancel or Skip when the filtered set includes closed tasks, and a bulk action's result banner now reports "N succeeded, M failed" instead of a generic "completed" message when something fails. Task resolutions made directly from a campaign's task execution plan — for example after a mid-campaign reassignment — now also show up in the Submissions tab and export, alongside batch submissions.

  {/* PR #24707, #24651, #23801 */}
  {/* Consolidated per the 2026-09-03 tightening pass from three separate bullets (select-all-in-filter fix, submissions visibility, page retention). */}
...
- **Push rules are more trustworthy**: "Last push" only updates when a push rule actually executes during a sync, and connector attribute pushes no longer re-send no-op updates when every attribute already matches — previously this ran thousands of redundant pushes a day for users whose attributes hadn't changed.
  ...
  {/* PR #24143, #24481 */}
  {/* Consolidated per the 2026-09-03 tightening pass. */}

- **Custom entitlement slugs stick on V2 connectors**: Custom entitlement slugs (e.g. on Azure DevOps) no longer silently reset to the connector's default on the next sync.
...
- **Connector error digests reach the right people**: The daily "Connector Errors" digest now reaches only the actual owners of the affected connector, instead of every Application Admin and System Owner.
...
- **Slack notifications are more reliable**: A stale Approve/Deny/Provision click now shows an "expired" message instead of failing the workflow and paging oncall, stale or unresolvable interactions fail fast instead of retrying forever, and user-submitted text renders cleanly instead of showing artifacts like trailing asterisks.
  ...
  {/* PR #24145, #24112, #24108 */}
  {/* Consolidated per the 2026-09-03 tightening pass. */}

- **Accept-step messages now reach the requester**: A policy accept step's configured message is now also sent to the requester via Slack, matching how reject steps already behave.
...
- **Owner mutations are scoped to the app they belong to**: Owner-mutation endpoints for app resources are now app-scoped — only that app's owners or an Application Admin can mutate its owner list.
...
- **Headless enrollment codes actually redeem now**: Headless (multipass) enrollment codes were always rejected by the CLI's device-authorization step; they now succeed.
