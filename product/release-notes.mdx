...
C1 has a fresh look. You'll see an updated logo, new colors, and refreshed visuals across the product — but nothing about how C1 works has changed.

Your tenant URL (`tenant.conductor.one`) isn't changing, and there's nothing you need to do. Read [our announcement](https://www.c1.ai/blog/wearec1) to learn more.

### Access review improvements

Several labels and interactions in the campaign setup flow and reviewer task list have been updated.

**Campaign setup**

- The button to begin staging a campaign is now labeled **Stage campaign**. The staging panel now includes a brief description explaining that staging locks in campaign data and prepares the campaign to run.

- The campaign due date field is now labeled **Due date** in the staging panel and campaign list. (Note for review: the campaign detail status bar still shows "Target completion" — confirm whether this is intentional before publishing.)

- The scope summary for entitlement-scoped campaigns now shows **Apps and entitlements**.

- The **Validate scope** button has moved from its own banner on the **Scope** tab into the **Stage campaign** panel, keeping scope validation and staging in one place.

**Reviewer task list**

- The view selector now shows the active view — for example, **View: App** or **View: User** — so it's always clear which view you're in.

- Bulk certify and deny actions are now icon-only toggle buttons — a checkmark for certify, an X for deny. Hover over either button to see its label.

- The discard button tooltip now shows how many pending reviews will be affected before you confirm.

- The task list now defaults to **Show: Incomplete**, hiding completed reviews automatically.

- The **Account owner** column is no longer shown by default. You can add it from the column customization menu.

- The AI assistant button is now labeled **AI review assistant**.

- **View submissions** is available from the overflow menu (the **⋯** button) in the campaign toolbar. (Note for review: the PR described this as moving next to the progress bar — code shows it's in the overflow menu. Please verify against the UI before publishing.)

### Automation improvements

- The automation execution log page now includes an app filter, making it easier to find execution history for automations tied to a specific app.
...
- File downloads through the public API now return properly normalized filenames in the HTTP response headers, ensuring compatibility across clients and browsers.
...
The [C1 integration for Microsoft Teams](https://marketplace.microsoft.com/en-us/product/office/WA200009797) is now live on Microsoft Marketplace, making it easier to discover, install, and deploy C1 directly within your Microsoft ecosystem. Ready to bring C1 into your Teams workspace? Check out our docs on the [MS Teams integration](/product/admin/ms-teams-public-public) to get started. 
