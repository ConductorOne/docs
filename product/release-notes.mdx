...
A new **Forms** page under Governance lists every entitlement request form in your tenant in one place, so admins can browse and edit forms without opening each entitlement individually. See [View and manage all forms](/product/admin/customize-requests#view-and-manage-all-forms).
...
* HTTP Basic Auth for MCP servers can now be configured in two credential modes: 
