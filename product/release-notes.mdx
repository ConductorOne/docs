...
### Save and revisit reports in C1

Ask a reporting question in a conversation with C1AI, then select **Save report** to give the result a name and description you can return to later.
...
Find every report you've saved on the new **Platform > Reporting** page — refresh it, edit it with the **Report assistant**, or review the sources behind its results. See [Save and revisit reports](/product/admin/ai-assistant#save-and-revisit-reports) for details.
...
Tracking down who changed a connector, a policy, or another configuration object used to mean asking around or digging through the system log by hand.

Many configuration objects across C1 — connectors, policies, entitlement routing rules, SSO and branding settings, AI Gateway spend controls, and more — now show a **Change history** icon on their detail page. Click it to see a timeline of every change, with a clear before-and-after for each field. See [View change history for an object](/product/admin/change-history) for details.

{/* PR #25808 (small wiring change only, no release-notes text of its own). Wording and object list sourced from ductone/c1 frontend (ObjectHistoryIconButton usage sites, ObjectHistoryDrawerContent) and Michael Burton's #product-releases heads-up on 2026-09-21 that OBJECT_HISTORY and CONNECTOR_CONFIGURATION_CHANGE_CONTROL (both SKU_MANUAL as checked that same day) are rolling out to all tenants that day. He owns CONNECTOR_CONFIGURATION_CHANGE_CONTROL in the flag registry, matching the announcement. Added on that word per Melinda, ahead of confirming the rollout actually completed. */}

### Assign a finding to a specific person

Findings can now be assigned to a specific person. Assign one manually, or automatically with a new Set assignee step in Transformation rules — a manual assignment always wins. See [Transformation rules](/product/admin/findings#transformation-rules) for details. {/* PR #25487, #23470 */}
...
- **Require a reason for connector configuration changes.** Admins can now require a justification whenever a connector's configuration changes, audited alongside a secret-safe [change history](/product/admin/change-history). {/* PR #25438 */}
...
- **Request forms now show up in Slack.** Requesting or extending access through the C1 app for Slack now renders the entitlement's configured request form right in the modal, and submits your answers with the ticket — no more finishing the form on the web after starting in Slack. See [Collect additional information from requestors using request forms](/product/admin/customize-requests#collect-additional-information-from-requestors-using-request-forms) for details. {/* PR #24671 */}
...
- **Campaigns can now include users with pending account invitations.** Accounts with an outstanding invitation the principal hasn't accepted yet can now be included in a campaign's scope, instead of being silently dropped. {/* PR #25014 */}
...
- **See a group's size at a glance.** Groups now show their member count at the top of the group detail page. {/* PR #25412 */}
...
- **Policy comments now show you what they'll actually post.** Wait-step and SLA-violation comment fields now hint that markdown works and preview the result as you type, and the policy summary renders the comment instead of showing raw markdown source. {/* PR #23155 */}
...
- **Terraform support for MCP sources.** Manage hosted and external MCP sources — details, authentication, and tool enablement policy — as code. See [C1's Terraform provider](/developer/terraform#getting-started-with-c1s-terraform-provider) for the new resources. {/* PR #25273 */}
...
- **Breaking change: deleting a shared secret no longer also revokes its credential.** `DELETE /api/v1/secrets/{vault_id}` now deletes only the secret; use the new `POST /api/v1/app-secrets-admin/revoke` to queue a credential revoke separately. Service SDKs rename Revoke to Delete to match. {/* PR #25587 */}
