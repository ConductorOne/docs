...
The campaign setup flow and reviewer task list have been refreshed with clearer labels and more compact controls. Staging a campaign is now a single **Stage campaign** step that includes scope validation, and reviewer actions in the task list use icon buttons with tooltips to keep more of each review row visible. To view e-signature submissions, use the **⋯** (more actions) menu at the top of the reviews page. See [Create a campaign](/product/admin/campaigns) and [Complete access review tasks](/product/how-to/review-tasks) for updated documentation.
