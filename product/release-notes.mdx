...
--- 
{/* Editor Refresh: 2026-01-16 */}

<Update label="January 23, 2026">

### Batch submit reviews

We've told customers that this will be turned on for everyone on the 20th. Reviewers can now submit multiple review decisions at once, streamlining the access review process and saving time when completing large campaigns.

### Requestable automation actions

Users can now request to run specific automations directly from the request catalog. This feature allows you to make certain automation workflows available for end users to trigger on-demand, streamlining processes like account provisioning, access modifications, or other custom workflows.

### Access operations dashboard enhancements

The access operations dashboard now includes several new capabilities:

- **Download dashboard data** - You can now see all the top values (apps with most new requests, revocations, user accounts) and download them as CSV.
- **Export open extensions and expiring grants** - You can export CSV for open extension requests for expiring grants and grants expiring in X days.
- **User response time table** - New user response time table in the access operations dashboard. For each user see p50, p90, p99 response time. Columns are sortable too.

### Copy connector ID

Users can now easily find and copy the Connector ID directly from the Connector details page. Hovering over the connector's name in the page header will reveal a tooltip containing the Connector ID and Catalog ID (if applicable), along with a copy-to-clipboard option.

### Export reviewer decision table

This is part of the pending actions/batch submission release, which is scheduled for Jan 20. Reviewers can now export the decision table showing all review actions taken during a campaign.

### Grant feed enhancements

The exported grant feed report now includes two additional columns: `AppName` and `EntitlementName`. These new fields provide clearer context for each grant and revocation event by displaying the name of the associated application and entitlement.

### Usability improvements

- Extension tasks now have a dedicated icon to make them easier to identify in task lists.
- The 'Revoke' button is now hidden for grants that are inherited through group memberships. This prevents users from attempting to directly revoke access that must be managed at the group level, improving the accuracy and usability of grant management.
