...
### Automation actions (privileged actions)

You can now safely expose powerful automations to business users without giving them standing admin access. Automation actions are requestable automations that run on demand, under policy control, with full auditability.

Use automation actions for operations that go beyond normal access requests, such as:
...
Each automation action can collect structured inputs via a custom form, is governed by an approval policy, and runs using pre-defined steps with full logging. Actions are visible only to the audience you define, reducing blast radius and enforcing least privilege at the workflow level.

Check out [Configure automation actions](/product/admin/automation-actions) to learn more and get started.
...
When an extension grant task is canceled because the associated app user was deleted (this often occurs because the access times out and the account is automatically deprovisioned), a new extension grant task will now be automatically created. The new task includes a comment linking back to the original canceled request, and the audit log shows the origin of the task and includes a link to the original.
