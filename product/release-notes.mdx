...
An MCP client shouldn't need a separate connection for every tool source, and a governed one shouldn't get more than it's entitled to just because it can reach a server. Admins can now create a **Virtual MCP server** that curates an approved set of tools and toolsets pulled from multiple Hosted or External MCP sources into a single endpoint.

A user needs both access to the virtual server itself and an existing or requestable entitlement for each underlying tool or toolset it exposes — removed assignments and revoked leaf access are rechecked before a tool actually runs, not just at connection time.

{/* TODO: docs link - no dedicated page found for Virtual MCP servers; check with eng/PM for the right product/admin page or whether one needs to be written */}
{/* PR #25355 */}

### Shadow apps get a full workflow, not just a finding

Discovering an unmanaged app used to be the end of the story — the finding sat there until the next detection cycle caught up, however you resolved it. Shadow apps now carry through from detection to resolution:

- **Discovered usage resources become findings automatically**, and each finding links straight to the usage-resource identity that surfaced it — no separate hunt through Shadow apps required. {/* PR #25003 */}
- **Authorize or ignore a shadow app right from the finding details drawer** — no need to leave Findings to act. Admins without remediation permission see guidance to ask one who has it instead of a dead end. {/* PR #25631 */}
- **The finding resolves immediately**, instead of waiting for the next detection cycle — if the resolve step itself fails after a successful authorize/ignore, a warning explains the finding will still clear automatically on the next run. {/* PR #25728 */}
- **Shadow app detection now defaults on when the Shadow Apps feature is enabled**, and its settings only appear when the feature is actually on, so there's nothing to configure to start seeing findings. {/* PR #25786, #25509 */}
- **Row actions and evidence views are cleaned up**: Authorize/Ignore buttons use consistent primary/outlined styling, and the deactivated-owner finding's evidence now renders as a read-only Account/Account status table. {/* PR #25214, #25285 */}
{/* QUESTION: PR #25190 says "Shadow app findings are no longer available in findings settings or finding workflows" - this reads like it could contradict the rest of this arc (which is about deeper findings integration). Need to check with eng what this actually removed before this section can be confirmed accurate. */}
{/* TODO: docs link - product/admin/shadow-apps.mdx already exists, confirm it covers this workflow and link to the relevant section */}
...
- **AI agents can now author their own connectors, end to end.** An in-app authoring agent can read the authoring contract, source the OpenAPI spec, design the access model, write and build connector source, test-sync it, and deploy — running the whole funnel without stopping after the first step. Activation stays a human decision. {/* PR #24771, #24877, #24925 */}
...
- **Employees can see their own AI budget.** When self-service is enabled, employees can view their current AI spend allowance and recorded calls and manage their own per-app restrictions (including temporary replacements) directly — server-side limits and admin funding caps still apply underneath. {/* PR #25258 */}
...
{/* TODO: could open with a bolded lead bullet similar to Aug 21/Sep 4 style once trim pass decides how much weight this section gets relative to Virtual MCP servers / Shadow apps */}
...
- Jump straight to nested pages from command search (Cmd+K) - Shadow MCPs, Virtual MCP servers, password policies, and unmanaged/shadow apps are all now reachable this way, following the same role and feature-flag rules as the sidebar. {/* PR #25695 */}
