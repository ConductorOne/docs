...
### View and manage all your forms

The new **Forms** page lists every entitlement request form in your tenant in one place, so admins can browse and edit forms without opening each entitlement individually. See [View and manage all forms](/product/admin/customize-requests#view-and-manage-all-forms).

### Assign app and entitlement ownership to the holders of selected entitlement grants

You can now assign owners for apps and entitlements by entitlement as well as by user. When you add an entitlement as an owner, everyone currently assigned that entitlement becomes an owner automatically — and ownership updates dynamically as users are granted or removed from that entitlement. Each app or entitlement supports up to 32 direct user owners and up to 32 entitlement owners.
...
The **Analytics** page now surfaces significantly more data about your environment, organized across six categories: **Access**, **Reviews**, **Identity**, **Security**, **Operations**, and **AI & MCP**. Navigate to **Analytics** to explore.
...
### Update to access profiles

Entitlements on an access profile's **Apps** tab are now grouped by app, with a count of included entitlements shown next to each app name. Click the pencil icon next to an app to adjust which of its entitlements are in the profile, or click **Manage** to add or remove apps from the profile entirely. See [Manage entitlements from the access profile's Apps tab](/product/admin/profiles#manage-entitlements-from-the-access-profiles-apps-tab).
...
### Access review: reviewer attribute visibility

Admins can now control which app user profile attributes reviewers see during an access review. Configure specific attribute keys per app in the campaign or template — for example, surface department and manager while keeping personal email hidden. Reviewers see no profile attributes by default; attributes are only shown when an admin has explicitly enabled them for that app. See [Map user attributes](/product/admin/attributes) for attribute setup.
...
* **Shared or per-user credentials for HTTP basic auth**: HTTP basic auth for MCP servers can now be configured in two credential modes: 

    - **Shared (admin authorizes)**: Enter credentials once and all users connect using the same username and password — useful when a shared service account is appropriate.

    - **Per-user (each user submits their own)**: Each user provides their own credentials when they connect, so MCP requests run under their individual identity.

- **Featured and popular MCP tools**: The tools list on an app's MCP server now opens to a curated view of Featured and Popular tools, organized by classification. A **Visibility** column also appears in every MCP tool table alongside **Classification**. This makes it easier for users to find the tools they need and understand which tools are approved for use.

### Usability improvements

- **Access-only option in revocation automation steps**: Revoke steps in automations now include an **Access only** option, which targets just the access entitlement on an app. This is most useful in offboarding automations: it removes the user's app account without generating downstream revoke tasks and notifications for entitlements that are dropped automatically when access to the account is removed.

- **External ticket column in campaign reports**: Campaign reports now include an optional **External Ticket** column for tenants with external ticketing enabled. For each task linked to an external ticket — such as a Jira or ServiceNow ticket — the column shows the ticket ID as a link that opens it directly. Enable the column from the custom column picker when generating a report.

- **Recently visited**: Several navigation areas now surface recently visited items. The Governance left nav shows a **Recent** list of campaigns, policies, and access profiles — pin any item to keep it at the top across sessions. The Directory and Workflows nav flyouts show recently visited users, groups, automations, and functions. Spotlight (Ctrl+K / Cmd+K) now searches access profiles and recently visited automations and functions directly.
