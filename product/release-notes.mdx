...
We've rolled out a refreshed Role Mining experience that makes it faster to explore access patterns and turn them into access profiles — all from a single, streamlined page.

- **Unified single-page layout** — Explore cohorts, review results, and take action without jumping between separate screens.
- **Token-based filter bar** — Build precise cohorts in a few clicks by adding filters for the attributes that matter to your analysis.
- **Inline recommendations** — Suggested cohort refinements appear alongside your results, so you can fine-tune as you work.
- **Floating action bar** — Common actions, such as creating a new access profile or adding entitlements to an existing one, stay within reach as you review and select.

This experience is now enabled automatically for all customers with Role Mining — no setup or configuration required.

### AI Governance Administrator role

When the AI Governance Administrator role feature flag is enabled, the role appears in role assignment. Users holding it can access AI governance settings, MCP clients, AI connections, and Show Terraform. The option to create a new managed app when adding an MCP server is now limited to Super Admins and AI Governance Administrators.

### Usability improvements

- **"Access was inappropriate" reason chip**: A new built-in reason chip is now available in the access removal, revocation, and certification comment dialogs.
