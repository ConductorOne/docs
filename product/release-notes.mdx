...
## Product digest: September 8, 2026

### Reorganized admin navigation

The left sidebar is reorganized around six product areas — **AI**, **Governance**, **Identities** (formerly Directory), **Security** (formerly Identity security), **Vaults**, and **Platform** (formerly Workflows) — each with its own **Overview** dashboard. These dashboards replace the single **Analytics** page, which previously bundled access, reviews, identity, security, and AI & MCP data into one set of tabs.

- **Platform** now holds Search, Reporting, and Task log (previously under Explore), along with Policies, Forms, Functions, and the non-connector integrations: Webhooks, Service desk, External ticketing, External data sources, and Bridges.
- **Connectors** moved from Integrations to **Apps**.
- **Automations** moved into **Governance**, alongside Campaigns, Access profiles, and Passwords.
- **Decoys** and the **Graph** explorer moved into **Security**.
- Shared secrets and the groups you belong to are surfaced together in **My resources**; bookmarks to the previous standalone Secrets page continue to work.
