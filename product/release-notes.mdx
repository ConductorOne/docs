...
{/* DRAFT STATUS: Session 1 draft, consolidation/tightening pass, and Session 2 verify/polish all complete as of 2026-09-03 - every TODO/QUESTION comment resolved (fixed, cut, or user-overridden with rationale recorded inline). Outstanding: the MCP resources and MS Teams items are kept despite confirmed SKU_MANUAL status per explicit user override - re-verify their rollout status isn't stale before actually publishing if time passes before this merges. PR #24638 (MCP guided-connect GA) should also be re-checked if it's been a while since 2026-09-03. */}
...
- **Campaign notifications can go to Microsoft Teams**: Campaign notification templates now support Microsoft Teams as a notification channel.

  {/* PR #24144 */}
  {/* RESOLVED 2026-09-03: confirmed gated behind two SKU_MANUAL flags (FEATURE_FLAG_ID_MS_TEAMS_CAMPAIGN_MESSAGES, depends on FEATURE_FLAG_ID_MS_TEAMS, both SKU_MANUAL). User made the call to keep this item despite the flag status - not a verification gap, a deliberate override. */}
  {/* Docs updated 2026-09-03: minimal wording tweak per user's scope call - added a one-line Teams mention alongside the existing Slack callout in campaigns.mdx (both occurrences: new-campaign flow and template flow). No dedicated Teams integration/setup page yet - deferred given SKU_MANUAL status; revisit once closer to GA. */}
