...
C1 now governs AI tool access through the same identity platform you already use for application access. When AI access management (AIAM) is enabled, every tool call from an MCP-compatible AI client routes through C1's identity-aware proxy. C1 authenticates the caller, checks the call against the user's approved access profile, enforces your configured policies, and writes a full audit log entry.

Admins register MCP servers from a catalog of 3,000+ integrations, review and approve individual tools, bundle approved tools into toolsets, and bind toolsets to access profiles using the standard C1 request and approval workflow. Downstream credentials are vaulted in C1 and never stored on end-user devices.
...
The workflow automation circuit breaker is now generally available. When an automation's execution rate exceeds a configured threshold within a set period (hourly, daily, weekly, or monthly), C1 automatically pauses new executions rather than running them. Paused executions remain in the queue so you can decide whether to run or cancel them before resuming.

See [Automation circuit breaker](/product/admin/automation-circuit-breaker) for configuration details.

### Activity metrics

Activity metrics are now on by default across C1. List pages for applications, connectors, access review campaigns, automations, and more include counts and 30-day trend cards so you can assess the state of a surface without opening individual records. The webhooks and Shared Signals receiver tables also show a per-row sparkline with recent activity volume.

### Object annotations

You can now attach custom key/value annotations to policies, apps, app entitlements, app resources, access profiles, automations, and access review templates. Annotations are available through the C1 API, OpenAPI spec, and C1 Terraform provider.

Objects managed by Terraform, OpenTofu, or Pulumi display a **Managed by** chip on their detail page. Editing an IaC-managed object in the C1 UI transfers ownership to C1.
...
- Request catalog role chips now appear correctly when the selected role comes from a later page of the role picker.

- Access profiles created from role mining now appear in the access profile list immediately after creation.

- The **Learn more about the C1 Slack app** link in the Slack notifications install dialog is now fixed.
