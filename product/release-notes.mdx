...
- **Set bulk request limits**: Admins can now set their own max number of entitlements a user can request at once in bulk (max 100, default 10) in the **Forms** settings drawer.
...
- **Connector sync failure finding type**: Two consecutive failed full syncs on a connector now raise a "Connector sync failing" finding, which auto-resolves on the next successful sync. Can be turned off per tenant.
...
- **See and manage the full owner list**: The "Select owners" picker now has an "All users / Owners" toggle, so admins can see and manage the complete current owner list, including suspended or otherwise filtered-out users.
  ...
  {/* PR #23832 */}
    {/* TODO: clarify where this is in the product */}
...
- **Breaking change: Findings get an "Assignee," not an "Owner"**: The "Owner" column, filter, and detail field on findings are now labeled "Assignee" — how findings actually get assigned hasn't changed. If you consume the Finding object via API or webhook see the [docs] for info on this breaking change.

  {/* PR #24617 */}
  {/* put this in a note in the docs and link to that from the release note: "If you consume the Finding object via API or webhook, `computed_owner`/`assigned_owner` are deprecated and no longer populated; use `assignee_identity_user_id` going forward. If you consume the audit log or feed it to a SIEM, the finding event previously logged as `OWNER_CHANGED` (with `old_owner`/`new_owner` keys) is now `ASSIGNEE_CHANGED` (with `old_assignee`/`new_assignee` keys) — only newly emitted events use the new name; historical rows are unaffected.   "*/}
