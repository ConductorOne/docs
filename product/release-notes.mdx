...
- **Make sure revoked access actually goes away**: Revoking an inherited entitlement — one a user has because of a group or role, not because it was granted to them directly — used to only remove the entitlement itself. The access that produced it stayed in place, so the next sync brought it right back, and nobody looking at the review could tell the revoke hadn't actually stuck. Turn on **Revoke inherited access** (under **Entitlement management** on an app's details page) and a revoke now removes the upstream grants too — off by default, set per app, with a notice to approvers listing what else it'll affect.

  {/* PR #20849 */}
  {/* TODO: screenshot of the Entitlement management toggle and the approver notice. QUESTION: confirm exact location/label of the setting on the app details page, and whether this needs a new subsection in managing-entitlements.mdx or a standalone page. */}
