...
Super Admins can now ask C1AI to diagnose a specific task. The assistant traces its full history — approvals, provisioning, access-profile origin, and the connector handoff — and recommends the safest next step, without making any changes.

See [Diagnose a stuck task](/product/admin/ai-assistant#diagnose-a-stuck-task) for details.

### App and connector management improvements

**A single guided flow for creating apps and adding connectors.** Instead of a multi-page form, C1 now asks only the questions relevant to your choices — pick a source, then set a name, description, and owners through the same picker for both apps and connectors, before configuring credentials. See [Create a new application](/product/admin/applications#create-a-new-application) for details. {/* PR #25052 */}
...
- **Shorter, more focused AI responses.** C1AI now gives brief up-front plans for multi-step tasks and skips routine narration while it works. {/* PR #24941 */}

### Access request and review improvements

- **Find your open requests faster.** The "already has a pending task" banner now links each listed item straight to its open task. {/* PR #24824 */}
...
- **Breaking change: deleting a vault no longer also revokes the credential.** `DELETE /api/v1/secrets/{vault_id}` now deletes only the vault; use the new `POST /api/v1/app-secrets-admin/revoke` to queue a credential revoke separately. Service SDKs rename Revoke to Delete to match. {/* PR #25587 */}
