...
---
{/* Editor Refresh: 2026-05-01 */}

<Update label="May 1, 2026">

### Early access: AI connections

You can now connect AI assistants — including Claude Desktop, Codex, Cursor, and VS Code — to your C1 data using the Model Context Protocol (MCP). Once enabled, users connect from their profile under **AI & API** > **AI connections** and paste the MCP server URL into their AI tool. Connected assistants can query users, resources, entitlements, access reviews, and more. All queries are read-only and logged in the system log.

A Super Admin must enable AI connections at **Settings** > **AI connections** before users can connect. Optional IP restrictions let you limit which networks can use the feature.

See [C1 MCP](/product/admin/c1-mcp) for setup instructions.

### Access review navigation by resource

Reviewers can now switch to a **By resource** view when completing access review tasks. The resource view groups your assigned tasks by resource, so you can see all access across a resource tree in one place rather than working through tasks individually. Use it when you want to make consistent decisions about who should have access to a specific application or resource.

### Self-service access profile revocation
...
### Employee ID in campaign reports

Campaign reports now support custom column selection. When generating a report, choose between the default column set or a custom set that includes **Employee ID** alongside other fields.

### Onboarding sync status

During onboarding, the C1 chat now shows a live status card for each connector you've configured. The card displays real-time counts of users, resources, entitlements, and grants as they sync, a progress indicator for each sync step, a rolling connector log, and the uplift queue status. The card is pinned in the onboarding sidebar while any connector is actively syncing.

### Usability improvements

- You can now sort the **Automations** table by clicking any column header — **Name**, **Triggers**, **Status**, **Last run**, or **Created at** — in ascending or descending order. **Triggers** and **Status** are now separate columns, and **Created at** is included by default. Sort state is saved in the URL, so you can bookmark or share a sorted view.
...
- The connector sync card in the onboarding flow no longer shows **Connected** before sync is complete. The card stays at **Syncing** through the initial sync and subsequent data processing, and only updates to **Connected** when both are finished.

- Opening an application's detail page now shows a loading indicator during the transition, preventing a brief flash of the Applications list.

### Fixes

- External links to specific pages — such as a notification linking to a ticket — are now preserved through the sign-in flow. Previously, signing in from an external link would redirect you to the homepage instead of your destination.

- Ticket searches filtered by **Finding** now return results. Previously, filtering by the Finding ticket type returned no results.

- Requests that return a missing app icon or similar asset now correctly return HTTP 404 instead of 504.
