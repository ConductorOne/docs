...
- **Date and person fields on request forms**: Request and automation forms could previously only capture free text or dropdown choices, so questions like "what date do you need this by" or "who is this for" had no native answer. Forms now support date fields (with optional earliest/latest limits, fixed or relative to today) and C1-user fields (single- or multi-select, optionally narrowed to a specific set of people).
  ...
  {/* MM: add docs PR #22390 */}

- **Filter devices by owner and type**: Finding a specific device in the devices list previously meant scanning the whole list by eye. You can now filter by owner and device type, and the status filter covers the full lifecycle (Pending, Active, Revoked, Disabled).

  {/* PR #22716 */}

### Notification settings improvements

- **Clearer notification settings**: Notification settings previously mixed together things you can control and things you can't, making it hard to tell what a toggle actually did. The "your access request was created" confirmation is now its own setting you can turn on or off per channel (email, Slack, and Teams) — on by default, so nothing changes unless you opt out. Provisioning notifications have moved under **Requests**, and always-on system notifications are no longer shown as a toggle since there was never anything to switch off.
