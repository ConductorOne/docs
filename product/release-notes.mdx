...
The reviewer experience has been redesigned for clarity and speed. Recommendations now appear as text labels — **Approve**, **Deny**, or **Look closer** — so you can scan your task list and make decisions without hovering over icons. User details open in a drawer rather than expanding inline, keeping your working view clean as you move through reviews. The AI review assistant is now a persistent button at the bottom-right of the page, always accessible without leaving your queue.

For campaign administrators, the campaign setup flow has been streamlined with clearer step labels and a more focused path through the essential configuration — so you can scope and launch a campaign more quickly.

See [Create a campaign](/product/admin/campaigns) and [Complete access review tasks](/product/how-to/review-tasks) for updated documentation.
...
See [Tenant notification settings](/product/admin/notifications-tenant-settings) and [Your notification settings](/product/admin/notifications-user-settings).

### Automation improvements

- Three new automation steps — **Generate password**, **Set credential**, and **Store credential** — let you generate a password, apply it to an account in a self-hosted application, and securely deliver it to the account holder's manager via a one-time secret link. See the [automation steps reference](/product/admin/automations-steps-reference) and the [new account provisioning example](/product/admin/automation-examples#new-account-provisioning-with-initial-password).

- The **Automations** page's **Executions** tab now includes an app filter, so you can quickly find the execution history for automations tied to a specific app.
