...
- The button to begin staging a campaign is now labeled **Stage campaign**. The scope summary for entitlement-scoped campaigns now shows **Apps and entitlements**.

- Bulk certify and deny actions in the task list are now icon-only toggle buttons — a checkmark for certify, an X for deny. Hover over either button to see its label.

- The discard button tooltip now shows the number of pending reviews that will be affected, so you can confirm the scope before discarding.
