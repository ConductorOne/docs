...
---
{/* Editor Refresh: 2026-02-13 */}

<Update label="February 13, 2026">

### Service principals and workload federation

You can now create and manage service principals for programmatic API access to ConductorOne. Service principals provide a secure way to authenticate automated systems, scripts, and integrations without using personal credentials.

This release also introduces workload federation, which allows you to configure external identity providers and establish federation trusts with policy-based conditions. The new token exchange capability supports the RFC 8693 standard protocol for federated authentication.

To get started, navigate to **Settings** > **Service principals** to create your first service account, or **Settings** > **Federation providers** to configure workload federation.

### Access review improvements

- Campaign administrators can now save custom column configurations in the **Campaign tasks** view. Your preferred column layouts persist across sessions, making it faster to review large campaigns with the information you need most.

- Bulk certify and deny actions now correctly enforce justification requirements. If your policy requires a reason for approvals or denials, you'll be prompted to provide justification when using bulk actions, ensuring compliance with your organization's audit requirements.

### Policy and CEL improvements

- A new Automation CEL Agent helps you generate complex CEL expressions for workflow skip conditions, user ID filtering, and connector arguments. The agent validates your expressions with syntax checking and return-type verification, and provides in-context assistance with documentation and available variables.

- You can now look up users by display name in policy expressions using the new `c1.directory.users.v1.FindByName(string)` function. This is useful when user identifiers are stored as display names in custom fields.
...
- Dynamic manual provisioner assignment is now available for customers with the Access Reviews SKU. This feature allows you to route provisioning tasks to different assignees based on policy conditions.

- Policy provisioning now supports additional dynamic assignment methods including app owner delegation, group-based provisioning, manager delegation, expression-based routing, and entitlement owner delegation. Fallback provisioner options and configurable reassignment controls provide more flexibility in your provisioning workflows.

- The Okta AWS Federation connector now supports field name mapping for authentication and deployment parameters, enabling proper configuration of api-token, domain, aws-okta-app-id, and group-to-direct-assignment conversion settings.

### Task management improvements

- Basic users can now send reminders for their own outstanding approval tasks. If you're waiting on an approver to review your grant or revoke request, you can send a reminder directly from the task details page to help expedite the process.

### Notification improvements

- Ticket comments now appear in linked Slack threads, so recipients can follow ticket updates directly within their existing conversations. The system automatically handles rate limiting and prevents duplicate comments.

- Fixed an issue where Slack notifications could be sent multiple times during delivery retries. Each ticket comment now appears only once per notification thread.
