...
{/* DRAFT STATUS: Session 1 draft, consolidation/tightening pass, and Session 2 verify/polish all complete as of 2026-09-03 - every TODO/QUESTION comment resolved (fixed, cut, or user-overridden with rationale recorded inline). Outstanding: the MCP resources item is kept despite confirmed SKU_MANUAL status per explicit user override - re-verify its rollout status isn't stale before actually publishing if time passes before this merges. The MS Teams campaign-notifications item was reverted the same day (see REMOVED comment in Access request and review improvements) after initially being kept on override - the developer wasn't reachable to confirm, so flag evidence won out; tracked for next cycle. PR #24638 (MCP guided-connect GA) should also be re-checked if it's been a while since 2026-09-03. */}

### Agent classifiers

{/* SOURCE: colleague docs-update copy, not from PR digest. RESOLVED 2026-09-03: gating flag confirmed as FEATURE_FLAG_ID_AIGOV_AGENT_GUARDRAILS, status SKU_ALL (on automatically for any tenant with the SKU) - not gated the way the other cut items this cycle were. The doc page's "Early access" banner reflects a SKU-availability/design-partner rollout decision, not a code-level gate; consistent with the flag status, no conflict. */}
...
{/* SOURCE: colleague docs-update copy, not from PR digest. RESOLVED 2026-09-03: re-confirmed FEATURE_FLAG_ID_MCP_RESOURCE_SURFACE is still SKU_MANUAL ("resource governance is in development") on fresh origin/main. User made the call to keep this item despite the flag status - not a verification gap, a deliberate override. */}
...
- **Set bulk request limits**: Admins can now set their own max number of entitlements a user can request at once in bulk (1-100, default 10) from **Governance > Forms**. See [Configure tenant-wide request settings](/product/admin/customize-requests#configure-tenant-wide-request-settings).
  ...
  {/* Docs updated 2026-09-03: added a new subsection to customize-requests.mdx documenting the Forms settings drawer (gear icon on Governance > Forms), confirmed against frontend source (FormsSettingsDrawer.tsx) - exact label is "Maximum entitlements per bulk request". */}
...
{/* REMOVED (2026-09-03): "Campaign notifications can go to Microsoft Teams" (PR #24144) - gated behind FEATURE_FLAG_ID_MS_TEAMS_CAMPAIGN_MESSAGES, which is FEATURE_FLAG_STATUS_SKU_MANUAL on origin/main as of this date (depends on FEATURE_FLAG_ID_MS_TEAMS, also SKU_MANUAL). Initially kept on a user override, then reverted the same day: the developer who worked on it wasn't available to confirm rollout status, so the flag evidence wins per the standing rule. User is tracking this for the next cycle and will check with the developer directly. Revisit once confirmed GA, following the same pattern as the policy-references item. Companion doc tweaks in campaigns.mdx (two "Microsoft Teams is also available..." sentences) reverted alongside this. */}
...
- **Connector sync failure finding type**: Two consecutive failed full syncs on a connector now raise a "Connector sync failing" finding, which auto-resolves on the next successful sync. Can be turned off per tenant. See the [finding catalog reference](/product/admin/findings#finding-catalog-reference).
  ...
  {/* PR #22952 */}
  {/* Docs updated 2026-09-03: added a Configuration-category row to the finding catalog table in findings.mdx. */}
...
- **See and manage the full owner list**: From an app's details page, click **Edit owners** — the picker now has an "All users / Owners" toggle, so admins can see and manage the complete current owner list, including suspended or otherwise filtered-out users.
  ...
  {/* PR #23832 */}
  {/* Location confirmed 2026-09-03: SelectOwnersModal, entry point is the "Edit owners" action in the app details page's info bar (frontend/components/pages/app/details/tabs/app-details/SelectOwnersModal.tsx, SmartAppDetailsV2.tsx:720). Same modal is reused from ~15 other places (connectors, entitlements, groups, etc.) but the app details page is the canonical one to describe. */}
...
- **Check a session policy's blast radius before enforcing it**: Four new read-only API endpoints let admins preview which session policy applies to a given user and why (direct assignment, group membership, or tenant default), and list every user a policy applies to — direct and group-conferred, with the conferring group named.
  ...
  {/* PR #24521 */}
  {/* RESOLVED 2026-09-03: confirmed 4 endpoints, not 3 - ListAssignments, GetEffectiveSessionPolicy, ListUserPolicies, SearchPolicyUsers (protos/c1api/c1/api/session_policy/v1/session_policy.proto, ductone/c1). All 4 have operationIds and full schemas in pkg/registry_api/c1oas/openapi.yaml on origin/main, so they are live in the public API spec. NOT linked: this repo has no local api-reference page for session-policy endpoints yet (it's generated from the remote OpenAPI spec) and I couldn't confirm the live rendered URL without checking the actual site - check c1.ai/docs/api-reference before adding a link. */}
...
- **Breaking change: Automation expressions must reference workflow context directly**: An expression consisting of the bare workflow context, or one that passes it to a function, no longer compiles — name the step and field directly instead. This is a save-time check: an automation already running is unaffected until you next edit and save it, at which point an indirect-reference expression will need to be rewritten to a static path before it saves.
  ...
  {/* PR #24042 */}
  {/* RESOLVED 2026-09-03: checked PR #24042 directly - no effective/enforcement date given (already merged 2026-08-28, no flag gating), and no explicit "grandfathered" language, but the PR's own description confirms this is a compile/validation-time failure ("no longer compiles"), not a runtime block, which is why existing automations keep running until next save. No separate customer broadcast planned - user confirmed the digest mention is sufficient notice for this one. */}
...
- **Breaking change: Findings get an "Assignee," not an "Owner"**: The "Owner" column, filter, and detail field on findings are now labeled "Assignee" — how findings actually get assigned hasn't changed. If you consume the Finding object via API or webhook, or feed the audit log to a SIEM, see [Work with a finding](/product/admin/findings#work-with-a-finding) for the field and event-name changes.
  ...
  {/* PR #24617 */}
  {/* Docs updated 2026-09-03: added an Assignee bullet plus a Note with the full API/webhook/audit-log deprecation detail to the "Work with a finding" section of findings.mdx, since that page never actually documented an Owner/Assignee field before this - there was nothing to literally rename. */}
