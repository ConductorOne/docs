...
<Update label="A new home for every section">

### The admin nav caught up with everything else we've shipped

C1's admin navigation grew a lot of new capability without a matching home for it, so we gave it one. Each section still works the same way it always has — this is about where things live, not how they behave.

A few of the bigger moves:

- **Connectors** joined **Apps**, since a connector's whole job is governing access to one.
- **Automations** joined **Governance**, next to Campaigns and Access profiles — a more natural fit alongside onboarding and offboarding work than its old home.
- **Policies**, **Forms**, and **Functions** now live together in a new **Platform** section, along with Task log and most of what used to be under Integrations.
- **Directory** is now **Identities**, and **Identity security** is now **Security** — which also absorbed **Decoys** and the **Graph** explorer.
- **AI** and **Vaults** haven't changed. **Settings** hasn't either, though you'll find it further down the sidebar now.

One more thing: every section except **Apps** now has its own **Overview** dashboard, so the metrics that used to live crammed into one **Analytics** page are broken out by area instead.
