...
You can now assign a workflow step in a request or review policy to an agent. The agent evaluates each request that reaches the step using ConductorOne's built-in guidance plus any custom instructions you provide.
...
- **Search for what you need, see what grants it**: Searching the request catalog for a specific resource — a repo, a folder, a project — now surfaces the role, team, or group that grants access to it, even when the granting entitlement doesn't match the search term by name. Users can now find the right thing to request without knowing which group owns the resource they're after.
...
{/* Docs update needed: /product/admin/policies and /product/admin/audit-ai-tool-usage cover agent policy steps and the Conversations page generally, but not the new System tab or the C1 System identity attribution specifically. */}

- **Agent policy evaluations are now auditable conversations**: AI agent evaluations of approval policy steps are now attributed to the built-in **C1 System** agent identity and labeled as AI-agent actions in the audit log. Each evaluation is also recorded as a conversation you can review under **Agentic AI > Conversations**, System tab. How the agent evaluates and acts on a request — approve, deny, reassign, re-route, or comment — is unchanged. See [Review AI agent conversations](/product/admin/audit-ai-tool-usage).
...
Limiting a personal API key's permissions below your own role previously required Super Admin access, so everyone else's keys defaulted to full account permissions. Any user can now optionally restrict a key to a narrower role, such as read-only, when creating it. See [Create a personal API key](/conductorone-api/api#create-a-personal-api-key).
