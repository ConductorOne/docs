...
Every team has different data they care about, but the **Apps**, **App accounts**, **Entitlements**, and **Resources** tables previously showed the same fixed columns for everyone. You can now toggle columns on or off and drag to reorder — and your layout is saved per table. When you export, clicking **Generate CSV** opens the **Download to CSV** drawer, where you choose exactly which columns appear in the file, so you get a clean, targeted export instead of stripping out unwanted columns after the fact.
