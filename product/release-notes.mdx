...
Many configuration objects across C1 — connectors, policies, entitlement routing rules, SSO and branding settings, AI Gateway spend controls, and more — now show a **Change history** icon on their detail page. Click it to see a timeline of every change, with a clear before-and-after for each field.

Connectors get an extra layer of control: admins can now require a justification whenever a connector's configuration changes, captured alongside its change history.

{/* PR #25808 and #25438 (neither has release-notes text of its own - #25808 is a one-line internal flag-dependency wire-up, #25438 is the justification feature itself). Wording and object list sourced from ductone/c1 frontend (ObjectHistoryIconButton usage sites, ObjectHistoryDrawerContent) and Michael Burton's #product-releases heads-up on 2026-09-21 that OBJECT_HISTORY and CONNECTOR_CONFIGURATION_CHANGE_CONTROL (both SKU_MANUAL as checked that same day) are rolling out to all tenants that day. He owns CONNECTOR_CONFIGURATION_CHANGE_CONTROL in the flag registry, matching the announcement. Added on that word per Melinda, ahead of confirming the rollout actually completed. */}
