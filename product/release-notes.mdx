...
c1i is available now at [github.com/ConductorOne/c1i](https://github.com/ConductorOne/c1i). See [Install c1i](/product/cli/c1i) for setup instructions. For a human-friendly CLI experience, see [Cone](/product/how-to/cone).

### Schedule automation trigger

Automations now support a **Schedule** trigger that runs on a recurring schedule without targeting a specific user. Use it for periodic tasks like generating reports, triggering syncs, or running system maintenance workflows. The trigger can be configured to run hourly, daily, weekly, or monthly with timezone support, and appears alongside the existing Schedule triggers in the automation trigger picker.
...
Azure Blob Storage is now supported as an external data source. Admins can configure an Azure Blob container under **Settings** > **External Data Sources** to push audit logs to offline storage or a SIEM, or to ingest application data from CSV uploads. Authentication uses an Azure AD service principal (tenant ID, client ID, client secret) with the **Storage Blob Data Contributor** role on the target container.

### Usability improvements

- The **Groups** tab on an application's page lets you view all groups for that application and create custom groups that exist only in C1. Custom groups support the same visibility controls as other resources — set visibility to **Everyone**, **Members**, or **Owners** to control who can see the group and its entitlements. See [Manage resources](/product/admin/managing-resources) for details.

- The **Send Slack message** automation step can now send direct messages in addition to channel posts. Choose **Direct message** as the destination and specify recipients by selecting the subject user, choosing specific users by name, or writing a CEL expression to identify recipients dynamically.

- The **Automations** page now includes an **Executions** tab with a combined log of all automation runs across your organization. Filter by automation, status, or app to find specific runs — without navigating to each automation individually.

- The C1 Slack app has a new name and logo. Going forward, use **@C1** instead of **@ConductorOne** to mention or message the app in your workspace.

### Fixes

- The **Sync now** button in the Membership automation section now appears for newly configured automations, even before the first run has completed.
