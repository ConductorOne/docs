...
{/*
- Hovering over an app icon badge on a user or account name now shows a tooltip with the app name, making it easier to distinguish identities across multiple app instances.
*/}
...
Role mining analyzes the access granted across your organization and surfaces suggested access profiles based on patterns it finds. Instead of building access profiles by hand, you can let ConductorOne identify which entitlements are commonly held by similar groups of people — grouped by department, job title, employment status, manager's team, and more — and turn those patterns into ready-to-use profiles with a single click.

ConductorOne offers two ways to use role mining:

- **Suggestions**: ConductorOne continuously analyzes access patterns across your organization and surfaces recommended access profiles automatically after each connector sync.
- **Custom analysis**: You define a specific cohort and ConductorOne analyzes that group's access patterns on demand.

To learn more, visit [Discover access profiles with role mining](/product/admin/role-mining). Ready to try it out? Contact the ConductorOne Support team to enable the feature for your tenant.

### Automation improvements

- Three new automation steps support credential generation and delivery workflows. Use **Generate password** to create a random or policy-based password, **Set credential** to apply it to a user's account in a connected application, and **Store credential** to store it in a ConductorOne vault and deliver it securely to a recipient. These steps can be chained together to automate onboarding flows that provision access and set initial credentials in a single automation.
...
- Cone, the ConductorOne CLI, now supports AWS IAM Identity Center (SSO) integration. Users can now run `cone aws setup` once to generate AWS CLI profiles for every permission set available to them in ConductorOne, then use their normal AWS CLI workflow — Cone handles access requests and temporary credential fetching automatically in the background. If a user doesn't have access when they run an AWS CLI command, Cone submits a request and returns credentials immediately if the request is auto-approved. See [Use Cone with AWS SSO](/product/how-to/cone-aws-sso-integration) for setup instructions.

### Resource visibility controls

Admins can now control the visibility of resources and their entitlements in ConductorOne. Each resource has a new **Visibility** setting with three options: **Everyone** (all users in the organization, the default), **Members** (users with an active grant on the resource, plus owners), or **Owners** (resource and entitlement owners only). Restricting a resource's visibility hides it from search results, directory listings, and the access catalog for users who don't qualify. Access profiles take priority — users included in an access profile for a restricted resource can still see and request its entitlements. SuperAdmins can always see all resources regardless of visibility settings. See [Resource visibility controls](/product/admin/managing-resources#resource-visibility-controls) for details.

### Usability improvements

- Access review task tables now support optional columns for app user email, username, and profile attributes. Enable them from the column customization menu to see more context about reviewees during a review.

- The submit button in the end user reviews UI has moved from the top to the bottom of the screen, putting it closer to the review tasks you're working through.

- Toasts and alert notifications now appear at the top center of the window instead of the top right.

- A bulk configure action is now available in the linked entitlements configuration drawer.

### Connector updates

Learn about this week's connector updates and new connectors in the [connector release notes](/baton/_release-notes).
...
ConductorOne can now sync identity risk scores from CrowdStrike Falcon Identity Protection and surface them in context across ConductorOne. Reviewers see an identity's risk score and contributing risk factors directly on access review tasks, in the task log, and on access request approval tasks — without leaving ConductorOne.
...
ConductorOne now supports dark mode! If your operating system is configured to prefer dark mode, ConductorOne will automatically use the dark theme when you sign in.
...
* ConductorOne now prevents you from requesting access to an access profile you're already enrolled in, avoiding duplicate enrollment tasks and notifications.
...
Excited to try out the review assistant? Contact the ConductorOne Support team to enable the feature for your tenant. 
...
When used in a campaign template, the criteria are saved dynamically — each time you prepare a campaign from the template, ConductorOne re-evaluates which entitlements match, so newly tagged entitlements are automatically included without any manual template updates. 
...
You can securely share credentials, files, API keys, and other sensitive content with teammates or external contacts from ConductorOne. Content is encrypted in your browser before upload, so ConductorOne never sees your plaintext. Recipients authenticate via SSO (internal) or a one-time magic link (external),and you control how long a secret stays available as well as how many times it can be viewed. Administrators can view metadata and audit logs for all secrets across the tenant.  
...
Service principals are machine identities for non-human actors (such as scripts, CI/CD pipelines, Terraform runs, and API integrations) that are fully separate from any human user account. Assigning ConductorOne roles to a service principal instead of a human account means automated processes have only the access they need, and that access is auditable, ownable, and revocable. Learn more in the [Service principals](/product/admin/service-principals/overview) documentation.
...
Ready to get started with service principals? Contact the ConductorOne Support team to enable the feature for your tenant. 

### Early access: Functions 

Functions are serverless TypeScript functions that let you extend ConductorOne's identity governance capabilities with your own logic — without managing infrastructure. Use them to call external APIs, implement organization-specific workflows, and automate tasks that go beyond built-in features.
...
Excited to give functions a try? Contact the ConductorOne Support team to enable the feature for your tenant. 
...
- To speed up the creation of new [ConductorOne groups](/product/admin/groups), you can now click **Duplicate** on any existing group to create a new group pre-filled with the original's name, description, and automation rules, then make your adjustments from there. 
...
- An application's grant feed CSV export now includes a **TaskID** column showing the associated task ID for each grant added or removed in ConductorOne. This provides parity with the information shown in the **Grant feed** drawer and simplifies audit workflows.
...
The [ConductorOne integration for Microsoft Teams](https://marketplace.microsoft.com/en-us/product/office/WA200009797) is now live on Microsoft Marketplace, making it easier to discover, install, and deploy ConductorOne directly within your Microsoft ecosystem. Ready to bring C1 into your Teams workspace? Check out our docs on the [MS Teams integration](/product/admin/ms-teams-public-public) to get started. 
...
With the new [ConductorOne app for Microsoft Teams](/product/admin/ms-teams-public), you can interact directly with ConductorOne without leaving Teams to manage access requests, receive notifications for new approval tasks, and approve or deny requests directly from the Teams interface. This integration also allows users to track the progress of their open access requests and get immediate updates. 
...
- Accounts in [Docusign](/baton/docusign) that have a **Pending** status are now shown as **Disabled** in ConductorOne.
...
**OneLogin connector is now generally available.** Check out the [OneLogin integration instructions](/baton/onelogin) to connect your OneLogin instance with ConductorOne.
