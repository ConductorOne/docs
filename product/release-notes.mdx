...
### Retry provisioning without restarting the task

{/* Docs update needed: /product/how-to/access-change-tasks doesn't document the Retry provisioning action yet. */}

When connector provisioning fails on an access task, you previously had to cancel the task and start over once the issue was fixed, losing any approvals already collected. You can now retry provisioning directly on the task instead. See [Access and change tasks](/product/how-to/access-change-tasks).

### Custom scheduling for access review campaign templates

Campaign templates previously offered only fixed recurrence presets. You can now set a template to recur on any custom interval — for example, every 2 days or every 3 months — and choose exactly how long each campaign runs. See [Manage campaigns](/product/admin/campaigns).

### Access requests wait for entitlements created ahead of sync
...
You can now set whether an MCP server's tools require enablement or are auto-enabled directly on the server, independent of your tenant's default AI governance setting. See [Govern tools and toolsets](/product/admin/tools-and-toolsets).

### Readable IAM policy documents in access reviews

{/* Docs update needed: /product/how-to/review-tasks doesn't cover resource attributes or an IAM policy viewer. */}

Reviewing an AWS resource's access previously meant reading its IAM policy document as an unformatted block of raw JSON. Resource attributes that carry an IAM policy document now show a **View policy** button that opens it as a readable, collapsible JSON tree — available on resource-level access reviews and in task details. See [Review tasks](/product/how-to/review-tasks).

### Requestable C1 groups
...
### Step-up approvals that also require a comment

{/* Docs update needed: /product/admin/step-up-auth documents the step-up flow but doesn't mention the comment-required case. */}

Approving a task that required both step-up (MFA) authentication and a comment previously failed with an error and couldn't be completed. Approvers can now complete both requirements together — the approval prompts for step-up verification with your identity provider and completes once verified. See [Step-up authentication](/product/admin/step-up-auth).

### Generate CEL conditions from plain English

{/* Docs update needed: /product/admin/expressions doesn't document an AI-assisted generate action. */}

Writing a CEL condition previously meant working through an AI chat to iterate toward the right expression. Describe the condition you need in plain English and the CEL editor's **Generate** action drops the expression directly into the field in one step. See [Write expressions](/product/admin/expressions).
...
### Register MCP servers past relaxable OAuth discovery issues
...
Hosted MCP servers without an admin-set tool prefix previously fell back to a generic derived name. They now use the prefix the connector itself declares, so tool names stay recognizable to AI clients by default. See [Set up an MCP server](/product/admin/mcp-servers).
