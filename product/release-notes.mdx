...
### Ask ConductorOne AI why a task is stuck

A failed, stuck, or waiting task usually means digging through approval history, provisioning logs, and app-account state by hand to figure out where it actually broke — and whether that's even still true, since the picture can shift between when it stalled and when someone gets around to looking.

Super Admins can now ask ConductorOne AI to diagnose a specific task. The assistant reconstructs the full history — approval and provisioning state, access-profile origin, app-account access, and the connector handoff — and recommends the safest next step. Diagnosis is read-only: it traces and explains, but never changes the task itself.

See [Diagnose a stuck task](/product/admin/ai-assistant#diagnose-a-stuck-task) for details.

{/* PR #25528 */}

{/* REMOVED: Policy references ("build a policy rule/baseline that points to another policy") - FEATURE_FLAG_ID_POLICY_REFERENCES_POLICY is SKU_MANUAL as of 2026-09-18 (introduced 2026-08-04, never promoted since - single commit, no history of a GA change). docs PR #506 already holds this exact writeup and says the same thing: enabled for one customer only, do not merge until broader rollout is confirmed. On 2026-09-18 a developer twice told Melinda (including after being asked "are you sure?") that this had been broadly available since mid-August - code says otherwise. Re-add once the flag actually promotes to SKU_ALL (or similar), pulling the writeup back from PR #506. */}
...
- **A unified home for MCP sources.** Registered MCP integrations are now managed as MCP sources throughout ConductorOne, with a single guided "Add MCP source" flow covering both catalog and external sources — connection, access, tool approval, and authentication setup in one place. See [Set up an MCP server](/product/admin/mcp-servers) for details. {/* PR #25296 */}
{/* Note: linked to the existing mcp-servers.mdx page, which still uses "MCP server" terminology throughout - the "server" -> "source" rename is already tracked separately in ConductorOne/docs PR #531 (open, not yet merged). Not duplicating that work here; re-point this link if #531's page move/rename lands first. */}
...
See [Create a new application](/product/admin/applications#create-a-new-application) for the current new-app flow.

{/* PR #25052 */}
{/* Note: verified 2026-09-18 - product/admin/applications.mdx's "Create a new application" section already matches the described new-app flow. Couldn't find any existing page documenting the "add connector to an app you already manage" sub-flow specifically (owner picker, credentials-after-details ordering) - flagging as a doc gap rather than assuming it's covered too. */}
{/* Consolidated 2026-09-18: merged this section with what was a separate "Updated flows for adding new apps and connectors" section, and removed a duplicate one-line bullet that described the same shipped feature - both cited the same PR #25052. */}
...
- **Approval steps requiring the app owner now fall back correctly.** Fixed an approval policy bug where a step requiring review by the app owner could be reassigned to system admins when the app's owner-source grants hadn't yet been reconciled into the ownership store; approval evaluation now resolves owner IDs directly from the configured ownership sources in that case, and records the rescue in the ticket's audit trail. The same fix applies to entitlement-owner and app-resource-owner approval steps. {/* PR #25626 */}
...
- **Findings can now be assigned to a specific person, automatically.** Add a Set assignee step to any Findings → Transformation rule, and every finding it matches is assigned to that person as it arrives; a person's own manual assignment always wins over the rule. See [Transformation rules](/product/admin/findings#transformation-rules) for details. {/* PR #25487 */}
...
- **Identities & NHI is now available to every tenant.** The redesigned Inventory page - with dedicated Overview, Human users, Service, Secrets, NHI, and Agents tabs - was previously limited availability; it's now open to everyone. See [Non-human identities (NHI)](/product/admin/nhi) for details. {/* PR #24792 */}
...
- **Terraform support for MCP sources.** Terraform users can manage hosted and external MCP sources - source details, authentication configuration, and tool enablement policy - as code. See [C1's Terraform provider](/developer/terraform#getting-started-with-c1s-terraform-provider) - the new resources are documented on the provider's own registry page, not duplicated in this repo. {/* PR #25273 */}
