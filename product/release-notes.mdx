...
### MCP server authentication: HTTP Basic Auth

MCP servers in C1 now support HTTP Basic Authentication. Configure it in two modes depending on how credentials should be managed. See [Configure authentication](/product/admin/mcp-servers#configure-authentication) for details.
...
### Entitlement request forms

{/* TODO: add "See [page name](/path)" link once a docs page exists */}

A new **Forms** page under Governance lists every entitlement request form in your tenant in one place, so admins can browse and edit forms without opening each entitlement individually.

### Entitlement-based owners for apps and entitlements
...
### Automation revoke: access-only mode

{/* TODO: add "See [page name](/path)" link once a docs page exists */}

Revoke steps in automations now include an **Access only** option, which targets just the access entitlement on each app a user has an account on — one revoke task per app, rather than one per group, role, or permission. This is most useful in offboarding automations: it removes the user's app account without generating downstream revoke tasks and notifications for entitlements that are dropped automatically when the account goes away.

### Access review: reviewer attribute visibility

{/* TODO: add "See [page name](/path)" link once a docs page exists */}

Admins can now control which app user profile attributes reviewers see during an access review. Configure specific attribute keys per app in the campaign or template — for example, surface department and manager while keeping personal email hidden. Reviewers see no profile attributes by default; attributes are only shown when an admin has explicitly enabled them for that app.

### Usability improvements

- **Governance recents**: The Governance section in the left nav now shows a **Recent** list of the campaigns, policies, and access profiles you've recently opened. Pin any item to keep it at the top across sessions. Spotlight (Ctrl+K / Cmd+K) now searches access profiles directly.

- **MCP tools starting view**: The tools list on an app's MCP server now opens to a curated view of Featured and Popular tools, organized by classification. A **Visibility** column also appears in every MCP tool table alongside **Classification**.

- **Recently visited in Directory**: The Directory nav flyout now surfaces recently visited users and groups.

- **Recently visited in Workflows**: The Workflows nav flyout and the Cmd+K palette now surface recently visited automations and functions.

- **External ticket column in campaign reports**: Campaign reports now include an optional **External Ticket** column for tenants with external ticketing enabled. For each task linked to an external ticket — such as a Jira or ServiceNow ticket — the column shows the ticket ID as a link that opens it directly. Enable the column from the column picker in the Generate Report dialog.

- **Automations for app accounts without identity users**: Revoke entitlements and Grant entitlements steps in automations now work for app accounts that aren't linked to a C1 identity user, such as service accounts or shared accounts. Previously, automations triggered by AppUserCreated or AppUserUpdated events for these accounts would fail.

- **MCP server setup guidance**: MCP servers whose application has no users linked now show a warning indicator in the servers list and a banner on the server detail page with a direct link to the application to add users.

### Terraform provider v1.3.0

Terraform provider v1.3.0 is now available. This release adds `annotations` support to most writable resources — the provider automatically populates `managed_by` and `iac_workspace` annotations, and user-supplied values always take precedence.
...
See [Terraform provider](/developer/terraform) for upgrade instructions.
