...
The campaign setup flow and reviewer task list have been refreshed with clearer labels and more compact controls. Staging a campaign is now a single **Stage campaign** step that includes scope validation, and reviewer actions in the task list use icon buttons with tooltips to keep more of each review row visible. See [Create a campaign](/product/admin/campaigns) and [Complete access review tasks](/product/how-to/review-tasks) for updated documentation.

{/* Note for review before publishing:
- "Due date" label: confirmed in staging panel and campaign list; status bar may still show "Target completion" — verify in UI.
- "View submissions" placement: code shows it's in the overflow menu (⋯), not next to the progress bar as described in the PR — verify in UI.
*/}
