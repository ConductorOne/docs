...
### Get notified when a finding routing rule matches

Finding routing rules could sort findings but couldn't tell anyone when one fired. Rules can now send a notification when they match — to specific people on their own enabled channels, or to a Slack channel — delivered immediately or grouped into a digest, with either a count/severity summary or full finding details.
...
- **Customize the Findings rules tables**: Admins can now show, hide, and reorder columns on the Transformation and Routing rules tables under Findings settings.
