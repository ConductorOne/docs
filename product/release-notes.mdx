...
Each agent evaluation is attributed to the built-in **C1 System** agent identity and labeled as an AI-agent action in the audit log. Every evaluation is also recorded as a conversation you can review under **Agentic AI > Conversations**, **System** tab.
...
{/* Docs update needed: /product/how-to/create-requests documents App catalog search by entitlement name, not this behavior of surfacing the granting role/group when searching by resource name. */}

Searching the request catalog for a specific resource — a repo, a folder, a project — now surfaces the role, team, or group that grants access to it, even when the granting entitlement doesn't match the search term by name. You can find the right thing to request without knowing which group owns the resource you're after.

See [Request an app or permission](/product/how-to/create-requests#request-an-app-or-permission).

{/* Screenshot needed: request catalog search results for a resource name (e.g. a repo), showing the matching group/role entitlement surfaced. Placeholder path below. */}

<Frame>
<img src="/images/product/assets/PLACEHOLDER-catalog-search-grants.png" alt="Request catalog search results for a resource name, showing the group or role entitlement that grants access to it."/>
</Frame>
...
- **Retry provisioning without restarting the task**: When connector provisioning fails on an access task, you previously had to cancel the task and start over once the issue was fixed, losing any approvals already collected. You can now retry provisioning directly on the task with the new **Retry provisioning** action. See [Access and change tasks](/product/how-to/access-change-tasks).
