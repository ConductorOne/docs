...
{/* DRAFT STATUS: built from PR digest chunk 1 of 2 (period Aug 21-28), plus colleague-provided copy for two docs-update projects (Agent classifiers, Govern MCP resources). Awaiting chunk 2 (Aug 28-Sept 4) before final section ordering, headline-item selection, and the Session 2 verify/polish pass. */}
...
The system log adds four new Identity & Access event classes — Account Change, Entity Management, User Access Management, and Group Management — plus a `groups[]` field naming each actor's platform roles and a shared `correlation_uid` linking the paired events a privilege change now produces. Admin-tier actors also log a new actor type value as of September 3, 2026, distinguishing them from non-admin users for the first time; affected customers were notified separately ahead of this change. See the [system log reference](/product/admin/system-log) for the full event catalog.

{/* SOURCE: docs PR #511 (ConductorOne/docs), merged 2026-08-31, not from the PR digest. Breaking change (actor.user.type_id remap) already covered in a customer broadcast per user - kept brief here, general-awareness/historic-record entry only. */}
