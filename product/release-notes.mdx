...
We've added a new [OpenAI connector](/baton/openai) to our integrations library, enabling you to manage access to your OpenAI organization. The connector syncs users, projects, and roles, allowing you to run user access reviews and enable just-in-time access requests for OpenAI resources.

### Extension grant task improvements

When an extension grant task is cancelled because the associated app user was deleted (e.g., due to access expiration with provisioning), a new extension grant task will now be automatically created. This new task targets the C1 user, allowing them to easily re-request access without manual intervention and preventing "bricked" tickets for administrators.

The new task will include a comment linking back to the original cancelled request, and the audit log will clearly state that the task was "created from" the original task, including a link to the original.

### New CEL variables for policy conditions

Policy conditions can now leverage `task.subjectUserId` and `task.requestorUserId` in CEL expressions. This enables more granular policy configurations, such as automatically confirming tasks where the requestor is the subject (e.g., a user revoking their own access).

### Application search improvements

Application searches now display results sorted alphabetically by display name, making it easier to find the apps you're looking for.

### Help desk admin grant revocation
...
### Fallback groups for policy steps

You can now assign fallback groups to any policy step that also has fallback users. This provides additional flexibility in ensuring tasks are always assigned to an available reviewer.

### CEL attribute mapping error visibility
...
* We've redesigned the CEL editor with an updated theme for improved readability and a better editing experience.
