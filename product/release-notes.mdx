...
A failed, stuck, or waiting task usually means digging through approval history, provisioning logs, and app-account state by hand to figure out where it actually broke.

Super Admins can now ask ConductorOne AI to diagnose a specific task. The assistant traces its full history — approvals, provisioning, access-profile origin, and the connector handoff — and recommends the safest next step, without making any changes itself.
...
**A single guided flow for creating apps and adding connectors.** Instead of a multi-page form, C1 now asks only the questions relevant to your choices — pick a source (connector, file, external data source, or an app from your identity provider), then set a name, description, and owners through the same picker for both apps and connectors, before configuring credentials.

See [Create a new application](/product/admin/applications#create-a-new-application) for details.
...
- **A data drop is easier to act on.** When a sync looks like it dropped data unexpectedly, C1 now suggests running a new sync with one click, and shortened the message so the next step is obvious — in the banner and in Slack. {/* PR #25188, #25209 */}

- **Require a reason for connector configuration changes.** Admins can now require a justification whenever a connector's configuration changes, audited alongside a secret-safe configuration history. {/* PR #25438 */}

### AI and MCP improvements

- **Agents can now talk to each other.** Customer agents connected to your C1 MCP endpoint can discover and hold a conversation with c1aw agents, and admins can manage those sessions the same way. {/* PR #24790, #24783 */}

- **MCP access now checks the toolset grant, not just app membership.** Being a member or admin of an app no longer implies access to its MCP server — you need an explicit grant on that server's toolset. {/* PR #24574 */}

- **One place to manage MCP sources.** Registered MCP integrations — catalog and external — now share a single guided setup flow for connection, access, tool approval, and authentication. See [Set up an MCP server](/product/admin/mcp-servers) for details. {/* PR #25296 */}
...
- **One less step to get started.** Setting up C1 Gateway and MCP connections no longer requires accepting AI Access Management terms first. {/* PR #24830 */}

- **Shorter, more focused AI responses.** C1 AI now gives brief up-front plans for multi-step tasks and skips routine narration while it works. {/* PR #24941 */}

{/* REMOVED: AI access requests receiving clearer recipient guidance/errors (PR #24944) - cut in the 2026-09-18 trim pass as too vague/low-impact for a customer digest ("clearer guidance" with no concrete example). */}
...
- **Locked entitlements now request against the right policy.** Entitlements that are locked but don't have their own request policy now correctly use the application's policy, matching what the UI already showed. {/* PR #25095 */}

- **Access search is more reliable.** It keeps focus as you type, shows each entitlement's own expiration instead of borrowing one from another grant, and no longer loses your place when a filtered page comes back empty. {/* PR #24929, #24927 */}

- **Find your blocked request faster.** The "already has a pending task" banner now links each listed item straight to its open task. {/* PR #24824 */}

- **A denial always reads as a denial.** Denial emails now always correctly say denied, instead of occasionally telling the requester their request was granted. {/* PR #24743 */}

- **Reviewer denial reasons reach your ticketing system automatically.** Access-review revoke tasks now include the reviewer's justification, and existing templates pick it up with no changes. See [Customizing ticket templates](/product/admin/external-ticketing#customizing-ticket-templates) for the new template variables available to custom templates. {/* PR #25108 */}

- **Campaigns can now scope to pending invitations.** Accounts with an outstanding invitation the principal hasn't accepted yet can now be included in a campaign's scope, instead of being silently dropped. {/* PR #25014 */}

- **Lopsided access-conflict groups are no longer a workaround.** Conflict monitors can now handle groups of very different sizes (say, 90 and 11) as long as the total stays under 1024 — no more splitting one large group across multiple monitors. {/* PR #24868 */}

- **A stuck campaign close no longer stays stuck.** Campaigns now resolve this automatically, and any genuinely unresolved review returns a clear, recoverable result. {/* PR #24845 */}

- **Add a Resource description column to reviews.** Reviewers can now see a resource's custom description — or connector-synced metadata as a fallback — right in the review table. {/* PR #24812 */}

- **Approval steps requiring the app owner now stay with the app owner.** A step could get reassigned to system admins when the app's ownership data hadn't fully synced yet; the right owner is now found either way. {/* PR #25626 */}

- **Task visibility now matches who can actually see it.** Simply having participated in a task in the past no longer makes it visible in your task list — visibility now follows the same rules as task details. {/* PR #25374 */}

{/* REMOVED: Slack/Teams campaign channel name showing immediately in Settings (PR #24799) - cut in the 2026-09-18 trim pass, too minor to be compelling on its own. */}
...
- **Consistent behavior for enrolled-user requests.** When adding scopes to an access profile, requests for already-enrolled users are now only created when you check that option — matching how entitlements already work. {/* PR #25665 */}

- **Campaign progress now reads accurately.** Progress stays below 100% while any review is open, and above 0% once work has started. {/* PR #25346 */}

- **Access reviews sync faster.** Approve/deny decisions now reflect instantly on large campaigns, with fewer redundant background requests. {/* PR #21773 */}

### Automation improvements

- **Resource-type filters are now real pickers everywhere.** Grant Found, Grant Deleted, and revoke criteria all use the same faceted, searchable resource-type picker, with real counts and disambiguation when two apps share a name. {/* PR #25146, #25042 */}

- **Fewer stuck approvals.** Fixed requestable automation approvals that could get stuck before expression-based approvers were evaluated. {/* PR #24990 */}

- **Find automation steps faster.** Builders can now search available steps and trigger types by name or description instead of scanning the full list. {/* PR #25690 */}

### Role mining

Role-mining filters now hold up better against messy data: high-cardinality attributes (like Squad or Tribe) stay searchable instead of disappearing, and an attribute stays visible even with no values, over 200 values, or a temporary preview failure. {/* PR #25500, #25495 */}

### Vaults and secrets

**My resources** brings together everything a user owns or has access to — groups, vaults, credential stores, and secrets — in one page, now also with Apps and Entitlements tabs. Secrets shared with you show up alongside your own, and old Groups/Secrets bookmarks still redirect here. {/* PR #24681, #24680, #24648, #25117 */}

**Faster headless device approval.** Headless devices that join a vault with a user-minted enrollment code are now staged and auto-approved — no manual approval needed, and C1 still never stores the vault contents themselves. {/* PR #24971 */}

### Usability improvements

- **Findings can be assigned to a specific person.** Assign one manually, or automatically with a new Set assignee step in Transformation rules — a manual assignment always wins. See [Transformation rules](/product/admin/findings#transformation-rules) for details. {/* PR #25487, #23470 */}

- **Identities & NHI is now available to every tenant.** The redesigned Inventory page — with dedicated tabs for human users, service accounts, secrets, NHI, and agents — was previously limited access; it's now open to everyone. See [Non-human identities (NHI)](/product/admin/nhi) for details. {/* PR #24792 */}

- **See a group's size at a glance.** Groups now show their member count at the top of the group detail page, so you don't have to count rows in the Members table. {/* PR #25412 */}

- **C1 emails no longer get cut off in Gmail.** Rendered emails are about 55% smaller, keeping every notification under Gmail's clipping limit. {/* PR #25247 */}

{/* REMOVED: command search reaching Shadow MCPs / Virtual MCP servers / password policies / unmanaged apps (PR #25695) - 2 of 4 destinations are behind non-GA flags; not worth a bullet for "password policies" alone. Revisit once Virtual MCP servers / Shadow apps GA. */}

- **Better screen-reader support across C1.** Every admin list page now announces status icons, row selection, and column-resize widths correctly — nothing changes visually. {/* PR #25646 */}

- **Chart legends no longer overlap your data.** Pie, donut, and time-series charts move a long legend below the chart, and clicking a legend item now focuses on just that slice instead of repainting the whole chart. {/* PR #24915, #24878 */}

- **Policy comments now show you what they'll actually post.** Wait-step and SLA-violation comment fields hint that markdown works and preview the result, and the policy summary renders the comment instead of showing raw markdown source. {/* PR #23155 */}
...
{/* REMOVED (2026-09-18 trim pass, too minor to be compelling on their own): clearer table headers/pagination styling (#24870); paginated dropdowns loading more on scroll (#25457); local policy editing/dropdown z-index fixes (#24823); navigation highlighting consistency + Vaults redirect context (#25132). */}

### API and developer tools

- **Manage AI governance through the API.** Classifiers, tool gates, classifier bindings, templates, and the agent classifier policy can all be managed through the public REST API. {/* PR #25540 */}

- **Terraform support for MCP sources.** Manage hosted and external MCP sources — details, authentication, and tool enablement policy — as code. See [C1's Terraform provider](/developer/terraform#getting-started-with-c1s-terraform-provider) for the new resources, documented on the provider's own registry page. {/* PR #25273 */}

{/* REMOVED: clearer runnable code-mode examples for AI integrations (PR #24974) - cut in the 2026-09-18 trim pass; better docs/examples aren't themselves a product capability, too meta for a customer digest. */}

- **Clearer signal when C1 is briefly overloaded.** API requests that fail from temporary capacity limits now return 503 with a Retry-After header instead of a generic 500; rate limits (429) are unchanged. {/* PR #25267 */}

- **Manage AI spend approvals through the API.** When governed AI spend is enabled, API clients can request allowance changes, check status, and retrieve review details permitted by their role. {/* PR #24952 */}

### Breaking changes

- **Breaking change: a revoke criteria scoped only by a legacy resource-type name now fails closed.** It used to silently match every app that happened to share that name; it now fails the step instead. Re-scope any automation still relying on name-only matching. {/* PR #25042 */}

- **Breaking change: deleting a Paper Vault no longer also revokes the credential.** `DELETE /api/v1/secrets/{vault_id}` now deletes only the Paper Vault; use the new `POST /api/v1/app-secrets-admin/revoke` to queue a credential revoke separately. Service SDKs rename Revoke to Delete to match. {/* PR #25587 */}

- **Breaking change: the spend-remedy-status endpoint is gone.** `GET /api/v1/tasks/{task_id}/spend-remedy-status` has been removed — migrate to the task APIs. JS/TS clients must handle spend-control revision values as strings, and integrations with a closed list of task action types need to handle the new `TYPE_SPEND_REMEDY = 7`. {/* PR #25374, #24952 */}
