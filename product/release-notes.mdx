...
Previously, anyone who needed to manage AI governance settings, MCP clients, or AI connections required Super Admin access — a broader grant than the work required. The new AI Governance Administrator role scopes that access precisely, so you can delegate AI governance responsibilities without over-provisioning. When the feature flag is enabled, the role appears in role assignment and limits who can create new managed apps when adding an MCP server to Super Admins and AI Governance Administrators.

### Bulk role assignment on the Users page

Assigning or correcting roles for a group of users one at a time is tedious, especially when onboarding a team or cleaning up after an org change. You can now select multiple users on the **Users** page and assign roles to all of them at once with the **Change role** action. See [User roles](/product/admin/user-roles) for details on available roles.

### Customizable columns and CSV export for key tables

Every team has different data they care about, but the **Apps**, **App accounts**, **Entitlements**, and **Resources** tables previously showed the same fixed columns for everyone. You can now toggle columns on or off and drag to reorder — and your layout is saved per table. When you export, a **Download to CSV** drawer lets you choose exactly which columns appear in the file, so you get a clean, targeted export instead of stripping out unwanted columns after the fact.
...
- **"Access was inappropriate" reason chip**: Reviewers previously had no built-in reason chip for access that shouldn't have been granted in the first place. The new **"Access was inappropriate"** chip is now available in the access removal, revocation, and certification comment dialogs.
- **Webhook URL in trigger drawer**: When setting up an [inbound webhook](/product/admin/webhooks-inbound), you previously had to construct the full endpoint URL from the listener ID. The trigger drawer now shows the complete URL ready to copy.
...
- **Export policy rules to CSV**: You can now export [policy](/product/admin/policies) rules to CSV directly from the policy detail page or the policy list page, making it easier to audit or document your policies outside C1.
- **Owner selection when creating apps**: The inline owner pickers in the create app flow offered limited search and no way to filter by attributes. When creating an app, you now assign owners through the full **Select owners** modal, with the same search, filtering, and Users/Entitlements tabs available elsewhere in C1.

### Connector updates

Learn about the latest connector updates and our many new connectors in the [connector release notes](/baton/_release-notes).
