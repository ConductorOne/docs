...
{/* DRAFT: restructured per editorial pass - headline features (Campaign assistant, NHI dashboard, onboarding plan) lead, smaller items folded into grouped "improvements" bullet sections so they don't compete visually with the headline features. Bug fixes and the Secret Access Vault API restriction excluded - this digest covers what's new, not what's fixed/restricted. Open questions and TODOs marked inline; resolve before merging. */}

### AI Campaign assistant is now generally available

Setting up an access review campaign means configuring policy, owners, and scope by hand, so teams often put off launching a review or reuse a template that doesn't quite fit. The AI-powered Campaign assistant is now GA for all customers with C1 AI enabled: describe the review you need in plain language, and it configures policy, owners, and scope for you to confirm before the campaign runs.

{/* PR #22238 */}
{/* QUESTION: does this get its own doc page, or does it belong as a new subsection under Create an access review campaign? */}
{/* TODO: screenshot or short clip of describing a campaign in plain language and the resulting config */}

### See all your identities, human and non-human, in one place

Non-human identities — service accounts, API secrets, agents, and other machine identities — tend to sprawl across apps with no clear owner and no single place to check on them. The new **Identities & NHI** page (under **Identity security**) gives you one view for all of it: human users, service accounts, secrets, agents, and other NHI resources, each in its own filterable, searchable, exportable table.
...
The **Overview** tab surfaces where to focus first: **ownership coverage** shows what share of each identity type has an assigned owner, and **secrets health** flags credentials that are expired, unused for 3+ months, or aging past 6 months.

<Frame>
<img src="/images/product/assets/nhi-dashboard-2.png" alt="Ownership coverage and Secrets health panels on the Identities & NHI overview page."/>
</Frame>

{/* NOTE: this page also shows Findings-related UI (a Recent findings panel, Has findings filters, Findings columns on every tab), but Findings hasn't shipped yet, so I've left it out of this description and cropped it out of both screenshots. Revisit this copy once Findings is user-facing. */}
{/* QUESTION: no PR number given for this one - is there a doc page (existing or new) this should link to, and what's the official nav path/page name to cite? */}

### A guided setup plan when you sign up

New customers get guided setup for access profiles, access reviews, and automated access management in one flow — reducing time-to-value and eliminating the configuration overhead that slows down early adoption. See [Get a personalized onboarding plan](/product/admin/ai-assistant#get-a-personalized-onboarding-plan) for details.

{/* QUESTION: no PR number given - please share it so I can cite it in the source comment like the other entries. */}
...
- **See what a reviewed resource can access**: Reviewers previously had to leave the task to check what the resource itself could reach, losing the context that makes a decision easy to justify. The task details drawer now shows what the reviewed resource has access to, right alongside the review.
  ...
  {/* PR #22129 */}
  {/* QUESTION: is this specific to resource-level review tasks, or does it also show up on user-level tasks? */}

- **Bulk-revoke denied reviews**: Revoking access for everything you've denied in a campaign previously meant provisioning each one by hand. You can now select multiple denied reviews from a campaign's Access reviews tab and revoke them all at once.
...
- **Remove entitlements straight from the scope summary**: Dropping an entitlement from a campaign or template's scope previously meant going back into the scope configuration to find it. You can now remove entitlements directly from the scope summary.
  ...
  {/* PR #21928 */}
  {/* QUESTION: confirm this applies to both campaigns and templates, or just one */}
...
- **Campaign reports via the public API**: Getting a campaign report into another system previously meant exporting from the UI and moving the file yourself. Campaign report generation (JSON/CSV/XLSX) is now available on the public API, including for campaigns that are still in progress.
  ...
  {/* PR #21265 */}
  {/* TODO: link to the API reference page for this endpoint once published */}

- **Policies can reference other policies**: Keeping related policies in sync previously meant copying rule logic between them by hand. Behind the new `POLICY_REFERENCES_POLICY` feature flag, a rule outcome or policy baseline can now defer to another policy of the same type instead.

  {/* PR #22677 */}
  {/* QUESTION: is this flag self-serve, or something support/CS enables per tenant? affects how we phrase availability */}

### Request and user management improvements
...
- **Filter devices by owner and type**: Finding a specific device in the devices list meant scanning the whole list by eye. You can now filter by owner and device type, and the status filter covers the full lifecycle (Pending, Active, Revoked, Disabled) instead of only Active and Revoked.

  {/* PR #22716 */}

### Notification settings improvements

- **Clearer notification settings**: Notification settings previously mixed together things you can control and things you can't, making it hard to tell what a toggle actually did. The "your access request was created" confirmation is now its own setting you can turn on or off per channel (email, Slack, and Teams) — on by default, so nothing changes unless you opt out. Provisioning notifications moved under **Requests**, and always-on system notifications are no longer shown as a toggle since there was never anything to switch off.

  {/* PR #22167 */}

- **Automations no longer go silent with notifications off**: Turning off system notifications in Admin settings previously blocked automation steps that send an email or Slack DM, even though those steps aren't a "notification" in the usual sense. Send-email and Slack DM automation steps now deliver regardless of that setting.

  {/* PR #21572 */}

### Agentic AI and MCP improvements

- **Resume paused MCP servers faster**: Resuming a paused MCP server previously meant digging into an overflow menu to find the sync action. The MCP servers list now shows a **Resume** button inline for paused servers.

  {/* PR #22065 */}

- **Trend charts from the AI assistant**: The AI assistant could report on a point-in-time number but not show how it moved. Ask it for a trend and it can now generate over-time charts (e.g. grants vs. revocations, review decisions, automation runs) directly from C1's aggregated metrics — fast, with exact numbers you can trace to their source, rather than hand-computed from raw records.
...
{/* REMOVED: "Get notified when a finding routing rule matches" (PR #22397) - Finding routing rules are part of the unshipped Findings feature. Re-add once Findings is user-facing. */}

{/* REMOVED: "Secret Access Vault owners are now read-only via the public API" (PR #22337) - this is a security-hardening restriction (closes a gap where the v2 owner API had no guard against writes to C1-app vault resources), not a new capability, and reads as a downer in a "what's new and exciting" digest. Cut per editorial review on 2026-08-07. Revisit as a separate API changelog entry for integrators if that's ever wanted. */}

### Usability improvements

- **Customizable columns for app account grants**: The Grants tab on an application account now supports configurable columns, including resource type, description, grant count, and grant origins (when grant reasons are enabled), alongside the entitlement and revoke controls.

  {/* PR #22791 */}
  {/* QUESTION: raw PR notes say this tab "once again" shows these columns, suggesting a regression fix rather than a new feature. Confirm which framing is accurate before publishing. */}
...
{/* REMOVED: "Customize the Findings rules tables" (PR #21939) - describes column controls on Findings' Transformation and Routing rules tables, and Findings hasn't shipped yet. Pulled per the same rule applied to the NHI dashboard section above. Re-add once Findings is user-facing. */}
