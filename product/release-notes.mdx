...
## Product digest: September 18, 2026

{/* TODO: headline section needed - Virtual MCP servers (PR #25355, FEATURE_FLAG_ID_VIRTUAL_MCP_SERVER) and the whole Shadow apps arc (SHADOW_APP_FINDINGS / SHADOW_MCP_FINDINGS) were cut on 2026-09-18 after checking ductone/c1 pkg/builtin_tenant/features_2026.go: both are DEV_MANUAL/SKU_MANUAL, not GA. Need a replacement lead item once the rest of this draft is confirmed GA. */}

### Policy references

You can now build a policy rule — or a policy's baseline — that points to another policy instead of repeating the same steps. Define a common flow once, like a security review or a manager-then-owner approval chain, and reuse it across every policy that needs it. Update the shared policy, and every policy pointing to it picks up the change automatically — no more hunting down and updating copies scattered across your policy library.

{/* TODO: merge docs PR https://github.com/ConductorOne/docs/pull/506 and reference it here as part of finalization */}

### Updated flows for adding new apps and connectors 

Creating a new application or adding a connector now uses a single guided drawer instead of a multi-page flow — only the questions relevant to your choices appear, so the process is shorter for most setups.

- New application: choose the app's primary source — a connector, a file, an external data source, or an application discovered from your identity provider (or track an empty app and add data later) — and C1 asks only the follow-up questions that source needs.
- Add connector: choose whether to create a new app or add the connector to one you already manage, optionally link the new app to an identity-provider application, then set a name, description, and owners — all before you configure credentials.
- Owners are now set the same way for both apps and connectors, using the standard multi-owner picker, replacing the old single "connector owner" email handoff.

{/* Note: docs for this already complete */}

### AI & Copilot

- **Ask ConductorOne AI why a task is stuck.** Super Admins can now have the assistant trace a failed, stuck, or waiting task's full history — approval and provisioning state, access-profile origin, app-account access, connector handoff — and get a recommended next step. It only recommends; it doesn't change anything on its own. {/* PR #25528 */}
{/* QUESTION: couldn't find a dedicated feature flag gating this in pkg/builtin_tenant/features_2026.go - the task_debugging intent is registered unconditionally alongside other GA intents in pkg/claw/skill/intent/skills.go, same pattern connector_authoring uses even though THAT one is flag-gated elsewhere. Static registration alone doesn't prove this is enabled for all tenants - confirm with eng before trusting this is GA (candidate for a new headline if it checks out, since Virtual MCP servers and Shadow apps both got cut). */}
- **c1aw agents are now reachable through the curated MCP tool catalog**: customer agents connected to a tenant's C1 MCP endpoint can discover c1aw agents and hold a turn-based conversation with one, and System owners, AI governance admins, and support agents can start, list, and message c1aw sessions the same way. {/* PR #24790, #24783 */}
{/* QUESTION: is this MCP-session-tooling item (24790/24783) something we want to walk through in more depth, or is a two-line summary enough? Flagging since it's fairly technical/API-shaped for a product digest. */}
- **MCP connections respect toolset grants, not just app membership.** A PER_USER MCP server now only shows up for users who actually have a grant on that server's toolset — being an app member or admin no longer implies you can authenticate to it. {/* PR #24574 */}
- **A unified home for MCP sources.** Registered MCP integrations are now managed as MCP sources throughout ConductorOne, with a single guided "Add MCP source" flow covering both catalog and external sources — connection, access, tool approval, and authentication setup in one place. {/* PR #25296 */}
- Enabling C1 Gateway and setting up MCP connections no longer requires accepting AI Access Management terms first. {/* PR #24830 */}
- C1 AI's responses are now more concise, with brief up-front plans for multi-step tasks and less routine narration while it works. {/* PR #24941 */}
- AI access requests now come with clearer recipient guidance and actionable errors when the target user or account is missing, cutting down on requests that failed from identity-field confusion. {/* PR #24944 */}
{/* REMOVED: AI agents authoring their own connectors end-to-end (PR #24771, #24877, #24925) - FEATURE_FLAG_ID_CONNECTOR_AUTHORING is SKU_MANUAL (tenant-SKU-gated, not broadly available) as of 2026-09-18. Re-add once it GAs. */}
{/* REMOVED: employee self-service AI budget viewing (PR #25258) - FEATURE_FLAG_ID_INFERENCE_PROVIDER is DEV_MANUAL as of 2026-09-18. Re-add once it GAs. */}

### Connector management

- **Adding a connector is now a guided flow**: create a new app, add a connector to an existing app, or link an IdP application — instead of one undifferentiated form. {/* PR #25052 */}
- **A data drop is easier to act on.** When a sync looks like it dropped data unexpectedly, the banner now suggests running a new sync (with a one-click **Sync now** button) since a data drop is often just a temporary API issue upstream. The message is shorter so the next step - confirm the change or try another sync - is easier to spot, and the same wording now shows up in the Slack notification too. {/* PR #25188, #25209 */}
- **Require a reason for connector configuration changes.** Admins can now require a justification whenever a connector's configuration changes, and that reason is audited alongside a secret-safe configuration history. {/* PR #25438 */}

### Access request and review improvements

- **Locked entitlements now request against the right policy.** Entitlements that are locked but don't have their own request policy previously defaulted to the App Owner policy regardless of what the UI showed; they now correctly use the application's request policy. {/* PR #25095 */}
- **Access search is more reliable while you're using it**: it keeps keyboard focus as you type, resets correctly when you change the search or target user, and request actions only appear once access status has actually finished loading (with a retry if it can't load). Filtered pages, including app-scoped ones, no longer lose their place when a page comes back empty, and each entitlement now shows its own expiration instead of borrowing one from another grant. {/* PR #24929, #24927 */}
- **Find your blocked request faster.** The "already has a pending task" banner on the request form now links each listed item straight to its open task. {/* PR #24824 */}
- **A denial always reads as a denial.** Manually denying an access request could previously still send the requester an email saying it was granted; denial emails now always correctly say denied. {/* PR #24743 */}
- **Reviewer denial reasons reach your ticketing system automatically.** Access-review revoke tasks now include the reviewer's denial justification in their description, so existing external ticket templates using `.Description` pick it up with no changes. Custom templates can also reference `ReviewTaskNumber`, `ReviewTaskURL`, and `DeniedBy` directly. {/* PR #25108 */}
- **Campaigns can now scope to pending invitations.** Access-review campaigns can filter on accounts with an outstanding invitation the principal hasn't accepted yet - previously these were silently dropped from any campaign whose scope restricted account status, with no error and no way to include them. {/* PR #25014 */}
- **Lopsided access-conflict groups are no longer a workaround.** Conflict monitors no longer require both entitlement groups to individually stay at or under 32 members - groups can now be lopsided (say, 90 and 11) as long as the total combination count stays at or under 1024, so a customer with one large group no longer has to split it across multiple monitors. {/* PR #24868 */}
- **A stuck campaign close no longer stays stuck.** Closing a campaign could hang when a completed review was left in a pending state; campaigns now reconcile that stale state during close, and any genuinely unresolved review returns a clear, recoverable result instead of leaving the campaign unclosable. {/* PR #24845 */}
- **Add a Resource description column to reviews.** Campaign owners can surface a resource's custom description (falling back to connector-synced metadata) as its own reviewer-visible column; existing campaigns are unaffected until it's turned on. {/* PR #24812 */}
- **Approval steps requiring the app owner now stay with the app owner.** Fixed a bug where a step requiring review by the app owner could get reassigned to system admins if owner-source grants hadn't been reconciled yet - the wrong people could end up approving sensitive access changes. {/* PR #25626 */}
{/* QUESTION: flagging this one specifically since it's a permission/approver-correctness bug (see skill's "who can do this" check) rather than a routine fix - want to confirm before publishing that the fix is complete and there's no remaining window where this can happen. */}
- **Task visibility now matches who can actually see it.** Task lists follow the same view permissions as task details - app, entitlement, or campaign ownership, or having merely participated in the past, no longer makes a task visible in a list on its own. {/* PR #25374 */}
{/* QUESTION: this PR number (#25374) is also cited by the source digest for the removed spend-remedy-status API endpoint under Breaking changes below - same number, two unrelated changes. Verify the real PR number for one of these against `gh pr view` before publishing. */}
- Adding a Slack or Teams channel to a campaign now shows the channel name immediately in Settings, no refresh needed. {/* PR #24799 */}
{/* REMOVED: entitlement request forms rendering in the Slack request/extend modals (PR #24671, flagged by eng for possible inclusion) - FEATURE_FLAG_ID_SLACK_REQUEST_FORMS is SKU_MANUAL as of 2026-09-18, not GA. The PR itself also notes it hasn't been verified against a live Slack workspace yet. Re-add once it GAs. */}
{/* TODO: the developer who flagged this told Melinda it would roll out "soon" - re-check FEATURE_FLAG_ID_SLACK_REQUEST_FORMS's live status in ductone/c1 pkg/builtin_tenant/features_2026.go on Monday (2026-09-21) before trusting that verbally; per the skill, a verbal "it's rolling out" is not a substitute for the grep - it's been wrong before. */}
- When adding scopes to an access profile, requests for already-enrolled users are now only created when "Create requests for currently enrolled users" is checked - matching how entitlements already behave. {/* PR #25665 */}
- Campaign progress now stays below 100% while any review is open and above 0% once work has started, with progress bars and summary charts rounding consistently between those endpoints. {/* PR #25346 */}
- Access reviews now reflect approve/deny decisions instantly and stay in sync more efficiently on large campaigns, with fewer redundant background requests - no workflow change for reviewers. {/* PR #21773 */}

### Automation improvements

- **Scope triggers by resource type, not just by app.** Grant Found and Grant Deleted triggers can now be scoped by resource type through the same faceted, per-app picker already used for revoke criteria - with real counts and disambiguation when two apps share a resource-type name. Grant-trigger cards now show an inclusion-criteria summary chip. {/* PR #25146 */}
- **Revoke criteria get the same faceted picker.** The Resource type filter on a revoke automation's criteria is now a real faceted picker with server-side search, counts, and disambiguated labels. {/* PR #25042 */}
- Fixed requestable automation approvals that could get stuck before expression-based approvers were evaluated. {/* PR #24990 */}
- Automation builders can now search available steps and trigger types by name or description instead of scanning a list. {/* PR #25690 */}
{/* QUESTION: this PR number (#25690) is also cited by the source digest for a bug fix (automations with a deprecated revoke-entitlements step crashing the step editor) - dropped that item under the skip-routine-bug-fixes call, but flagging the reused number here too in case it points at a mislabeled PR. */}

### Role mining

Role-mining filters now hold up better against messy real-world attribute data: high-cardinality custom attributes (like Squad or Tribe, with many distinct values) stay available with paged, searchable value selection instead of disappearing, and an attribute stays visible in the filter catalog even when it currently has no values, has more than 200, or hits a temporary value-preview failure. {/* PR #25500, #25495 */}

### Findings improvements

- **Findings can now be assigned to a specific person, automatically.** Add a Set assignee step to any Findings → Transformation rule, and every finding it matches is assigned to that person as it arrives; a person's own manual assignment always wins over the rule. {/* PR #25487 */}
- Findings can now also be assigned directly to an identity user (not just handled by role); assignees see their assigned findings and can update state according to their role. {/* PR #23470 */}

### Vaults and secrets

**My resources** brings together everything a user owns or has access to - groups, vaults, provisioned credentials (now called credential stores throughout the admin UI), and shared secrets - in one page, and now also includes Apps and Entitlements tabs. Secrets shared *with* you show up alongside your own. The standalone Groups and Secrets pages redirect here, so existing bookmarks and links still work. {/* PR #24681, #24680, #25117 */}

Separately, headless devices that join a user's vaults with a user-minted enrollment code are now staged and auto-approved, with no manual approval step - vault contents remain metadata-only server-side either way. {/* PR #24971 */}

### Usability improvements

- The Inventory page is now **Identities & NHI**, with dedicated Overview, Human users, Service, Secrets, NHI, and Agents tabs. {/* PR #24792 */}
{/* QUESTION: is this a rename within the nav overhaul already described in the pinned "Updated navigation" entry at the top of this page, or a separate change worth its own mention? Didn't want to re-explain the whole nav restructure if it's already covered there - confirm the relationship before publishing. */}
{/* QUESTION: PR #24648 (dedicated overview pages for AI/Governance/Identities/Security/Vaults/Platform, existing Groups/Secrets bookmarks redirect) also appeared in the source material for this window. It reads like it may be the same nav overhaul already published in the pinned "Updated navigation" entry at the top of this page rather than new content for this cycle - left out entirely rather than duplicate it, but flagging in case it's actually a distinct follow-on change that needs its own bullet. */}
- Groups now show their member count at the top of the group detail page, next to the description and the access profiles that use the group - previously the only way to size a group here was counting rows in the Members table, which didn't work once a group had more than one page of members. {/* PR #25412 */}
- Rendered C1 emails are now minified, cutting their size by roughly 55% so notifications stay under Gmail's 102 KB clipping limit instead of getting cut off partway through. {/* PR #25247 */}
{/* REMOVED: PR #25695 (command search reaches Shadow MCPs, Virtual MCP servers, password policies, unmanaged/shadow apps) - dropped entirely since 2 of its 4 destinations (Shadow MCPs, Virtual MCP servers) are behind non-GA flags and "unmanaged/shadow apps" is ambiguous re: the same gate; "password policies" alone wasn't worth a standalone bullet. Revisit once Virtual MCP servers / Shadow apps GA. */}
- Screen-reader support across every admin list page (Directory, Applications, Task log, and more) is now fully readable: status and action icons announce their label, row selection announces on the row itself, and column-resize handles read out their current width. Nothing changes visually. {/* PR #25646 */}
- Charts with many legend items (pie/donut and time-series) now show the legend below the chart instead of overlapping it, wrapping into as many rows as needed; clicking a legend item on a pie or donut chart focuses the chart on just that item instead of repainting it, and clicking again restores the full view. {/* PR #24915, #24878 */}
- Clearer table headers and coordinated pagination styling make lists easier to scan throughout ConductorOne. {/* PR #24870 */}
- Paginated dropdowns now load more matching options as you scroll instead of jumping back to the top. {/* PR #25457 */}
- Fixed local policy editing and switching from entitlement details, and policy dropdowns no longer appear above newly opened policy drawers. {/* PR #24823 */}
- Navigation highlighting now stays consistent with the pages a user can actually access, and Vaults redirects keep the right page context while loading. {/* PR #25132 */}
- **Policy comments now show you what they'll actually post.** A wait step's "Post this comment while waiting" / "Post this comment when timed out" and an approval step's "Post comment upon SLA violation" fields now hint that markdown is supported and include a live preview, and the policy detail summary renders the configured comment instead of printing raw markdown source (asterisks and all). {/* PR #23155 */}
{/* Note: added 2026-09-18 at the head of product's request despite predating this digest's Sep 4-18 window - this PR (merged 2026-08-12) has no "## Release Notes" section, which is almost certainly why no prior digest picked it up. No feature flag involved. */}

### API and developer tools

- **Manage AI governance through the API.** Classifiers, tool gates, classifier bindings, classifier templates, and the agent classifier policy can now all be managed through the public REST API. {/* PR #25540 */}
- **Terraform support for MCP sources.** Terraform users can manage hosted and external MCP sources - source details, authentication configuration, and tool enablement policy - as code. {/* PR #25273 */}
- AI integrations get clearer, runnable code-mode examples that preserve access-approval outcomes and recommend scoped queries and bounded batches. {/* PR #24974 */}
- API requests that fail because the platform is briefly over capacity now return HTTP 503 Service Unavailable with a Retry-After header instead of HTTP 500; rate-limit responses (429) are unchanged. {/* PR #25267 */}
{/* QUESTION: is the 500->503 change significant enough to also flag under Breaking changes, in case an integration specifically branches on 500? Leaning no (it's additive/more-correct, not a removal) but flagging since Session 2 should confirm. */}
- When governed AI spend is enabled with an approval policy configured, API clients can request allowance changes, check fulfillment status, and retrieve review details permitted by their role. {/* PR #24952 */}

### Breaking changes

- **Breaking change: a revoke criteria scoped only by a legacy resource-type name now fails closed.** Previously it would silently match every app that happened to share that resource-type name; it now fails that step's execution instead. Re-scope any revoke automation still relying on the old name-only matching. {/* PR #25042 */}
- **Breaking change: PaperSecret's Revoke API is now Delete.** Service SDKs rename Revoke to Delete. The public `DELETE /api/v1/secrets/{vault_id}` endpoint now deletes only the Paper Vault and no longer queues a provider-side credential revoke - use the new `POST /api/v1/app-secrets-admin/revoke` endpoint to queue that. {/* PR #25587 */}
- **Breaking change: the spend-remedy-status endpoint is gone.** `GET /api/v1/tasks/{task_id}/spend-remedy-status` has been removed; integrations must migrate to the task APIs. JS/TS integrations must also handle spend-control revision values as strings, to preserve precision. API clients that use a closed list of task action types need to handle a new type: `TYPE_SPEND_REMEDY = 7`. {/* PR #25374, #24952 */}
{/* QUESTION: same PR-number collision flagged above under Access request and review improvements - confirm which PR this breaking change actually shipped in before publishing. */}
...
Please note that Microsoft Teams as a C1 notification and interaction surface is not yet supported on the EU data residency instance. 
...
We've reorganized the C1 admin navigation to keep pace with everything we've shipped recently. Many features have new homes, but nothing about how each section works has changed.

Here's what's different:

- **Apps** — same as before, plus **Connectors**, which now lives here since it powers governance for each app.
- **AI** — unchanged: still your hub for the C1 MCP, MCP Gateway, MCP connections, and AI agents.
...
- **Admins can now review and approve MCP resources and URI templates**: When C1 discovers a server that exposes static resources or URI templates, these appear on a new **Resources** tab where an admin can approve them. Each approved resource gets its own read entitlement, so access to a resource is governed separately from tool and toolset access — a user needs an explicit grant to read it. See [Govern MCP resources](/product/admin/mcp-resources) for details.
...
- **External Slack accounts can no longer bind to a C1 user**: Slack accounts from outside your workspace — including Slack Connect members — can no longer be linked to a C1 user account by email match. Legitimate workspace members are unaffected; a C1 user whose only Slack match is external now has notifications skipped with a warning.
...
- **Bring governed tools into Copilot Studio**: Build a Copilot Studio agent that calls the tools your IT team approved through C1, with every call authorized and logged under the calling user's own identity, then publish it to Microsoft 365 Copilot and Teams for your whole organization. See [Connect Copilot Studio to C1](/product/admin/mcp-server/copilot-studio).

- **Bring governed tools into Gemini Enterprise**: Gemini Enterprise can now call the C1 MCP gateway as a tool source, with every tool call attributed to the person who made it instead of a shared service account. See [Connect Gemini Enterprise to C1](/product/admin/mcp-server/gemini-enterprise).
...
See [Ask the C1 AI assistant about access history](/product/admin/ai-assistant#ask-questions-and-take-action).
...
When a new C1 tenant is created, you'll now get guided setup for access profiles, access reviews, and automated access management in one flow — reducing time-to-value and eliminating the configuration overhead that slows down early adoption. See [Get a personalized onboarding plan](/product/admin/ai-assistant#get-a-personalized-onboarding-plan) for details.
...
- **Trend charts from C1AI**: C1AI can now generate trend charts (such as grants versus revocations, review decisions, and automation runs) directly from C1's aggregated metrics — fast, with exact numbers you can trace to their source, rather than hand-computed from raw records.
...
You can now assign a workflow step in a request or review policy to C1 AI. The agent evaluates each request that reaches the step using C1's built-in guidance plus any custom instructions you provide.
...
- **Requestable C1 groups**: [C1 groups](/product/admin/groups) can now be added to a request catalog or access profile, so users can request group membership through the same approval flow as other requestable access. 
...
- **Data lineage for agentic reports**: When the C1 AI assistant generates a report, you can now click **View lineage** to see the exact queries behind each number, confirm each source was verified against the execution log, and copy the lineage for audit workpapers. 

- **Multi-chart dashboards and composed reports**: Ask C1 AI for multiple charts and they now render together as an aligned grid, each with its own export and sources. C1 AI can also answer reporting questions with a composed report — stat cards, a trend chart, and a data table in one response, each showing the sources behind it. 
