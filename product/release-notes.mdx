...
### Entitlement owner routing in policy steps
...
- The C1 Slack assistant can now generate and share downloadable files instead of pasting data inline. When you ask for a CSV or structured report, the assistant returns a short-lived download link.

- Pausing a connector is now a button on the connector detail page, next to the connector logs button. When you disable all resource types on a connector, C1 now prompts you to pause the connector instead of saving an empty configuration.

### Fixes

- Saving changes in workflow trigger and step configuration drawers no longer shows an "Unsaved changes" confirmation dialog after a successful save.
