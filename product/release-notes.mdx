...
The new agent is in beta while we gather feedback and improve its functionality. Please let us know what you think!

### Campaign improvements

- Scheduled campaigns (docs)
...
- When you export your user list to CSV, the file now includes a UserID column containing each user's unique ConductorOne identifier.

- Retrieving users without specified criteria now returns an empty result set instead of an error.

### Access reviews

- Fixed an issue where the submissions drawer link wasn't appearing for completed reviews.

- Access review validate scope reports now include the manager's email address.

- Resolved performance issues with `GenerateValidateScopeReport` that were causing out-of-memory errors on long-running operations.

### Task and request management

- The bulk action for revocation tasks now consistently displays **Confirm** instead of **Approve** in the dropdown menu.

- Task CSV exports now include separate **Status** and **StateTimestamp** columns instead of a combined column.

- When reassigning a ticket, you can now select **Reassigned due to approver unresponsive** as a quick response option.

- Task statuses now accurately reflect policy wait conditions. For example, you'll see **Waiting for policy condition** instead of **Awaiting assignee** when appropriate.

- Task Export CSV now includes a **Request Reason** column.

- Action tasks now display resolved form data, and resource types appear in sentence case.

### Access requests

- The Request Access form now displays app descriptions in the app selection dropdown, helping users identify the right application.

- The **How long is the access needed?** field now includes a tooltip explaining that the maximum configured duration per entitlement still applies.

- Updated button labels, added a lightning bolt icon, and applied consistent coloring across the Actions tab.

### Policies

- You can now create policy conditions based on account type (service account, system account, or human user) using the `account` variable.

- Task recommendations now use more intuitive icons: thumbs down for Deny and alert circle for Needs Review.

### Connector management

You can now edit connector names after creation.

### API and admin

- The API Key Inventory page now displays a **Scope** column showing each key's permissions (Full Permissions or specific roles).

- Fixed pagination cursor logic to prevent duplicate sorting in MCP API object queries.

### Usability improvements

Filters are now capped at 200px maximum width and no longer wrap unexpectedly.
