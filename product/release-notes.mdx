...
### Create campaigns with the Campaign assistant
...
{/* REMOVED: "Policies can reference other policies" (PR #22677) - gated behind POLICY_REFERENCES_POLICY, which is FEATURE_FLAG_STATUS_SKU_MANUAL (off by default, enabled per tenant manually, not self-serve). Not broadly available, so it doesn't belong in a customer-facing "what's new" digest yet. Re-add once it's on a real SKU/rollout. Also: release notes should never name an internal feature flag - if this comes back, describe the capability and its availability without citing the flag. */}
