...
Three new automation steps are now available: **Generate password**, **Set credential**, and **Store credential**. Use them together to generate a secure password, apply it to an account in a self-hosted application, and securely deliver it to the account holder's manager via a one-time secret link.
...
You can now configure C1 to send emails through your own infrastructure. Supported providers include SendGrid, Microsoft Graph, AWS SES, and Google Workspace. Once configured, automation email steps can send to external recipients using CEL expressions, with per-send audit logs and delivery metrics visible on the settings page.

Configure your email provider under **Settings** > **Email provider**. See [Custom email provider](/product/admin/PLACEHOLDER) for setup instructions.

### Organization contacts

Admins can now manage organization contacts under **Settings** > **Organization**. The Contacts section supports three categories — Security, Billing, and Operations — each with up to 20 email addresses. Super Admins can edit all contact categories; other admins can view the configured lists.

See [Organization contacts](/product/admin/PLACEHOLDER) for details.

### Access profile improvements

The access profile experience has been updated with a clearer layout, an overview tab, and streamlined editing for self-service, membership automation, and joiner and leaver settings.

### Built-in DLP patterns for hooks

Hooks now support built-in data loss prevention (DLP) patterns, so you can add common data protection behaviors without writing a custom function. Available patterns include PII field redaction, credit card blocking, query scope limits, write authorization based on data classification, and sensitive file guarding. Each pattern has a typed, form-based configuration UI.

See [Hook DLP patterns](/product/admin/PLACEHOLDER) for details.

### Redesigned Add connector page
...
- Entitlement names in Slack and Microsoft Teams notifications are now more consistent. When multiple entitlements exist on the same resource, notifications show both the resource name and the specific entitlement — for example, "Engineering Team — admin" rather than just "Engineering Team". Generic labels like "member" and "access" are automatically suppressed.
...
- Findings state changes — snooze, suppress, accept risk, create task — now reflect immediately without requiring a manual page refresh.

- Copilot conversations no longer show permission verification details as an initial message. The conversation now shows "Verifying permissions..." while it loads.
