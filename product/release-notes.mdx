...
See [Cloud Infrastructure Access](/product/admin/cloud-infrastructure-access) for details.
...
Entitlement routing rules let you automatically apply request settings — request policy, emergency grant behavior, and maximum grant duration — to entitlements across an entire app, based on conditions you define. Rules are evaluated in priority order; the first match wins. Use the Basic builder for simple field-matching or write a CEL expression for full control. For cloud infrastructure apps, conditions can also match on the granted role and the scope it applies to, so you can route Azure entitlements without configuring each one by hand.
