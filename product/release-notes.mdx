...
Find every report you've saved on the new **Platform > Reporting** page. From there, for any report, you can:

- Refresh it with the latest data

- Adjust its parameters or edit it with the **Report assistant**

- Review the underlying function and sources behind its results

- Rename or delete it

{/* PR: no single PR for this - spans a whole epic (IGA-3829 foundational through at least IGA-4282/4338), none of which have a "## Release Notes" section, so nothing to cross-check this wording against. Sourced from Melinda's own draft + verified against the attached screenshot (report.png -> saved-reports-1.png) for exact UI labels ("Report assistant", "Refresh"). */}
{/* QUESTION: added 2026-09-21 - FEATURE_FLAG_ID_SAVED_REPORTS and its dependency FEATURE_FLAG_ID_C1AW_REPORT_PROGRAMS are both SKU_MANUAL as checked this same day. Added anyway on the developer's word (via Melinda) that it's rolling out now - note this is "rolling out now" (in progress), not "already rolled out" like the Slack request-forms case, so there's a real chance some tenants won't have it yet when this publishes. If support signals suggest that, re-check the flags before assuming the rollout finished. */}
