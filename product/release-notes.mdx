...
The campaign setup flow and reviewer task list have been refreshed with clearer labels and more compact controls. Staging a campaign is now a single **Stage campaign** step that includes scope validation, and reviewer actions in the task list use icon buttons with tooltips to keep more of each review row visible. To view your submissions, use the **⋯** (more actions) menu at the top of the reviews page. See [Create a campaign](/product/admin/campaigns) and [Complete access review tasks](/product/how-to/review-tasks) for updated documentation.
...
Admins can now configure organization-wide notification defaults and control whether users can customize their own preferences. Set the default digest frequency and notification types for your organization, then lock individual settings to enforce them across all users, or leave them unlocked so users can adjust to their own needs. See [Tenant notification settings](/product/admin/notifications-tenant-settings) and [Your notification settings](/product/admin/notifications-user-settings).
...
- **Generate and deliver passwords in automations.** Three new automation steps — **Generate password**, **Set credential**, and **Store credential** — let you generate a password, apply it to an account in a self-hosted application, and securely deliver it to the account holder's manager via a one-time secret link. See the [automation steps reference](/product/admin/automations-steps-reference) and the [new account provisioning example](/product/admin/automation-examples#new-account-provisioning-with-initial-password).
