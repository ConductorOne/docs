...
---
{/* Editor Refresh: 2026-03-13 */}

<Update label="March 13, 2026">

### Access reviews

Access review reports now include a **Last Login** column, showing when each user last accessed an application. This date appears in the exported campaign report, making it easier to identify inactive users during the review process. Reviewers can also add a Last Login column directly to the task list view using the column picker — with color-coded indicators (green/yellow/red) based on recency — without needing to export the report first.

The **Reviews by app** card on the campaign dashboard now displays approved, denied, approval rate, percent complete, and completion totals for each app in the review.

When defining a campaign's scope, a new **By criteria** option lets you target entitlements by risk level or compliance framework rather than selecting them individually. When used in a campaign template, the criteria are saved dynamically — each time you prepare a campaign from the template, ConductorOne re-evaluates which entitlements match, so newly tagged entitlements are automatically included without any manual template updates. A new filter bar in the entitlement picker also makes it easier to narrow results by application, resource type, risk level, and compliance framework when selecting specific resources.

### Usability improvements

- The **Access profiles** list now includes an **Automation** column that shows whether membership automation and **Initiate Access Changes** are enabled for each profile.

- Filter dropdowns no longer display a colon when no value is selected. For example, a **Creator** filter with no selection now shows "Creator" instead of "Creator:".

- Entitlements on resources with restricted visibility (members-only or owners-only) are now hidden from access profile and request catalog views for users who don't have access, consistent with existing behavior on entitlement list and search pages. Grant feed events for these resources are also now hidden for the same users.
