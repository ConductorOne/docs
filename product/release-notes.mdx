...
Access review campaigns can now be scoped by access conflicts. When creating a campaign, select the **Review access conflicts** scope type to generate review tasks for users with active separation of duties violations detected by your conflict monitors. This makes it easier to run targeted reviews specifically for SoD policy violations without manually identifying affected users.

### Usability improvements

- The **Connectors** page now has filtering options. Filter by type (**All**, **Cloud**, or **File**) to show or hide file/CSV connectors, or filter by owner to find connectors managed by a specific user.

- A new **Delegate** column on the **Users** list shows each user's delegate. Use the **Has delegate** filter to quickly find all users with delegation configured.

- An orange notification dot now appears next to **Downloads** in the sidebar when new downloads are ready. Click **Downloads** to view them and clear the notification.
