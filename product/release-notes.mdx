...
* **Campaign scoping** - Filters now return up to 50 items (increased from 25) when scoping a campaign, making it easier to find and select the resources you need.
* **Unstructured user access reviews** - The page now uses full width, providing more space to review access details.

### New SLA violation actions for approval steps

You can now configure additional actions when an approval step SLA is violated, giving you more control over how your access request workflows handle time-sensitive approvals.

Two new actions are available:

* **Cancel ticket** - Automatically cancel the ticket when the SLA timer expires. Use this when you want requests to expire and be rejected if not acted upon within the configured time period.
* **Skip step** - Automatically skip the current approval step and proceed to the next step in the policy when the SLA timer expires. Use this when you want requests to continue progressing even if a specific approver doesn't respond in time.

These options complement the existing notification action, allowing you to design approval workflows that match your organization's requirements for timely decision-making.
