...
### Make sure revoked access actually goes away

Revoking an inherited entitlement — one a user has because of a group or role, not because it was granted to them directly — used to only remove the entitlement itself. The access that produced it stayed in place, so the next sync brought it right back, and nobody looking at the review could tell the revoke hadn't actually stuck.

Turn on **Revoke inherited access** (under **Entitlement management** on an app's details page) and revoking an inherited entitlement now revokes the upstream grants it came from too, so the access is actually gone. It's off by default and set per app; when it's on, approvers see a notice listing what else a revoke will affect before they confirm.
...
### Keep one-off policies out of the shared library

Every policy used to live in the global policy library, so a rule meant for just one app or entitlement still had to be added there — cluttering the library with one-offs that don't apply anywhere else.

Policies can now be created local to a single app or entitlement instead. Each policy configuration slot offers the object's local policy alongside the global library, with inline creation, so narrow, one-off rules stay out of everyone else's way. Local policies are still managed by policy administrators.
...
### Automate managing the apps your connectors discover

Bringing a discovered app under management — or just checking whether one already is — meant clicking through the UI app by app, which doesn't scale if you're managing hundreds of SSO-connected apps or want this wired into your own provisioning tooling.

The public API can now list connector-discovered applications, inspect their managed state, and promote an unmanaged application to managed. Promoting an app that's already managed returns a 409 instead of erroring silently, and you can read back the binding to recover the resulting managed application ID.

See [Move an unmanaged app to managed](/product/admin/applications#move-an-unmanaged-app-to-managed) for the UI equivalent.
...
### See exactly what every push rule did — and why one failed

Push rules ran with no record of what happened. If a push failed partway through, there was no way to see what was pushed, when, or why the target system rejected it — just try again and hope.

Every attribute push now shows up in execution history. Go to **Directory sources → Executions** to see pushes across all connectors, or open a push rule's row menu and choose **Show execution history** to see just that connector's. Each entry records who was pushed, when, which attributes changed, and — on failure — the reason the target system gave for rejecting it.
...
## Product digest: August 14, 2026

### See reviewer context without leaving the table

Deciding whether someone still needs access often comes down to context — their job title, department, cost center — but that information only lived in a hover tooltip, so reviewers had to mouse over every row to get it.

Campaign admins can now show any reviewer-visible app user attribute as its own column in the access review table — up to 10 per campaign — so reviewers can scan the context they need instead of hunting for it row by row.
...
An automation's revoke step always removed both a user's direct grants and anything they had through group membership — so automating cleanup of direct access meant accepting collateral revocation of group-inherited access too, or not automating it at all.

Revoke steps now have a **Grant source** setting (**All** or **Direct**), so you can scope an automation to touch only direct grants and leave group-inherited access untouched.
