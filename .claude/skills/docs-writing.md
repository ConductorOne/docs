...
| What | Examples |
| :--- | :--- |
| Domains and URLs | `conductorone.com`, `conductor.one`, tenant URLs like `example.conductor.one` |
| Email addresses | `support@conductorone.com` |
...
When in doubt: if a user would type it into a terminal, browser, or config file, leave it as-is.
