...
Baton-HTTP is a configuration-driven connector that lets you write YAML instead of Go code. Instead of implementing the `ResourceSyncer` interface, you describe how to map an API to C1.ai's resource model. Your ops team can own integrations directly — no engineering queue.
...
| `--client-id` | C1.ai client ID for service mode |
| `--client-secret` | C1.ai client secret for service mode |
...
Resource types define the entities you want to sync. Each resource type specifies how to list resources and map API responses to C1.ai resources.
...
### Step 1: Set up a new connector in C1.ai
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-http
        image: public.ecr.aws/conductorone/baton-http:latest
        args:
