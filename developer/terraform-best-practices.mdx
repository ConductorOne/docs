...
Key fields:
- **enrollment_behavior**: `REQUEST_CATALOG_ENROLLMENT_BEHAVIOR_BYPASS_ENTITLEMENT_REQUEST_POLICY` bypasses individual entitlement approval policies during enrollment, so users get all entitlements in a single request.
- **unenrollment_behavior**: `REQUEST_CATALOG_UNENROLLMENT_BEHAVIOR_REVOKE_ALL` revokes all entitlements when a user unenrolls.
- **unenrollment_entitlement_behavior**: `REQUEST_CATALOG_UNENROLLMENT_ENTITLEMENT_BEHAVIOR_BYPASS` bypasses approval policies when revoking entitlements at unenrollment.
...
resource "conductorone_app_entitlement" "access_profile_policy" {
  ...
  grant_policy_id  = var.grant_policy_id
  revoke_policy_id = var.revoke_policy_id
  duration_grant   = "600s"
}
```
