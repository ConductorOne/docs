...
## Each trait requires its own resource builder

Resources with traits (USER, GROUP, ROLE, APP) must use the specific builder function, not generic `NewResource`.
...
```go
// Using generic NewResource for an APP trait resource
...
**Why:** Each trait has associated data (email for users, profile for apps, etc.). The specific builders add the trait annotation that the SDK validates during sync.

## Use uhttp instead of raw http.Client

The SDK provides `uhttp` package with automatic retries, rate limiting, and logging. Don't use raw `http.Client`.
...
resp, err := httpClient.Do(req)  // No retry, no rate limit handling
```
...
**Why:** uhttp provides:
- Automatic retries on transient failures (429, 503)
- Rate limit header parsing (X-RateLimit-*, Retry-After)
- Exponential backoff
- Request/response logging for debugging

**For insecure TLS (testing only):**
...
## JSON API responses with dynamic fields need type assertions

Some APIs return dynamic JSON structures that Go unmarshals as `any` (interface{}). You must use type assertions to access nested fields.

**Example: API returns flexible metadata**

```go
// API response with dynamic fields
type Resource struct {
    ID       string `json:"id"`
    Metadata any    `json:"metadata"`  // Structure varies by resource type
}

// What metadata might contain after JSON unmarshal
// map[string]any{"custom_field": "value", "nested": map[string]any{...}}
```

**Wrong:**

```go
// Compiler error: any has no field or method
value := resource.Metadata.custom_field
```

**Correct:**

```go
func getMetadataField(metadata any, key string) string {
    if metadata == nil {
        return ""
    }
...
**Why:** APIs use dynamic fields for:
- Custom metadata that varies by resource
- Forward compatibility (new fields don't break old clients)
- Flexible schemas where structure isn't fixed

## Resource IDs must contain enough data for grants queries

When designing resource IDs, consider what your grants query needs.

**The problem:**

```yaml
# Resource uses just the name as ID
fields:
  id: name  # "my-repo"
...
1. **Composite ID:** Use `owner/name` as ID, parse when needed
2. **Store in profile:** Add owner to profile, reference as needed
3. **Different query:** Find a query that works with what you have

**Example with composite ID:**
