...
The Baton SDK handles orchestration, output format, pagination coordination, and communication with C1.ai. You focus on translating your system's API into the Resource/Entitlement/Grant model.
...
Traits tell C1.ai how to interpret the resource. Use `TRAIT_USER` for people, `TRAIT_GROUP` for collections, `TRAIT_ROLE` for permission bundles.
...
**Why this matters:** C1.ai uses the `RawId` to:
...
How you structure resources and entitlements determines what C1.ai can manage.
...
- **You can run without production C1.ai credentials** (local testing story exists)
