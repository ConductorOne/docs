...
Baton solves this by normalizing every system into a consistent shape so C1 can ask one question across all systems: who has access to what? Without this normalization, each system would be an island - auditors would see chaos, and access reviews would require expert knowledge of every platform.
...
Your connector produces an access graph that powers access reviews, certification campaigns, provisioning workflows, and compliance reporting. This single data structure drives everything C1 does. The graph has three main node/edge types:
...
Consider GitHub: organizations contain repositories, repositories have branches. Or AWS: accounts contain services, services have resources. When your connector models these hierarchies, C1 can:
...
Every resource has a **resource type** (string id) and can declare **traits** that tell C1 how to interpret it. Traits let C1 understand *what kind of thing* a resource is, even when different systems call it different names.
...
Traits are optional for custom resource types, but they unlock powerful features. When you mark a resource with `TRAIT_USER`, C1 knows it can correlate that resource with users from other systems, display it in user-centric views, and apply user-specific policies.
...
Entitlements have a `purpose` field that tells C1 how to interpret them:
...
When a connector runs, data flows through several stages. The clean separation between what you control and what C1 controls makes the system reliable and testable:
...
4. **Ingest** - C1's sync service reads the .c1z file
...
**What C1 controls:** Steps 4-5. The sync service and uplift process.

### ID correlation

C1 needs to know whether a resource in this sync is the same resource from a previous sync. This is where the `RawId` annotation matters.
