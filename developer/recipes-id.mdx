...
**Why this matters:** C1.ai uses these IDs to correlate resources across syncs. Using the wrong ID causes duplicate objects or failed correlations.
...
**Problem:** Ensure C1.ai can correlate your resources across syncs and match resources created via Terraform.

**What is RawId?** A string annotation containing the external system's native identifier. C1.ai uses this to match resources across syncs and to merge Terraform-created objects with connector-discovered ones.
