---
title: "C1.ai API integration reference"
sidebarTitle: "C1.ai API"
description: "How connectors communicate with the C1.ai platform in daemon mode."
---
...
This document describes the C1.ai APIs that connectors use internally. You don't call these APIs directly; the SDK manages them.

## Communication architecture

```mermaid
flowchart LR
    subgraph C1["C1 platform"]
...
**Connector produces:** c1z file uploaded to C1.ai
...
**Returns to C1.ai:** Created account, credentials (encrypted by SDK)
...
**Returns to C1.ai:** New credentials (encrypted by SDK)
...
C1.ai can adjust heartbeat interval per-task. If heartbeats stop, the task may be reassigned to another connector instance.
...
The SDK handles:
- Token acquisition from C1.ai's OAuth endpoint
- Token refresh before expiration
- Token injection into request metadata

### Host identification

Connectors identify themselves to C1.ai:
...
This helps C1.ai track which host is running which connector instance.
...
| Connector Error | SDK Handling | C1.ai Action |
|-----------------|--------------|---------------------|
...
| Cancelled by C1.ai | Stop processing | Task cancelled |
...
| `connection refused` | Network/firewall issue | Check connectivity to C1.ai |
