...
RawId annotation, external_id, and how C1.ai matches resources across syncs.

---

## Why ID Correlation Matters

C1.ai needs to know if a resource in this sync is the same resource from a previous sync. This enables:
...
**Omitting RawId:** Without it, C1.ai can't correlate resources across syncs.

## Pre-sync Reservation

The `match_baton_id` field (in Terraform/API) allows creating C1.ai objects before the connector discovers them. When the connector syncs, resources are matched by this ID.
