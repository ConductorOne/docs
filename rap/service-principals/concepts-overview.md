...
- Assigned C1.ai roles (same as human users)
...
|--------|-------------------|---------------------|
| How | Client ID + secret to token endpoint | External OIDC token exchanged for C1.ai token |
...
- Feature must be enabled (contact C1.ai account team during early access)
