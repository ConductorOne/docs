...
<Note>
The steps below generate a **client secret** for the app registration. If you plan to use the connector's **Certificate** tab instead (recommended for FedRAMP and other high-compliance environments where client secrets aren't permitted), you'll upload a certificate to the same app registration rather than creating a client secret. Follow Microsoft's guidance on adding a certificate credential to an app registration, then have the PEM-encoded certificate and its private key ready for the Certificate tab further down this page.
</Note>
...
**Optional.** Click to enable **Skip enterprise applications without assignments** to exclude enterprise applications that have no app role assignments from the sync. This can substantially reduce sync output in tenants that have accumulated many auto-registered service principals.
   ...
   <Warning>Toggling this setting on an existing connection will cause C1 to add or remove previously-synced enterprise applications and their associated entitlements and grants on the next sync to reflect the new filter.</Warning>
</Step>
<Step>
Click **Save**.
</Step>
