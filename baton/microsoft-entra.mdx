...
**Follow these instructions to use a built-in, no-code connector hosted by ConductorOne using certificate-based authentication.**
...
<Step>
On the **ConductorOne Integration** page, click **Permissions**.
</Step>
<Step>
...
<Step>
Check that the connector data uploaded correctly. In ConductorOne, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Entra ID connector to. Entra ID data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
**That's it!** Your Entra ID connector is now pulling access data into ConductorOne.
</Tab>
...
All users (both cloud-native and on-premises synced) are always synced to ConductorOne regardless of this setting. This setting only affects groups and group memberships.
