...
description: "C1.ai provides identity governance and just-in-time provisioning for OpenAI. Integrate your OpenAI instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance and just-in-time provisioning for OpenAI. Integrate your OpenAI instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
sidebarTitle: "OpenAI"
---
...
<Note>
**Archived projects are not synced.** The connector only syncs active (non-archived) projects. Service accounts, groups, and role assignments that exist only under an archived project will not appear in C1.ai.
</Note>

<Note>
**Account provisioning requires a payment method on the organization.** Inviting a user fails with a `payment_method_required` error if the OpenAI organization has no payment method on file. Add a payment method to the organization before using account provisioning.
</Note>

<Note>
**Service accounts cannot be created or deleted through the connector.** They must be created and deleted directly in OpenAI. The connector only syncs existing service accounts.
</Note>
...
<Note>
**Predefined organization roles on service accounts cannot be changed via the API.** The OpenAI API's user role update endpoint only accepts human user IDs. Attempting to revoke (downgrade) the `owner` predefined org role from a service account will fail with an error. Custom org roles on service accounts can still be revoked normally. Manage predefined org role assignments for service accounts directly in the OpenAI dashboard.
</Note>

<Note>
**The `reader` organization role cannot be revoked from any principal.** `reader` is the minimum required org role in OpenAI — every organization member must have at least this role. Revoking a `reader` grant will return an error. To remove all access for a user or service account, delete the principal from the organization entirely.
</Note>
...
<Note>
**Invitation counts fluctuate between syncs.** Pending and disabled invitations naturally come and go as users accept, expire, or are revoked. C1.ai skips sync anomaly detection on the Invitation resource type to avoid false-positive alerts from these expected changes.
</Note>
...
The C1.ai OpenAI connector uses the Admin API to manage project memberships. When you configure C1.ai to provision access to OpenAI projects, C1.ai handles these API calls for you, filling the gap that SCIM leaves for project-level access.

### What SCIM provides vs. what requires the Admin API

| Capability | SCIM | Admin API (via C1.ai) |
| :--- | :--- | :--- |
...
<Note>
Neither SCIM nor the Admin API currently supports automatically mapping IdP groups to OpenAI projects. To assign users to projects, use C1.ai's provisioning capabilities with the OpenAI connector, which calls the Admin API on your behalf.
</Note>
...
**Done.** Your OpenAI connector is now pulling access data into C1.ai.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-openai
        image: public.ecr.aws/conductorone/baton-openai:latest
...
<Steps>
  ...
  <Step>
  Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the OpenAI connector to. OpenAI data should be found on the **Entitlements** and **Accounts** tabs.
  </Step>
</Steps>
