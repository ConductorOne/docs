...
<Note>
**Archived projects are not synced.** The connector only syncs active (non-archived) projects. Service accounts, groups, and role assignments that exist only under an archived project will not appear in C1.
</Note>
...
<Note>
**Invitation counts fluctuate between syncs.** Pending and disabled invitations naturally come and go as users accept, expire, or are revoked. C1 skips sync anomaly detection on the Invitation resource type to avoid false-positive alerts from these expected changes.
</Note>
