---
title: "baton-scim: SCIM integration"
description: "Configure a custom connector to sync users, groups, and roles from SCIM-enabled applications."
og:title: "baton-scim: SCIM integration"
og:description: "Configure a custom connector to sync users, groups, and roles from SCIM-enabled applications."
sidebarTitle: "SCIM authoring"
---
...
Providing your C1.ai tenant client ID and client secret via flags automatically triggers Continuous Service Mode. This mode is recommended for production deployments.
...
```bash
# With a built-in service provider
docker run --rm -v $(pwd):/out -e BATON_TOKEN=your-oauth-token -e BATON_SERVICE_PROVIDER=slack public.ecr.aws/conductorone/baton-scim:latest -f "/out/sync.c1z"

# With a custom configuration
docker run --rm -v $(pwd):/out -v $(pwd)/config.yaml:/config.yaml -e BATON_API_KEY=your-api-key -e BATON_SCIM_CONFIG=/config.yaml public.ecr.aws/conductorone/baton-scim:latest -f "/out/sync.c1z"
```
