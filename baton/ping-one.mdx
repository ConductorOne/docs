...
og:title: Set up PingOne connector - C1 docs
og:description: Integrate your PingOne instance with C1 to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1 provides identity governance and just-in-time provisioning for PingOne. Integrate your PingOne instance with C1 to run user access reviews (UARs) and enable just-in-time access requests.
...
<Note>
This page covers the PingOne **connector**, which syncs and provisions access data for identity governance. It does not control how you sign in to C1. To set up signing in to C1 with your PingOne account, see [Authenticate with PingOne](/product/how-to/qs-set-up-c1#authenticate-with-pingone).
</Note>
