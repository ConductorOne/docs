...
og:title: Set up PingOne connector - C1.ai docs
og:description: Integrate your PingOne instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1.ai provides identity governance and just-in-time provisioning for PingOne. Integrate your PingOne instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests.
...
<Note>
This page covers the PingOne **connector**, which syncs and provisions access data for identity governance. It does not control how you sign in to C1.ai. To set up signing in to C1.ai with your PingOne account, see [Authenticate with PingOne](/product/how-to/qs-set-up-c1#authenticate-with-pingone).
</Note>
...
**Done.** Your PingOne connector is now pulling access data into C1.ai.
</Tab>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-ping-one
        image: public.ecr.aws/conductorone/baton-ping-one:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the ping-one connector to. ping-one data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
**Done.** Your PingOne connector is now pulling access data into C1.ai.
