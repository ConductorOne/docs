...
<Step>
**Disable custom roles sync** is checked by default, since legacy Workato roles are being retired. Uncheck it only if your workspace is still on the legacy roles model and you want C1 to sync legacy custom roles.
</Step>
<Step>
Click **Save**.
</Step>
...
stringData:
  ...
  # Defaults to true (legacy custom role sync is skipped by default, since legacy
  # Workato roles are being retired). Set to false only if your workspace is still on
  # the legacy roles model and you want C1 to sync legacy custom roles.
...
2. In the connector configuration:
   - Confirm **Disable custom roles sync** is enabled — it is on by default; enable it now if your connector was configured before this default changed and the box is still unchecked — since the legacy `/api/roles` endpoint no longer serves custom roles.
