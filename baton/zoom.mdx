...
When a resource type is excluded from the connector's sync, C1 does not emit grants that target it.
...
The Zoom connector supports account deprovisioning. C1's Delete user task calls Zoom with `action=delete`, which permanently removes the user and their meetings, webinars, cloud recordings, and PMI. That is stricter than Zoom's API default of `disassociate`, which only unlinks the user from the account. To disassociate instead of delete, or to transfer ownership first, use the `transfer_and_delete_user` connector action.
...
To run this action, give the Zoom app the `user:delete:user:admin` scope. When you set `transfer_email`, C1 confirms that the recipient exists before removing the user, which requires `user:read:user:admin` — a scope the connector already needs to sync accounts. The transfer options do not require any additional scopes.
