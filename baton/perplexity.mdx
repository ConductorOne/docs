...
description: "C1.ai provides identity governance and just-in-time provisioning for Perplexity. Integrate your Perplexity Enterprise instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Perplexity. Integrate your Perplexity Enterprise instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
When provisioning a new Perplexity user account, C1.ai prompts for the following field:
...
The user's email address is provided automatically by C1.ai and is used as the SCIM username. No password is required — Perplexity Enterprise authenticates users through SSO, so accounts are created without credentials.
...
In C1.ai, these IdP-pushed groups appear as the syncable, grantable resources. Granting a Perplexity user the "member" entitlement on the Admin group makes them a Perplexity Admin; revoking that entitlement (or moving them to the Member group) downgrades them to a Member.
...
| Deprovision (deactivate) user | <Icon icon="square-check" iconType="solid" color="#c937ae"/> |
...
<Steps>
  ...
  <Step>
    In your identity provider, create two groups — one to map to the Perplexity **Admin** role and one to map to the **Member** role — and assign users to the appropriate group. These groups will be pushed to Perplexity via SCIM and will appear as syncable resources in C1.ai.
  </Step>
...
{/* AUTO-GENERATED:END - config-params */}
  ...
  **Done.** Your Perplexity connector is now pulling access data into C1.ai.
  ...
  * [GitHub repository](https://github.com/ConductorOne/baton-perplexity): Access the source code, report issues, or contribute to the project.
  ...
  <Steps>
    ...
    <Step>
      Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Perplexity connector to. Perplexity data should be found on the **Entitlements** and **Accounts** tabs.
    </Step>
  </Steps>
