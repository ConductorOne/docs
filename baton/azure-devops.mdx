...
description: "C1.ai provides identity governance and just-in-time provisioning for Microsoft Azure DevOps. Integrate your Azure DevOps instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Microsoft Azure DevOps. Integrate your Azure DevOps instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
center, through another C1.ai connector that integrates with Entra, or by another
...
To have C1.ai manage Entra group membership through this connector, enable
...
Enabling this option also enables provisioning: C1.ai can grant and revoke project role assignments, adding or removing members from the underlying Azure DevOps security group.
...
<Steps>
  ...
  <Step>
  Enter whichever redirect URI matches your C1.ai tenant's domain as a **Web** redirect URI:
  ...
  **Optional — only required for Entra group membership provisioning.**
  ...
  If you want C1.ai to grant and revoke memberships on Entra-sourced Azure DevOps
...
call. If you need C1.ai to actually grant or revoke memberships on Entra-sourced
...
<Steps>
  ...
  <Step>
  Give the new token a name, such as "C1.ai", then select the relevant organization and set an expiration date.
  </Step>
  <Step>
...
<Steps>
  ...
  Select your method of authenticating to Azure DevOps.
    ...
    9. Log in and authorize C1.ai with your Azure DevOps instance.
    10. You will then be redirected back to the Azure DevOps setup page in C1.ai, where you'll see an authorization message.
...
**Done.** Your Azure DevOps connector is now pulling access data into C1.ai.
...
<Steps>
  ...
  <Step>
  Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Azure DevOps connector to. Azure DevOps data should be found on the **Entitlements** and **Accounts** tabs.
  </Step>
</Steps>
