...
<Steps>
  ...
  Select your method of authenticating to Azure DevOps.
    ...
    5. **Optional.** Check **Use project RBAC roles** to enable the Sparse ACL model for project access. Project-scoped groups are also synced as structured Project Role Assignment resources, with role and project context visible on each entitlement. See [Project RBAC](#project-rbac-opt-in) for details.
    6. Click **Save**.
    7. Click **Login with OAuth**.
    8. Log in and authorize C1 with your Azure DevOps instance.
    9. You will then be redirected back to the Azure DevOps setup page in C1, where you'll see an authorization message.
    ...
    5. **Optional.** Check **Use project RBAC roles** to enable the Sparse ACL model for project access. See [Project RBAC](#project-rbac-opt-in) for details.
    6. Click **Save**.
...
stringData:
  ...
  BATON_ORGANIZATION_URL: <https://dev.azure.com/{Your_Organization}>
  ...
  # Optional: enable the Sparse ACL model for project access
...
stringData:
  ...
  BATON_ORGANIZATION_URL: <https://dev.azure.com/{Your_Organization}>
  BATON_CLIENT_SECRET_TENANT_ID: <Entra tenant ID>
  BATON_CLIENT_SECRET_CLIENT_ID: <App registration client ID>
  BATON_CLIENT_SECRET_CLIENT_SECRET: <App registration client secret>
  BATON_PROVISIONING: "true"
  BATON_SYNC_ORGANIZATION: "true"
  BATON_ENABLE_INCREMENTAL_SYNC: "true"
  # Optional: requires GroupMember.ReadWrite.All Graph permission with admin consent
