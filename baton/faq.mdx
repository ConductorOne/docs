...
We’ll also automatically pause a connector’s sync cycle and alert you if significant data anomalies are detected (such as if the number of synced resources decreases by half between one sync and the next, or if a large number of grants are removed — for example, after narrowing a connector’s setup fields to sync a smaller scope than it previously synced). This setting is on by default, but you can configure it by navigating to an app’s **Configure connector** > **Advanced** tab. See [Connector sync pause](/baton/configure#connector-sync-pause) for details.
...
<AccordionGroup>
    ...
    - **Cloud-hosted connectors** are the built-in, no-code connectors hosted directly in the C1 tenant and provided via our SaaS service. They are configured on the **Connectors** page of C1.
