...
## Self-hosted connector network architecture

The following diagram shows how a self-hosted connector runs inside your network, how it reaches the systems it syncs, and how it communicates with C1.ai over an outbound-only connection.

<Frame>
<img src="/images/baton/assets/self-hosted-connectors-arch.png" alt="Reference architecture diagram of self-hosted connector network topology. On the left, a customer network trust boundary contains a Baton connector (running as a Docker container, Kubernetes deployment, systemd unit, or Windows service, optionally supervised by baton-runner) alongside a customer secret store (env vars, Kubernetes Secret, AWS Secrets Manager, or HashiCorp Vault) and the target systems it syncs, such as Active Directory/LDAP, SQL Server or PostgreSQL, Jira/Confluence/Bitbucket, and internal HTTP APIs. The connector reaches target systems over private network protocols like LDAPS, TDS, and HTTPS, and writes an ephemeral sync.c1z file and debug log to a local working directory. On the right, the C1.ai cloud (multi-tenant SaaS) exposes an OAuth 2.0 token endpoint and a connector API/task queue. The only connection crossing the corporate firewall is outbound HTTPS on TCP 443 using TLS 1.2+ and gRPC to *.conductorone.com — no inbound ports, VPN, or DMZ host are required, and it works behind NAT and egress proxies. This is a pull model: the connector always initiates, calling Hello (registers host ID, version, capabilities), GetTask/GetTasks (long-polls for work), Heartbeat (keepalive during a task), UploadAsset (streams sync.c1z in 512KB chunks), and FinishTask (reports success or failure); the task assignment response rides the same outbound connection rather than a new inbound one. Task types include full sync, grant/revoke entitlement, create account, delete resource, rotate credentials, issue credential, create/get ticket, event feed, and debug. Target-system credentials are never transmitted to C1.ai. For high availability, multiple connector replicas share one client ID and C1.ai distributes tasks round-robin. A separate diagram section shows the alternative periodic upload mode: a baton-<app> CLI run on a schedule (such as cron) produces a sync.c1z file with no persistent connection, which is either manually uploaded in the C1.ai UI or placed in a customer-owned S3 bucket that C1.ai polls hourly." />
</Frame>
...
- The technology or application does not have a line of sight to C1.ai’s platform and it would be inappropriate to expose that app to the internet.
- You can’t or don’t want to provide C1.ai with the API keys or credentials for the technology.
...
Integrating your self-hosted connector with C1.ai creates the most seamless and fully automated method of uploading your application’s data. Once the self-hosted connector is set up, the connector’s Baton service runs in your environment. The service maintains contact with C1.ai, syncs and uploads data at regular intervals, and passes that data to the C1.ai UI, where you and your colleagues can use it to run access reviews and facilitate access requests for the application.
...
<Steps>
    ...
    <Step>
    Choose where to add the connector: **Create a new app**, or **Add to an existing app** (then select the app of your choice).
    ...
    <Step>
    Set the connector's **Name** — for example, `<APP>` — and, optionally, a **Description**.
    </Step>
    <Step>
    Click the pencil icon next to **Owners** to choose who can configure and manage this connector. You can change owners later, if necessary.
    </Step>
    <Step>
...
As an alternative to running a self-hosted connector in service mode, you can set up a self-hosted connector and periodically upload its data to C1.ai, either manually or automatically.
...
Now that you’ve collected the connector data, you can choose to manually upload the data to C1.ai or to sync the data to an S3 bucket integrated with C1.ai. The next step walks through each option.

### Step 3: Upload data to C1.ai
...
Manually uploading data to C1.ai from an self-hosted connector is ideal when testing data ingested from the connector before automating the data upload process, or for times when you only need a single data sync.

<Steps>
    <Step>
    In C1.ai, navigate to an existing application you wish to add the connector data to, or create a new application.
...
Once the upload is complete, C1.ai adds the information pulled from the connector about accounts, groups, roles, resources, and grants (as relevant) to the application.

To update the information in C1.ai, re-run the `resources` command and re-upload the file to C1.ai using the process above.

### Option 2: Automatically sync connector data to an S3 bucket

The most convenient option is to automate the process of running of the sync and ingestion of the data into C1.ai. You can automatically pull connector data into an application by using an AWS S3 bucket as a data source.
...
<Steps>
	...
	This script syncs data from the connector (in this case, Okta) and uploads it to your configured S3 bucket. Once the upload is complete, C1.ai adds the information pulled from the connector about accounts, groups, roles, resources, and grants (as relevant) to the application.
    </Step>
</Steps>

You can run the script on demand, or set up a scheduler to run it periodically. The S3 bucket syncs with C1.ai once an hour.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-yourservice
        image: public.ecr.aws/conductorone/baton-yourservice:latest
...
For high availability, increase replicas. Multiple instances can share the same client ID and secret; C1.ai distributes tasks round-robin.
...
- Connector initiates connections to C1.ai
- No inbound ports required
- Works behind NAT and most firewalls

If you have egress filtering, allow HTTPS to whichever domain matches your C1.ai tenant's hosting region — default instance: `*.conductor.one`, EU data residency instance: `*.c1eu.ai`.
...
C1.ai sends email alerts after three consecutive sync failures. Monitor:
- Sync status in the C1.ai UI
...
- **`sync.c1z`** — the sync data file uploaded to C1.ai
