---
title: "Set up an Amazon EKS connector"
description: "C1.ai provides identity governance for Elastic Kubernetes Service (EKS). Integrate your EKS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:title: "Set up an Amazon Elastic Kubernetes Service connector"
og:description: "C1.ai provides identity governance for Elastic Kubernetes Service (EKS). Integrate your EKS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
<Steps>
    ...
    <Step>
    A list of the clusters in the current region is shown. Make a note of the name of the cluster you want to integrate with C1.ai.
    </Step>
</Steps>
...
**Done.** Your EKS connector is now pulling access data into C1.ai.
</Tab>
...
stringData:
  ...
  BATON_ROLE_ARN: <ARN of the IAM role to assume>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-eks
        image: public.ecr.aws/conductorone/baton-eks:latest
...
<Steps>
    ...
    <Step>
    Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the EKS connector to. EKS data should be found on the **Entitlements** and **Accounts** tabs.
    </Step>
</Steps>
