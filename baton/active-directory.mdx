...
- An Active Directory service account
	...
	- For access provisioning, the account needs permissions to modify group membership - see [below](https://www.conductorone.com/docs/baton/active-directory#entitlement-provisioning-permissions)
	- ‘Log on as a service’ permission. Note that you may be required to grant this via local or domain group policy depending on your local environment.
	- Write access to the C:\ProgramData\ConductorOne directory 
