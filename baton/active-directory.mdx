...
description: "C1.ai provides identity governance and just-in-time provisioning for Active Directory. Integrate your on-prem Active Directory domains with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Active Directory. Integrate your on-prem Active Directory domains with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
sidebarTitle: "Connector setup"
---

## Availability

The Active Directory connector is self-hosted and runs on a **Windows Server** joined to (or trusted by) the target Active Directory domain. Windows is the only supported platform.
...
The Active Directory connector supports [automatic account provisioning and deprovisioning](/product/admin/account-provisioning). When a new account is created by C1.ai, the account's password is sent to a [vault](/product/admin/vaults).
...
- Two connection modes: **WinLDAP** (default; uses `wldap32.dll` for Kerberos/GSSAPI) and **LDAP** (go-ldap library, e.g. for Simple Bind).
...
Connector actions are custom capabilities that extend C1.ai automations with app-specific operations. You can use connector actions in the [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) automation step.
...
- [C1.ai download center](https://dist.conductorone.com/ConductorOne/baton-active-directory): For stable release binaries.
...
The Active Directory connector is self-hosted — it runs on a Windows Server in your environment with direct network access to your domain controllers.

Once installed and configured, the connector runs as a Windows service. The service maintains contact with C1.ai, syncs and uploads data at regular intervals, and passes that data to the C1.ai UI for access reviews and access requests.
...
- The connector runs as a Windows service — a dedicated host is not required

**Network:**

- Outbound TCP/443 to your C1.ai tenant
...
Default file locations (`%PROGRAMDATA%` is typically `C:\ProgramData`):

- **Binary:** `C:\Program Files\ConductorOne\baton-active-directory\baton-active-directory.exe`
...
### Step 1: Set up a new Active Directory connector in C1.ai
...
<Steps>
  <Step>
    Download the latest Windows MSI from the [C1.ai download center](https://dist.conductorone.com/ConductorOne/baton-active-directory).

    <Note>
    **Security prompts during download and install.** The Windows MSI and `.exe` are not yet code-signed, so Microsoft Defender SmartScreen and some antivirus products may warn that the publisher is unknown or quarantine the download. This is expected for unsigned binaries — it is not an indication of tampering. To proceed safely:

    - On the SmartScreen prompt, click **More info → Run anyway**.
    - Verify you downloaded from `dist.conductorone.com` and, if your security policy requires it, confirm the file hash with ConductorOne support before running.

    Authenticode signing of the Windows artifacts is on the roadmap and will remove these prompts.
    </Note>
  </Step>

  <Step>
    Run the MSI on the host designated to run the connector. The MSI installs `baton-active-directory.exe` to `C:\Program Files\ConductorOne\baton-active-directory` and adds it to the system `PATH`.

    <Note>
    The MSI **only installs the binary** — it does not collect your configuration, write a config file, or register the Windows service, and there is no post-install configuration pop-up. All of that happens in the next step, when you run `setup`.
    </Note>
  </Step>

  <Step>
    From an elevated **Command Prompt** (`cmd.exe`), run `baton-active-directory.exe setup` to create the configuration and register the service. This is the same command described in [Option 2](#option-2-cli-setup-command) below — follow it from the `setup` step onward (the config editor opens automatically on this first run because no config exists yet).
  </Step>
</Steps>
...
<Steps>
  <Step>
    On the host designated to run the connector, create a folder: `C:\Program Files\ConductorOne\baton-active-directory`.
  </Step>

  <Step>
    Copy `baton-active-directory.exe` into `C:\Program Files\ConductorOne\baton-active-directory`.
  </Step>
  ...
  <Step>
    ...
    **First-run behavior:** the [GUI Configuration Editor](#gui-configuration-editor) launches automatically **only on the first run, when no config file exists yet**. Fill in your domain, base DN, and C1.ai credentials, click **Save**, and close the editor — `setup` resumes in `cmd.exe` and finishes registering the service. On subsequent runs the editor does **not** reappear; `setup` reuses the existing config.

    <Note>
    To review or change an existing config, either run `baton-active-directory.exe config-editor` directly, or run `baton-active-directory.exe setup --reconfigure` to open the editor and then re-register the service in one step.
    </Note>
  ...
  <Step>
    ...
    5. Enter the service account password and confirm it. **For a gMSA**, append `$` to the account name (e.g. `DOMAIN\svc-baton-ad$`) and leave **both password fields blank** — Windows retrieves the managed password from the domain.
    6. Click **Apply**
    7. Navigate back to the **General** tab and click **Start**

    <Tip>
    Configuring the account through `baton-active-directory.exe setup --service-account="DOMAIN\svc-baton-ad$"` (gMSA) or `--service-account="DOMAIN\user" --service-password="..."` (named user) is recommended over editing the service manually: `setup` grants the **Log on as a service** right, grants the account **Read** on the config file, and grants **Modify** on the log directory automatically. If you configure the account by hand in `services.msc`, you must grant those rights yourself (see the warning below) — otherwise the service fails to start.
    </Tip>
  </Step>
</Steps>

<Warning>
**gMSA / custom service account "service won't start".** When the service runs as any account other than Local System, that account must be able to **read the config file**. The connector locks the config down to SYSTEM and Administrators, so if you change the service account by hand without granting it read, the service exits at startup before it can load its config. `setup` and the MSI installer apply this grant automatically; to apply it manually, run from an elevated prompt:

```console
icacls "C:\ProgramData\ConductorOne\baton-active-directory\config.yaml" /grant "DOMAIN\svc-baton-ad$:(R)"
icacls "C:\ProgramData\ConductorOne" /grant "DOMAIN\svc-baton-ad$:(OI)(CI)(M)"
```

Secrets encrypted with DPAPI use **machine scope**, so a gMSA on the same host can decrypt them once it can read the file — no re-encryption needed.
</Warning>
...
<Steps>
  ...
  <Step>
    Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Active Directory connector to. Active Directory data should be found on the **Entitlements** and **Accounts** tabs.
  </Step>
</Steps>

### Validate the connector is working

A **Connected** status in C1.ai means the connector has registered and is sending heartbeats — it does **not** by itself confirm that the connector can reach a domain controller, bind, and sync. Use these checks to confirm it is actually functioning:

<Steps>
  <Step>
    **Test LDAP connectivity.** From an elevated `cmd.exe`, run:

    ```console
    baton-active-directory.exe test-ldaps
    ```

    This exercises the full path the connector uses — DC resolution, TCP connect, TLS handshake and certificate validation (for LDAPS), and the LDAP bind — for every configured domain, and prints actionable diagnostics for any step that fails. `setup` runs this check automatically at the end of installation (skip it with `--no-verify`).
  </Step>

  <Step>
    **Confirm the service is running.** Run `baton-active-directory.exe status` and verify the state is **Running** and the account/binary/config paths are what you expect.
  </Step>

  <Step>
    **Check the log.** The connector writes to `C:\ProgramData\ConductorOne\baton-active-directory\baton.log`. A healthy run logs sync progress and a completion message; an empty or error-filled log (for example `open sink ".../baton.log": Access is denied`) points to the permission issues described above.
  </Step>

  <Step>
    **Confirm data in C1.ai.** In C1.ai → **Apps** → your AD app, the **Accounts** and **Entitlements** tabs should populate after the first successful sync. Counts that match your directory confirm an end-to-end working connector.
  </Step>
</Steps>

**Done.** Your Active Directory connector is now pulling access data into C1.ai.
...
| `use-display-name` | bool | `false` | Use the LDAP `displayName` attribute as the C1.ai resource label instead of `cn`. Falls back to `cn` when `displayName` is empty. Applies to user and gMSA resources. |
| `locked-account-is-disabled` | bool | `true` | Map a locked-out account to `STATUS_DISABLED` in C1.ai. Set to `false` to keep locked accounts `STATUS_ENABLED` and report lockout solely via the `account_locked` profile field. See [Incomplete-read attributes](#incomplete-read-attributes). |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `client-id` | string | *(required)* | C1.ai client ID. On Windows, can be stored as `dpapi:…`. |
| `client-secret` | string (secret) | *(required)* | C1.ai client secret. On Windows, can be stored as `dpapi:…`. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `password-config` | map | — | Shape of randomly generated passwords (length and per-charset minimum counts). **YAML-only** — not exposed as a CLI flag or environment variable. See [Password generation](#password-generation). |
...
Separately, a principal's own grant listing is paginated once it holds a large number of memberships (1,000 grants per page) — this is unrelated to the LDAP-level paging above, and only affects principals with very large numbers of group memberships (previously a principal in more than roughly 2,400 groups could fail to sync at all). It costs extra LDAP round trips per page and is otherwise invisible; no configuration is needed.

### Password generation

When the connector generates a random password — during account creation and password rotation — the optional `password-config` setting controls its shape so generated passwords satisfy your domain's password policy:

```yaml
password-config:
  length: 20                 # required when constraints are set; 8-64
  constraints:               # optional; minimum counts per character set
...
Like `additional-domains`, this setting is YAML-only — it cannot be set via CLI flags, environment variables, or the C1.ai connector settings UI. It can also be edited in the [GUI Configuration Editor](#gui-configuration-editor) (Advanced tab).

Behavior:

- When present and C1.ai requests a random password, `password-config` replaces the requested password parameters. When absent, behavior is unchanged. "No password" and admin-supplied (encrypted) password flows are never altered.
- Constraints are *minimums* — the remainder of the password is drawn from the full character set (upper/lower/digits/symbols). Making the min-counts sum to `length` leaves no remainder, so your char-sets become the entire alphabet; see the character-class rule below before doing that.
- **Character-class guarantee.** Active Directory's default policy requires three of upper, lower, digit and non-alphanumeric. Without constraints the generator already guarantees one of each; with constraints it emits your minimums and fills the rest at random, which guarantees nothing. So when your char-sets cover fewer than three classes, the connector adds a `min-count: 1` constraint for missing classes until three are covered — your own minimums are never changed, and a class you already cover is never added. A symbol is never added: three classes are always reached from upper, lower and digit first, so a deliberately symbol-free policy stays symbol-free. A generated password therefore always satisfies the policy, rather than satisfying it most of the time and leaving an orphaned account object behind when it does not.
- **A char-set only guarantees a class if it contains nothing else.** The connector draws each `min-count` character at random from your set, so `char-set: "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"` with `min-count: 10` does *not* guarantee a digit — it produces one only by chance, and emits none about 69% of the time. Only a set confined to a single class makes its `min-count` a guarantee. Write one constraint per class rather than one wide set.
- **The config that is refused** is one whose min-counts leave no room to guarantee a missing class. The rule is `sum-of-min-counts + (3 - classes-you-already-guarantee) <= length`: each missing class needs one character to guarantee it. `length: 12` with a single 11-character min-count is refused for that reason even though 11 is less than 12. The error says how many characters to free up. Lower a min-count, or add a char-set for a class you do not already cover.
- `length` is required whenever `constraints` are set, so the length the platform requested is never silently replaced.
- Character sets must be printable ASCII (no spaces). The config is validated at startup and the connector fails fast with a clear error if invalid.
- The connector cannot validate against your domain's password policy — ensure the configured shape satisfies it (complexity, minimum length), or AD rejects the password-set operation.
- The setting is global: it applies to password generation for all configured domains.
...
Once synced, the specified attributes appear as profile fields on the user in C1.ai and can be configured as additional usernames for account correlation.

<Note>
**Casing:** LDAP attribute names are case-insensitive (RFC 4512 §2.5), so configuring `mail`, `Mail`, or `MAIL` all reference the same AD attribute. The connector matches the operator's configured casing case-insensitively against the entry's returned attribute names. The profile-map key preserves the operator's configured casing — so a config of `Mail` produces `profile["Mail"]`, not `profile["mail"]` — predictable for downstream consumers reading the C1.ai profile JSON.
</Note>
...
By default, the connector uses the LDAP `cn` attribute as the C1.ai resource label. AD often stores `cn` in `Last, First` order while `displayName` carries the operator-curated `First Last` form. To use `displayName` as the C1.ai Identity Name, set:
...
| `trusted_for_delegation` | stored `userAccountControl` | Static admin config (`TRUSTED_FOR_DELEGATION` bit) — a discriminating service-account signal |
| `trusted_to_auth_for_delegation` | stored `userAccountControl` | Static admin config (`TRUSTED_TO_AUTH_FOR_DELEGATION` bit) — a discriminating service-account signal |
...
#### Incomplete-read attributes

Two further fields appear on a user, group, or gMSA profile **only when a read came back incomplete**. On a healthy sync they are absent, so their presence is itself the signal.

| Field | Meaning |
|---|---|
| `incomplete_attribute_read` | `true` when at least one attribute on this object could not be read in full |
| `unreadable_attributes` | The affected attribute names, `;`-joined (for example `whenCreated;uSNChanged`) |

Three kinds of name can appear there:

- **Read-completeness sentinels** — `whenCreated`, `whenChanged`, `uSNCreated`, `uSNChanged`. AD writes these on every object of every class, so their absence can only mean the bind account cannot read them. They are the usual sign of a permission problem.
- **A range-restricted attribute whose walk stopped early** — one of `memberOf`, `managedObjects`, `servicePrincipalName`, or `proxyAddresses`. These are the attributes the connector walks when AD returns them in chunks; seeing one here means the value set was too large to return in a single read and could not be walked to completion. It is not a permission problem.
- **`userAccountControl`** — reported separately, since account status is derived from it.

The object is still emitted rather than dropped, so the account remains visible instead of silently disappearing — but values derived from those attributes should be treated as partial. This matters most for `memberOf`: group memberships, and any access derived from them, may be understated, so an access review reading such a record could conclude a user has less access than they actually do.

By default, locked-out accounts also map to `STATUS_DISABLED` in C1.ai (backwards-compatible). To keep locked accounts as `STATUS_ENABLED` and report lockout state solely via the `account_locked` field above, set:
...
The `update_profile` resource action accepts 23 named string fields, each named after the **canonical AD attribute** it targets. The platform's push-rule UI shows the AD attribute name in its "App attribute" column, so the value you map to in C1.ai corresponds directly to the attribute AD will receive. Attributes outside this list (including `extensionAttribute1` through `extensionAttribute15`) must be set via the `custom_attributes` map field on `update_profile`, or via the `update_user_attrs` global action.
...
**How the connector chooses a DC.** For a **write-capable** connection it asks Windows (`DsGetDcNameW`) for a **writable** DC (`DS_WRITABLE_REQUIRED`); for a **read-only** connection it does not, so the in-site DC (including an RODC) may be used. Write intent is per connection:

- The **primary domain** is always write-capable (it is the provisioning target). A read-only *primary* mode is not currently configurable.
...
- **WinLDAP mode** (default): TLS validation is delegated to the Windows certificate store. `ldaps-skip-verify` has **no effect** — the OS always validates against `Cert:\LocalMachine\Root`. To trust a CA, import its root certificate into the Windows machine store:
...
It is idempotent — already-encrypted values are skipped — so it is safe to re-run.

<Note>
DPAPI here is **machine-scoped** (`CRYPTPROTECT_LOCAL_MACHINE`). Any process on the same host can decrypt these values, which is what lets the service — including one running as a gMSA — read them without a per-user key. It protects the secrets if the file leaves the machine (backups, file shares, support bundles), **not** from other accounts on the machine. The file's permissions are what restrict local access: `setup` applies those automatically, and `encrypt-config` warns if the file is readable by ordinary users.

If a host's `config.yaml` has been readable by `BUILTIN\Users` — check with `icacls "C:\ProgramData\ConductorOne\baton-active-directory\config.yaml"` — then every local account on that server could read *and* decrypt its secrets. Encrypting or locking the file now does not undo that: **rotate the C1.ai client credentials and the AD bind password**, then store the new values with `setup` or `encrypt-config`.
...
`setup` locks down `config.yaml` itself. The directory containing it keeps the permissions it inherits from `%PROGRAMDATA%`, which include `BUILTIN\Users:(RX)` — so `baton.log` is readable by any account on the server. To restrict the whole directory:

```console
baton-active-directory.exe setup --lock-config-dir
```

This is **opt-in**. It limits the directory to `NT AUTHORITY\SYSTEM`, `BUILTIN\Administrators` and the service account, so anything else that reads it as a non-administrator — a log shipper, monitoring agent, or backup job — loses access. A service account changed later through `services.msc` rather than by re-running `setup` will not be in the resulting ACL either, and will fail at startup when it cannot open `baton.log` (SCM reports this as error 1053); re-run `setup` after changing the account. When setup runs on a console and the directory is open, it reports this and offers to restrict it, defaulting to No. The prompt is skipped when stdin is not a terminal, so scripted installs are unaffected.

To apply it later without reinstalling the service (add the `/grant:r` line for your service account only if the service does not run as LocalSystem):

```console
icacls "C:\ProgramData\ConductorOne\baton-active-directory" /inheritance:r ^
  /grant:r "NT AUTHORITY\SYSTEM:(OI)(CI)(F)" ^
  /grant:r "BUILTIN\Administrators:(OI)(CI)(F)" ^
  /grant:r "DOMAIN\svc-baton-ad$:(OI)(CI)(M)"
```

To undo it and restore inheritance:

```console
icacls "C:\ProgramData\ConductorOne\baton-active-directory" /inheritance:e
```
...
Once your Active Directory connector is synced, you can use C1.ai to run user access reviews on AD group memberships, enable just-in-time access requests for AD groups and gMSAs, and automate provisioning workflows using connector actions.
