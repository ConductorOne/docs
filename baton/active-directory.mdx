...
- [C1 download center](https://dist.conductorone.com/ConductorOne/baton-active-directory): For stable release binaries.
...
Once installed and configured, the connector runs as a Windows service. The service maintains contact with C1, syncs and uploads data at regular intervals, and passes that data to the C1 UI for access reviews and access requests.
...
<Steps>
  ...
  <Step>
    ...
    **First-run behavior:** the [GUI Configuration Editor](#gui-configuration-editor) launches automatically **only on the first run, when no config file exists yet**. Fill in your domain, base DN, and C1 credentials, click **Save**, and close the editor — `setup` resumes in `cmd.exe` and finishes registering the service. On subsequent runs the editor does **not** reappear; `setup` reuses the existing config.
...
A **Connected** status in C1 means the connector has registered and is sending heartbeats — it does **not** by itself confirm that the connector can reach a domain controller, bind, and sync. Use these checks to confirm it is actually functioning:
...
| `locked-account-is-disabled` | bool | `true` | Map a locked-out account to `STATUS_DISABLED` in C1. Set to `false` to keep locked accounts `STATUS_ENABLED` and report lockout solely via the `account_locked` profile field. See [Incomplete-read attributes](#incomplete-read-attributes). |
...
Like `additional-domains`, this setting is YAML-only — it cannot be set via CLI flags, environment variables, or the C1 connector settings UI. It can also be edited in the [GUI Configuration Editor](#gui-configuration-editor) (Advanced tab).

Behavior:

- When present and C1 requests a random password, `password-config` replaces the requested password parameters. When absent, behavior is unchanged. "No password" and admin-supplied (encrypted) password flows are never altered.
