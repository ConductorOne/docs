...
description: "C1 provides identity governance and just-in-time provisioning for Active Directory. Integrate your on-prem Active Directory server with ConductorOne to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1 provides identity governance and just-in-time provisioning for Active Directory. Integrate your on-prem Active Directory server with ConductorOne to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
Each attribute name you specify will be fetched from AD and added to the user profile in ConductorOne, where it can be used as a profile attribute for account correlation.

**Example:** To pull a custom `githubUserName` field from AD so ConductorOne can link GitHub accounts to AD users:
...
Once synced, `githubUserName` will appear as a profile attribute on the user in ConductorOne and can be configured as an additional username for account correlation with GitHub.
