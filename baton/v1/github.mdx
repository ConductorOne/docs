...
og:title: Set up GitHub connector - C1.ai docs
og:description: Integrate your GitHub instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
...
<Tip>
**A newer version of this connector is available.** If you're setting up a GitHub connector with C1.ai for the first time, use the [v2 version](/baton/github). 
</Tip>
...
- Sync user identities from GitHub to C1.ai
...
- **Super Administrator** role in C1.ai
...
<Steps>
<Step>
In C1.ai, navigate to **Apps** > **Connectors** > **GitHub**.
</Step>
<Step>
...
Choose whether to add the new GitHub connector as a data source to an existing application (and select the app of your choice) or to create a new application.
   ...
   <Tip>
   **Do you SSO into GitHub using your identity provider (IdP)?** If so, make sure to add the connector to the GitHub app that was created automatically when you integrated your IdP with C1.ai, rather than creating a new app.
   </Tip>
</Step>
<Step>
Set the owner for this connector. You can manage the connector yourself, or choose someone else from the list of C1.ai users. Setting multiple owners is allowed.

   <Warning>
   A GitHub connector owner must have the following permissions:

   - **Connector Administrator** or **Super Administrator** role in C1.ai
   - **Org Owner** access in GitHub
   </Warning>
</Step>
<Step>
...
- **If you are the integration owner**, proceed to **Integrate your GitHub instance [chosen method]** for instructions on integrating GitHub with C1.ai.

- **If someone else is the integration owner**, C1.ai will notify them by email that their help is needed to complete the setup process.

<Tip>
Choose the appropriate set of integration instructions based on the type of GitHub access token you want to use with C1.ai:
...
<Warning>
A user with the **Connector Administrator** or **Super Administrator** role in C1.ai and **Org Owner** access in GitHub must perform this task.
</Warning>
...
### Step 3: Add your GitHub credentials to C1.ai
...
<Step>
In the **Resource owner** dropdown, select the username associated with the GitHub organization you're integrating with C1.ai.
</Step>
<Step>
...
In the **Permissions** section of the page, give the token the following permissions:
   ...
   <Warning>
   The repository permissions are used by C1.ai to sync and display data on repo membership (C1.ai cannot provision repositories). **If you do not want C1.ai to sync and display your GitHub organization's repo data, do not give your token these permissions.**
   </Warning>
</Step>
<Step>
Click **Generate token**. Copy and save the new token. We'll use it in Step 3.
