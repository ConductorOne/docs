...
description: "Integrate your Okta instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access."
og:description: "Integrate your Okta instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access."
sidebarTitle: "Okta (v1)"
---

<Tip>
 **A newer version of this connector is available.** This version of the connector is no longer available for installation. If you're setting up an Okta connector with C1.ai for the first time, use the [v2 version](/baton/okta).
</Tip>

## Capabilities

- Sync user identities from Okta to C1.ai
- Promote identities from application users to C1.ai users
...
Depending on the permissions of the Okta API Token, C1.ai has different functionality that it can provide. See the next section for more information.
...
| C1.ai capability | Read-only admin token | Read-only + app admin + group admin token | Super admin token |
| :--- | :--- | :--- | :--- |
...
Proceed to [Configure your Okta connector](/baton/v1/okta#configure-your-okta-connector) for instructions on integrating Okta with C1.ai.

## Configure your Okta connector

<Warning>
A user with the **Connector Administrator** or **Super Administrator** role in C1.ai and **Read Only Admin** or **Super Admin** role in Okta must perform this task.
</Warning>
...
### Step 3: Add your Okta credentials to C1.ai

<Steps>
<Step>
In C1.ai, navigate to the Okta connector by either:
...
<Step>
**Optional.** If you want C1.ai to skip syncing inactive child apps, or to sync deprovisioned users, click the checkboxes accordingly.
</Step>
