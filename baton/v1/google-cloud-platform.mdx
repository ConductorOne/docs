...
og:title: Set up a Google Cloud Platform connector - C1.ai docs
og:description: Integrate your Google Cloud Platform instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1.ai provides identity governance and just-in-time provisioning for Google Cloud Platform. Integrate your Google Cloud Platform instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access.
sidebarTitle: Google Cloud Platform
---

<Tip>
**A newer version of this connector is available.** If you're setting up a Google Cloud connector with C1.ai for the first time, use [Google Cloud Platform with Google Workspace](/baton/google-cloud-platform). 
</Tip>

<Tip>
**Which Google connector should I use?** 
Each of the Google connectors offered by C1.ai varies in what it can sync and what it can provision. Selecting the right connector for you will depend on which Google services your organization uses, and how you want to manage Google resources within C1.ai. 
...
* [Google Cloud Platform with Google Workspace](/baton/google-cloud-platform) can do all of the above, and it combines all of the resources and entitlements of the two connectors into a single app within C1.ai. 
...
Under **Service account details**, fill in the following:
   ...
   - **Service account description:** for example, "Service account for C1.ai Google Cloud Platform Integration"
</Step>
<Step>
...
Under **Grant this service account access to a project**, grant the appropriate permission level:
   ...
   - **Editor** to provision access via C1.ai and run access reviews

   Alternatively, you can create and assign a custom role on the org level: 

   You'll need these permissions to give C1.ai **READ** access (for syncing access data):
   ...
   You'll need these permissions to give C1.ai **READ/WRITE** access (for syncing access data and provisioning access):
...
Select the appropriate roles:
   ...
   - **Organization Viewer** and **Viewer** to give C1.ai **READ** access

   - **Organization Administrator** and **Editor** to give C1.ai **READ/WRITE** access
</Step>
<Step>
Click **Save**.
</Step>
...
<Steps>
<Step>
In the C1.ai project, search for "API keys" and enable it.
</Step>
<Step>
Next, grant the API Keys Viewer Role to the service account you created for C1.ai. Navigate to **IAM & Admin** > **IAM**.
</Step>
<Step>
...
Next, we'll return to the C1.ai integration project you created earlier to generate the necessary credentials.
...
**Done.** Your Google Cloud Platform connector is now pulling access data into C1.ai.
</Tab>
