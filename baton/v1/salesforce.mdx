...
description: "Integrate your Salesforce instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access."
...
<Tip>
 **A newer version of this connector is available.** This version of the connector is no longer available for installation. If you're setting up a Salesforce connector with C1.ai for the first time, use the [v2 version](/baton/salesforce).
</Tip>
...
- Sync user identities from Salesforce to C1.ai
...
Proceed to [Configure your Salesforce connector](/baton/v1/salesforce#configure-your-salesforce-connector) for instructions on integrating Salesforce with C1.ai.

## Configure your Salesforce connector

<Warning>
A user with the **Connector Administrator** or **Super Administrator** role in C1.ai and a Salesforce user account with a profile that includes the **API Enabled** permission must perform this task.
</Warning>

### Step 0: Enable API access for your Salesforce user

Before you begin, make sure that the Salesforce user who will set up the integration with C1.ai has a profile that includes the **API Enabled** permission. This permission is included by default in the **Standard User** and **System Administrator** profiles.
...
<Steps>
<Step>
In C1.ai, navigate to the Salesforce connector by either:
...
<Step>
**Optional.** Check the box to tell C1.ai to use Salesforce usernames as the email addresses for your organization's accounts. This option is especially helpful if your organization uses multiple service accounts that all share a `noreply@salesforce.com` email address.
</Step>
...
<Step>
Log in and authorize C1.ai with your Salesforce instance.
</Step>

<Step>
You will then be redirected back to the Salesforce setup page in C1.ai, where you'll see an authorization message.
</Step>
