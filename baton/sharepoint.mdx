...
og:title: Set up a Microsoft SharePoint connector - C1.ai docs
og:description: "C1.ai provides identity governance for SharePoint. Integrate your SharePoint instance with C1.ai for unified visibility and governance over user access."
description: "C1.ai provides identity governance for SharePoint. Integrate your SharePoint instance with C1.ai for unified visibility and governance over user access."
sidebarTitle: Microsoft SharePoint
---
...
<Step>
Give the application a name, such as "C1.ai", and select the relevant supported account type. You do not need to set a redirect URI. 
</Step>
<Step>
Click **Register**. 
</Step>
...
<Note>
**About groups that restrict membership visibility**

Some SharePoint groups have *"Who can view the membership of the group"* set to **Group Members** instead of **Everyone**. Only members of such a group (or an administrator) can list its members.
...
**What the flag does:** when enabled, it skips any SharePoint group whose membership visibility is restricted (regardless of the permission you granted), so those groups and their memberships are excluded from the sync.

**Enabling it on an existing connector:** if the connector previously synced these groups (for example, while using `Sites.FullControl.All`), enabling the flag will remove those groups and their grants from C1.ai on the next sync, since they're now excluded.
</Note>
...
## Deduplicate site collection groups

SharePoint defines groups and users **once per site collection**, not once per site. A site collection's SharePoint groups, site users, and site collection administrators are shared by every site (web) in that collection, so by default the connector syncs the same groups again under every subsite. On tenants with deep site hierarchies this produces many duplicate **SharePoint group** resources and a correspondingly longer sync.

Enabling the **deduplicate-site-collection-groups** flag (or setting `BATON_DEDUPLICATE_SITE_COLLECTION_GROUPS=true`) syncs each site collection's groups, users, and administrators a single time, at the collection's root site. Subsites under the `/sites` and `/teams` managed paths, and a first-level web directly under the host root (e.g. `https://host/subweb`), are still synced as **Site** resources, but they're nested under their collection's root site and no longer carry their own group entitlements or grants. A site whose collection root can't be derived from its URL (a custom managed path like `/portals/<hub>`, or a web nested more than one level under the host root) is left un-nested and keeps its own groups, users, and administrators — the same as with the flag off — since there's no root to safely consolidate it under.

The flag is **off by default**, so existing connectors keep their current behavior until you turn it on. New connectors created in C1.ai have the toggle **pre-checked on**, since deduplication is the recommended setting going forward — uncheck it during setup if you'd rather opt in later.

<Warning>
**Enabling it on an existing connector produces one large diff.** On the first sync after you enable the flag, C1.ai removes the duplicate SharePoint group resources, re-parents the remaining groups to their site collection root, re-parents subsites under their root site, and removes subsite-level administrator grants. This is expected: the access data itself is unchanged, but the resources representing it are consolidated. Subsequent syncs are stable.

If you have access reviews or policies that reference specific duplicate group resources, review them before enabling the flag.
</Warning>
...
<Step>
Finally, tell the connector where to find the identities that will be used for this app in C1.ai. 
...
**Done.** Your SharePoint connector is now pulling access data into C1.ai.
</Tab>
...
[Contact C1.ai's support team](mailto:support@conductorone.com) to download the latest version of the connector.
...
stringData:
  ...
  BATON_SKIP_MEMBERSHIP_RESTRICTED_GROUPS: <Optional. Set to "true" when using Sites.Read.All to skip groups that restrict membership visibility. Leave unset/false when using Sites.FullControl.All.>
  BATON_DEDUPLICATE_SITE_COLLECTION_GROUPS: <Optional. Set to "true" to sync each site collection's groups, users, and administrators once at the collection root instead of repeating them under every subsite. Defaults to false.>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-sharepoint
        image: public.ecr.aws/conductorone/baton-sharepoint:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the SharePoint connector to. SharePoint data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
**Done.** Your SharePoint connector is now pulling access data into C1.ai.
</Tab>
