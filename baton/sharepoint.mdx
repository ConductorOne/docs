...
og:title: Set up a Microsoft SharePoint connector - C1 docs
og:description: "C1 provides identity governance for SharePoint. Integrate your SharePoint instance with C1 for unified visibility and governance over user access."
description: "C1 provides identity governance for SharePoint. Integrate your SharePoint instance with C1 for unified visibility and governance over user access."
...
**Enabling it on an existing connector:** if the connector previously synced these groups (for example, while using `Sites.FullControl.All`), enabling the flag will remove those groups and their grants from C1 on the next sync, since they're now excluded.
</Info>
...
The flag is **off by default**, so existing connectors keep their current behavior until you turn it on. New connectors created in C1 have the toggle **pre-checked on**, since deduplication is the recommended setting going forward — uncheck it during setup if you'd rather opt in later.

<Warning>
**Enabling it on an existing connector produces one large diff.** On the first sync after you enable the flag, C1 removes the duplicate SharePoint group resources, re-parents the remaining groups to their site collection root, re-parents subsites under their root site, and removes subsite-level administrator grants. This is expected: the access data itself is unchanged, but the resources representing it are consolidated. Subsequent syncs are stable.
