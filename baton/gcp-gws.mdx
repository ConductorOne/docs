...
The service account needs the `roles/cloudasset.viewer` role at the organization level to search resources across projects.

**Required APIs:**

Enable these APIs for each project you want to sync (or only for specified projects if using the **Project IDs** filter):
...
Use these flags to enable syncing of secrets and buckets:

| Flag | Description |
|------|-------------|
| `--sync-secrets` | Enable Secret Manager sync |
| `--sync-secret-manager-secrets` | Enable Secret Manager secrets sync |
| `--sync-buckets` | Enable Cloud Storage buckets sync |

**Example:**

```bash
./baton-google-cloud-platform \
...
<Note>
We recommend using `--project-id` to explicitly filter which projects to sync. Without this filter, the connector attempts to sync resources across all projects, which requires the service account to have permissions in every project.
</Note>
