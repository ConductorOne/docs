...
og:title: Set up a Slack Enterprise Grid connector - C1.ai docs
og:description: Integrate your Slack Enterprise Grid instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1.ai provides identity governance and just-in-time provisioning for Slack Enterprise Grid. Integrate your Slack Enterprise Grid instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests.
sidebarTitle: Slack Enterprise Grid
---

<Warning>
**Yes, there are TWO kinds of Slack integration!** The instructions below integrate a Slack Enterprise Grid account with C1.ai so that you can review access data and grant user access to Slack. This integration supports GovSlack. 

(We also have an integration for [Slack Pro and Business+ accounts](/baton/slack).)

If you want to install the C1.ai app for Slack, so that you and your colleagues can request access and get notifications about new C1.ai tasks in your Slack workspace, go to the **Settings** page in C1.ai and click **Notifications**.
</Warning>
...
### Create a C1.ai app in Slack Enterprise Grid
...
<Step>
Give your new app a name, such as "C1.ai integration", and select a workspace you want to integrate with C1.ai (later we'll install the app org-wide).
</Step>
<Step>
Click **Create App**.
</Step>
...
Scroll down to the **Scopes** section of the page. In the **Bot Token Scopes** area, add the following OAuth Scopes:
   ...
   - team:read
   - usergroups:read
   - usergroups:write (include only if you want C1.ai to provision user group membership)
   - users.profile:read
   - users:read
   - users:read.email
</Step>
...
In the **User Token Scopes** area, add the following OAuth Scopes:
   ...
   - admin
   - admin.roles:read
   - admin.roles:write (include only if you want C1.ai to provision access)
   - admin.teams:read
   - admin.usergroups:read
   - admin.usergroups:write (include only if you want C1.ai to provision user group membership)
   - admin.users:read
   - admin.users:write (include only if you want C1.ai to provision access)
   - usergroups:write (include only if you want C1.ai to provision user group membership)
</Step>
</Steps>
### Install the C1.ai app org-wide

You must now install the new app org-wide so that C1.ai can sync data from all of your organization's workspaces.
...
### Add the C1.ai app to all workspaces
...
**Done.** Your Slack Enterprise Grid connector is now pulling access data into C1.ai.
</Tab>
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Slack Enterprise Grid connector to. Slack Enterprise Grid data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
**Done.** Your Slack Enterprise Grid connector is now pulling access data into C1.ai.
</Tab>
