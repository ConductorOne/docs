...
description: "C1.ai provides identity governance and just-in-time provisioning for Microsoft Dynamics 365 - Finance & Operations. Integrate your Microsoft Dynamics 365 - Finance & Operations instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Microsoft Dynamics 365 - Finance & Operations. Integrate your Microsoft Dynamics 365 - Finance & Operations instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
|-------------|-------------------|-------------|
| enable_user | `user_id` (string, required) | Enables a disabled Microsoft Dynamics 365 - Finance & Operations user account |
| disable_user     | `user_id` (string, required) | Disables an active Microsoft Dynamics 365 - Finance & Operations user account |
...
<Step>
Click the name of the environment you want to integrate with C1.ai. 
</Step>
<Step>
...
<Step>
Give the application a name, such as "C1.ai", and select the supported account type relevant to your Entra installation (typically **Accounts in this organizational directory only (Default Directory only - Single tenant)**). You do not need to set a redirect URL. 
</Step>
<Step>
Click **Register**. 
</Step>
...
Add a new register to the table, filling in: 
    ...
    - The app's client ID 
    - Your user ID
    - A name that identifies this entry, such as "C1.ai integration" 
</Step>
<Step>
Click **OK**.
</Step>
...
<Step>
**Optional.** If desired, click to enable **Sync only enabled role assignments**. When this option is enabled, C1.ai will ignore all roles that do not have the `enabled` status. 
</Step>
<Step>
Click **Save**.
</Step>
...
**Done.** Your Microsoft Dynamics 365 - Finance & Operations connector is now pulling access data into C1.ai.
</Tab>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-microsoft-dynamics-fo
        image: public.ecr.aws/conductorone/baton-microsoft-dynamics-fo:latest
