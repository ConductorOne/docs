...
description: "C1.ai provides identity governance for Cursor. Integrate your Cursor instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance for Cursor. Integrate your Cursor instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
sidebarTitle: "Cursor"
---
...
| :--- | :--- | :--- |
| Account | <Icon icon="square-check" iconType="solid" color="#c937ae"/> | <Icon icon="square-check" iconType="solid" color="#c937ae"/> Create and delete via SCIM |
...
**Done.** Your Cursor connector is now pulling access data into C1.ai.
...
This data is immediately available in the C1.ai UI for access reviews and access requests.
...
stringData:
  ...
  BATON_CURSOR_SCIM_ENDPOINT: <Cursor SCIM endpoint URL>
  BATON_CURSOR_SCIM_TOKEN: <Cursor SCIM bearer token>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-cursor
        image: public.ecr.aws/conductorone/baton-cursor:latest
...
<Steps>
  ...
  <Step>
  Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Cursor connector to. Cursor data should be found on the **Entitlements** and **Accounts** tabs.
  </Step>
</Steps>
