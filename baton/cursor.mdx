...
description: "C1 provides identity governance for Cursor. Integrate your Cursor instance with C1 to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1 provides identity governance for Cursor. Integrate your Cursor instance with C1 to run user access reviews (UARs) and enable just-in-time access requests."
sidebarTitle: "Cursor"
---
...
<Steps>
  ...
  <Step>
  Navigate to **Team Settings** > **API**. This is the **Team**-level API page, not the organization-level **API Keys** page — an admin key created at the organization level does not work with this connector.
  </Step>
  <Step>
  Select the **Team** tab (as opposed to **User**).
  </Step>
  <Step>
  Click **New API Key**, provide a descriptive name (for example, `C1 Integrations`), and set the scope to **Admin**.
  </Step>
  <Step>
  Copy the generated API key and save it securely. The key starts with `crsr_`. You cannot retrieve the key after leaving the page.
  </Step>
</Steps>
...
<Steps>
  <Step>
  In the Cursor dashboard, navigate to **Team Settings** and locate the **SCIM** section.
  </Step>
  <Step>
