...
description: "C1.ai provides identity governance and just-in-time provisioning for Okta. Integrate your Okta instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Okta. Integrate your Okta instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
sidebarTitle: "Okta"
---

<Tip>
**This is an updated and improved version of the Okta connector!** If you're setting up Okta with C1.ai for the first time, you're in the right place.
</Tip>

<Note>
This page covers the Okta **connector**, which syncs and provisions access data for identity governance. It does not control how you sign in to C1.ai. To set up signing in to C1.ai with your Okta account, see [Authenticate with Okta](/product/how-to/qs-set-up-c1#authenticate-with-okta).
</Note>
...
<Note>
The standard roles **Workflows Administrator**, **Access Certifications Administrator**, and **Access Requests Administrator** appear as Custom Roles in C1.ai. Enable **Sync Custom Roles** to see them. The Okta API returns these roles through the same endpoint as custom roles.
</Note>
...
<Note>
**Skip app groups changes how app access is attributed.** When a group is assigned to an app, C1.ai records that group as the source of each member's access. Skipping `APP_GROUP` groups means they are never synced, so access conferred by one has no group to point at. Rather than dropping that access, the connector reports it as a direct assignment on the app.

Two things this does *not* do:

- **It does not flatten all group-based access on the app.** An `OKTA_GROUP` assigned to the same app still expands to its members normally, and those grants still name the group as the source.
- **It applies per app, not per group.** On an app assigned both an `APP_GROUP` and an `OKTA_GROUP`, members of the `OKTA_GROUP` keep their group-sourced access *and* gain an additional direct assignment they would not have otherwise.

On such an app, the direct assignments produced this way are marked immutable, because removing that access means changing group membership rather than the app assignment. Members who came only from a skipped `APP_GROUP` have no source recorded at all.
</Note>

The Okta connector supports [automatic account provisioning](/product/admin/account-provisioning).

The Okta connector supports account deprovisioning through C1.ai resource deletion. It first deactivates the user to the `DEPROVISIONED` state and then permanently deletes the user. The same operations are available separately through the `deactivate_user` and `delete_user` connector actions. These operations require an API token or OAuth app with `okta.users.manage` scope.

#### Account creation profile fields

When you configure account provisioning for Okta, C1.ai shows the following optional fields on the account creation form (in addition to first name, last name, email, and login):
...
| **Send Activation Email** | `True` / `False` | `True` | When set to `False`, activates the user without sending Okta's activation email. Use this when C1.ai (or another system) owns user communication. |
...
<Note>
**Okta Staged status (sign-in to Okta, not C1.ai).** In Okta, **Staged** means the account was created but not activated yet, so the user cannot sign in to Okta. That is an Okta lifecycle state — it is not about signing in to ConductorOne. Staged users sync as **disabled** in C1.ai so sync matches the lifecycle actions: `disable_user` is a no-op on staged accounts, and `enable_user` activates them (`/lifecycle/activate` without sending another activation email). After activation the account becomes **Active** or **Provisioned** (Provisioned only means they have not completed first login yet).
...
Connector actions are custom capabilities that extend C1.ai automations with app-specific operations. The Okta connector's actions are reached from **two different automation steps**, and which step offers an action is determined by the action itself — you cannot choose:

| Automation step | Actions offered | How the target is selected |
|-----------------|-----------------|----------------------------|
| [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) | `deactivate_user`, `delete_user`, `update_profile`, `create`, `modify_group` | Varies by action. The three user actions use a resource selector on the **User** resource type. `modify_group` takes the Okta group ID as a plain text field, and `create` has no target at all. |
...
In the Perform connector action step, actions are listed under their resource type and display name rather than their raw action name: `deactivate_user`, `delete_user`, and `update_profile` appear as **`user - Deactivate User`**, **`user - Delete User`**, and **`user - Update User Profile`**, and `modify_group` as **`group - Modify Group`**. An action with no display name registered falls back to its raw name, so `create` appears as **`group - create`**.

`enable_user`, `disable_user` and `update_user` take `user_id` as a plain string, and `modify_group` takes `group_id` the same way. Pass the raw Okta ID (for example `00u1abc2def3GHI4jk5`), not the C1.ai resource ID — the connector sends the value to Okta unchanged, and a C1.ai resource ID returns a not-found error. `deactivate_user`, `delete_user`, and `update_profile` take a resource selector instead, so the Okta ID is resolved for you.
...
| update_user | `user_id` (string, required), `user_profile` (JSON object, required) | Updates a user's profile from a JSON object of Okta profile attributes — accepts any attribute, not limited to update_profile's curated list. This is the action C1.ai's automated profile-push pipeline uses. |
...
<Warning>
`deactivate_user`, `delete_user`, and C1.ai account deprovisioning are destructive. Deactivation removes the user's assigned-app provisioning and can destroy downstream data such as email or files. Deletion permanently removes the Okta user and cannot be recovered. Use workflow approval gates where appropriate.
</Warning>
...
Use the raw Okta external ID (e.g. `00g1abc2def3GHI4jk5`), not the C1.ai resource ID. Group deletion is not available through the C1.ai UI or the Automations tab.
</Note>
...
To give the C1.ai integration limited read-only admin permissions, create a custom Okta admin role and resource set and assign them to the admin you'll use to generate the API key. 
...
We strongly recommend creating a dedicated token for the C1.ai integration so you can manage the token's lifespan and control per-token rate limits. 
...
<Step>
Give the app a name (e.g., "C1.ai Integration") and click **Save**.
</Step>
<Step>
...
Navigate to the **Okta API Scopes** tab and grant the scopes required for your use case:
   ...
   - `okta.logs.read` (required for the event feed)
   - `okta.apiTokens.read` (required when **Sync secrets** is enabled or the **API Token** resource type is enabled)
...
<Note>
**Private key format.** The **Okta Private Key** field in the C1.ai connector form holds a single line. Pasting a multi-line PEM into it replaces the line breaks with spaces, and the connector rejects the result with `oktaauth: parse private key: no PEM block found`.
...
<Note>
This setting also changes how app access from those groups is attributed. Access conferred by a skipped group is reported as a direct assignment on the app, since the group is no longer synced to point at. Groups of other types assigned to the same app still expand to their members normally. See **Capabilities** above for the full behavior.
</Note>
</Step>
<Step>
...
**Done.** Your Okta connector is now pulling access data into C1.ai.
</Tab>
...
stringData:
  ...
  # Optional: inactive (disabled) Okta applications are synced by default.
  # Set this to false to exclude them.
  # BATON_SYNC_INACTIVE_APPS: "false"
```
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Okta connector to. Okta data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
...
If Okta is your company's identity provider (meaning that it is used to SSO into other software), the connector sync will automatically create applications in C1.ai for all of your SCIMed software. Before you move on, review the [Create applications](/product/admin/applications) page for important information about how to set up connectors for the SCIMed apps.
