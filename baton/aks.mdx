---
title: "Set up a Microsoft AKS connector"
og:title: "Set up a Microsoft AKS connector"
description: "C1.ai provides identity governance for Azure Kubernetes Service (AKS). Integrate your AKS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance for Azure Kubernetes Service (AKS). Integrate your AKS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
sidebarTitle: "Microsoft AKS"
---
...
<Steps>
  ...
  <Step>
  Give the application a name, such as "C1.ai".
  </Step>
  <Step>
...
**Done.** Your AKS connector is now pulling access data into C1.ai.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-aks
        image: public.ecr.aws/conductorone/baton-aks:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the AKS connector to. AKS data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
