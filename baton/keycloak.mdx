...
description: "C1.ai provides identity governance and just-in-time provisioning for Keycloak. Integrate your Keycloak instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Keycloak. Integrate your Keycloak instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
**Done.** Your Keycloak connector is now pulling access data into C1.ai.
...
<Note>
  **Syncing subgroups**: Set `BATON_SYNC_SUB_GROUPS` to `true` to sync nested
  ...
  are surfaced in C1.ai, and grant and revoke operations are supported for nested
  groups.
</Note>
