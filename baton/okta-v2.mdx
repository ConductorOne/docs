...
<Icon icon="circle-info" /> The standard roles **Workflows Administrator**, **Access Certifications Administrator** and **Access Requests Administrator** will be synced as Custom Roles (`Sync Custom Roles` option should be enabled in order to see them). This is because how Okta API allows access to the data related to them.
