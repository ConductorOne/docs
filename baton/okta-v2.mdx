...
<Icon icon="circle-info" /> The standard roles **Workflows Administrator**, **Access Certifications Administrator**, and **Access Requests Administrator** are synced as Custom Roles (the `Sync Custom Roles` option must be enabled to see them). This is because of how the Okta API exposes data for these roles.
