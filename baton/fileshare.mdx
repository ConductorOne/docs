...
og:title: Set up a Windows File Share connector - ConductorOne docs
og:description: Integrate your Windows file share NTFS permissions with ConductorOne to run access reviews and govern shared folder access.
description: ConductorOne provides identity governance for Windows file shares. Integrate your file share NTFS permissions with ConductorOne to run user access reviews (UARs) and gain visibility into shared folder access.
...
<Step>
Search for **File Share** and click **Add**.
</Step>
<Step>
...
<Step>
Enter the file share path into the **Share Path** field. This can be a local Windows path (e.g., `C:\Shares\Data`) or a UNC path (e.g., `\\server\share`).
</Step>
<Step>
Optionally configure additional settings:

   * **Path Prefix** — A Windows path prefix to prepend to folder display names (e.g., `V:\HR` causes folder `Admin` to display as `V:\HR\Admin`).
   * **Skip Inherited Folders** — Skip folders that only have inherited ACLs and skip grants where the group permission is inherited (default: enabled).
   * **Max Folder Depth** — Limit how deep the connector traverses the folder hierarchy. Leave empty for unlimited depth.
   * **Exclude Groups** — Active Directory groups in `DOMAIN\GROUP` format to exclude from folder grants.
</Step>
<Step>
Click **Save**.
</Step>
...
**That's it!** Your Windows File Share connector is now pulling access data into ConductorOne.
</Tab>
<Tab title="Self-hosted">
**Follow these instructions to use the Windows File Share connector, hosted and run in your own environment.**

When running as a Windows service, the self-hosted connector maintains an ongoing connection with ConductorOne, automatically syncing and uploading data at regular intervals. This data is immediately available in the ConductorOne UI for access reviews and access requests.
...
### Step 1: Configure the File Share connector in ConductorOne
...
```yaml
# ConductorOne credentials (from Step 1)
...
<Steps>
<Step>
Check that the connector data uploaded correctly. In ConductorOne, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the File Share connector to. File share data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
<Step>
