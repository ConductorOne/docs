---
title: Set up a Microsoft Windows File Share connector
og:title: Set up a Microsoft Windows File Share connector - C1.ai docs
og:description: Integrate your Windows file share NTFS permissions with C1.ai to run access reviews and govern shared folder access.
description: C1.ai provides identity governance for Windows file shares. Integrate your file share NTFS permissions with C1.ai to run user access reviews (UARs) and gain visibility into shared folder access.
...
- **Grant expansion** — Each folder grant includes a `GrantExpandable` annotation pointing to the matched group's `member` entitlement. This allows C1.ai to expand group membership to see which individual users have access to each folder.
...
- **`baton-active-directory` is configured as a shared identity source** in C1.ai. The file share connector discovers Active Directory groups in folder ACLs, and relies on the AD connector to match those groups and their members to C1.ai identities.
...
When running as a Windows service, the self-hosted connector maintains an ongoing connection with C1.ai, automatically syncing and uploading data at regular intervals. This data is immediately available in the C1.ai UI for access reviews and access requests.
...
### Step 1: Configure the File Share connector in C1.ai
...
<Steps>
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the File Share connector to. File share data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
<Step>
...
**Done.** Your Windows File Share connector is now pulling access data into C1.ai.
</Tab>
</Tabs>
