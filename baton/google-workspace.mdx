...
Enable **Sync Enterprise Apps** to track which users have accessed OAuth and SAML apps in your domain.
...
Click **Add new** and fill out the form:
   ...
   - **OAuth Scopes**: Copy and paste in the relevant scopes
      ...
      **If using Sync Enterprise Apps**, add the following additional scopes:

      - Scope:   https://www.googleapis.com/auth/admin.directory.user.security <br/>
        Purpose: To discover OAuth apps via per-user token listing.

      - Scope:   https://www.googleapis.com/auth/admin.reports.audit.readonly <br/>
        Purpose: To fetch SAML and Google login events for last-login timestamps.
...
<Step>
**(Optional)** To sync enterprise applications and user login activity, enable the **Sync Enterprise Apps** toggle. This requires the additional OAuth scopes described in the [Add the service account to Google Workspace](#add-the-service-account-to-google-workspace) section.
</Step>
<Step>
Click **Save**.
</Step>
...
Re-add the relevant scopes:
   ...
   If using Sync Enterprise Apps, also add:

   ```bash
   https://www.googleapis.com/auth/admin.directory.user.security, https://www.googleapis.com/auth/cloud-identity.inboundsso.readonly
   ```
</Step>
</Steps>
