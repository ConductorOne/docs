...
The following examples show how to sequence connector actions for common Google Workspace workflows. Enabling and disabling accounts is handled through the [Modify user status](/product/admin/automations-steps-reference#modify-user-status) automation step, not a connector action. Every other action listed maps to a [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) step in the C1 automation builder.
...
| 7 | **Modify user status** (automation step, not a connector action) | New user status: `Disabled` |
...
| Step | Action | Fields |
|------|--------|--------|
| 1 | **Modify user status** (automation step, not a connector action) | New user status: `Active` |
...
Use the **Modify user status** step to activate accounts that were pre-created in a suspended state (a common pattern for managing start-date access). The `change_user_org_unit` step ensures the user inherits the correct group policies and settings for their team.
