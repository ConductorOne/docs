...
The connector also supports group creation and deletion, [continuous sync](/baton/faq#what-is-syncing), and targeted sync.
...
| update_user_profile | `user_id` (resource ID, required)<br/>`given_name` (string, optional)<br/>`family_name` (string, optional)<br/>`recovery_email` (string, optional)<br/>`recovery_phone` (string, optional)<br/>`department` (string, optional)<br/>`job_title` (string, optional)<br/>`cost_center` (string, optional)<br/>`employee_type` (string, optional)<br/>`employee_id` (string, optional)<br/>`manager_email` (string, optional)<br/>`custom_schemas` (JSON string, optional) | Partial profile update using patch semantics (only provided fields are modified). Supports name fields, recovery details, Employee Information attributes (department, job title, cost center, employee ID, employee type), the manager relation, and custom-schema values. Send an empty string to clear a field. At least one updatable field must be provided |
| update_user | `user_id` (resource ID, required)<br/>`user_profile` (JSON string, required) | Updates a user's profile from a JSON object. Used by C1 push rules for automated profile sync. Supported keys: `given_name`, `family_name`, `recovery_email`, `recovery_phone`, `department`, `job_title`, `cost_center`, `employee_type`, `employee_id`, `manager_email`, `custom_schemas` |
...
Click **Add new** and fill out the form:
   ...
   - **OAuth Scopes**: Copy and paste in the relevant scopes
      ...
      - Scope:   https://www.googleapis.com/auth/cloud-identity.inboundsso.readonly <br/>
