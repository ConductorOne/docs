...
description: "C1.ai provides identity governance and just-in-time provisioning for Google Workspace. Integrate your Google Workspace instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Google Workspace. Integrate your Google Workspace instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
sidebarTitle: "Google Workspace"
---

<Tip>
**This is an updated and improved version of the Google Workspace integration!** If you're setting up a Google Workspace connector with C1.ai for the first time, you're in the right place.
</Tip>

<Note>
This page covers the Google Workspace **connector**, which syncs and provisions access data for identity governance. It does not control how you sign in to C1.ai. To set up signing in to C1.ai with your Google account, see [Authenticate with Google](/product/how-to/qs-set-up-c1#authenticate-with-google).
</Note>
...
The following examples show how to sequence connector actions for common Google Workspace workflows. Enabling and disabling accounts uses the [Account lifecycle action](/product/admin/automations-steps-reference#account-lifecycle-action) step, with its own **Target account** selector. Every other action listed maps to a [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) step in the C1.ai automation builder.
...
| 7 | `disable_user` (via **Account lifecycle action** step) | Target account: departing user |
...
| Step | Action | Fields |
|------|--------|--------|
| 1 | `enable_user` (via **Account lifecycle action** step) | Target account: new hire |
...
- Either the **Connector Administrator** or **Super Administrator** role in C1.ai.
...
| **Service account** | The single identity C1.ai authenticates as. It needs no project IAM role. |
| **JSON key** | The credential you upload to C1.ai. Creating one requires the Service Account Key Admin role and an organization policy that permits key creation. |
...
Create a new project for your organization:
   ...
   - **Project Name**: Choose a name, such as "C1.ai Integration"
...
Create the service account C1.ai will authenticate as.
...
Under **Service account details**, fill in the following:
   ...
   - **Service account name:** C1.ai Integration
   - **Service account description:** for example, "Service account for C1.ai Google Workspace Integration"
...
| Related policies | `constraints/iam.serviceAccountKeyExpiryHours` caps how long new keys stay valid. If your organization sets it, plan to rotate the key in C1.ai before it expires. |
...
Choose one of the two sets below. Read-only access lets C1.ai sync access data. Read/write access lets C1.ai sync access data and provision access.
...
<Warning>
The write scopes let C1.ai provision and deprovision access. **If you do not want C1.ai to perform these tasks, use the read-only scope set.** Read-only still syncs users, groups, roles, and enterprise applications, so it is sufficient when you use Google Workspace as your [directory](#set-google-workspace-as-your-c1-directory) and run access reviews.

The read-only set has one exception. `admin.directory.user.security` appears in both sets, and it permits three actions that revoke a user's access: `sign_out_user`, `delete_all_oauth_tokens`, and `delete_all_application_passwords`. These actions remain available with the read-only set. **If you do not want C1.ai to perform these tasks, do not grant this scope.** The connector then stops discovering OAuth apps and stops syncing enterprise applications.
</Warning>
...
<Warning>
This task requires either the **Connector Administrator** or **Super Administrator** role in C1.ai.
</Warning>
...
<Step>
**Optional.** If you want to limit which domains C1.ai syncs, in the **Domain** field, enter your primary domain. If you leave this field blank, C1.ai will sync all available domains.
</Step>
<Step>
...
**Done.** Your Google Workspace connector is now pulling access data into C1.ai.
</Tab>
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Google Workspace connector to. Google Workspace data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
...
## Set Google Workspace as your C1.ai directory

If Google Workspace is your source of truth for who works at your company, designate the connector's app as your [directory](/product/admin/directory). C1.ai then creates a C1.ai user for each Workspace account it syncs, and accounts from your other connectors attach to those users.
...
<Step>
In C1.ai, navigate to **Directory** > **Directory sources**.
</Step>
<Step>
...
<Step>
**Optional.** [Limit which accounts will be pulled into C1.ai](/product/admin/directory#optional-limit-which-accounts-will-be-pulled-into-c1) or [configure merge matching](/product/admin/directory#configure-merge-matching). You can change both later.
</Step>
<Step>
...
For how directory accounts become C1.ai users, and how accounts from non-directory apps find an owner, see [Connect a directory](/product/admin/directory).
