...
|-----------------|-----------------|----------------------------|
| [Account lifecycle action](/product/admin/automations-steps-reference#account-lifecycle-action) | `enable_user`, `disable_user` | The step's own **Target account** selector — either the account in context or a specific account. |
| [Perform connector action](/product/admin/automations-steps-reference#perform-connector-action) | Every other action in the table below | Varies by action; most take a plain `user_id` or `resource_id` field. |
