...
og:title: Set up a Teleport connector - C1.ai docs
og:description: Integrate your Teleport instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1.ai provides identity governance and just-in-time provisioning for Teleport. Integrate your Teleport instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests.
...
Due to Teleport's security rules, it is not possible to auto-generate and assign passwords to newly created accounts. When a new Teleport account is created by C1.ai, a password reset link (associated with a token) will be sent to a [vault](/product/admin/vaults). This allows the user to configure the password for their new account.
...
This guide walks you through setting up the Baton Teleport connector in Kubernetes to run continuously and sync with C1.ai. The connector uses certificate-based authentication via `tbot` and connects to C1.ai using client credentials.
...
<Step>
Connect to C1.ai using your client credentials
</Step>
<Step>
Poll for tasks from C1.ai periodically
</Step>
<Step>
Execute sync tasks when requested by C1.ai
</Step>
<Step>
Handle provisioning requests when access needs to be granted/revoked
</Step>
<Step>
Send results back to C1.ai instead of writing to local files
</Step>
</Steps>
...
<Step title="Verify connector connectivity:">
   ...
   - Successful connection to C1.ai
