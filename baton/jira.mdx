...
description: "C1.ai provides identity governance for Jira Cloud. Integrate your Jira Cloud instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:title: "Set up an Atlassian Jira Cloud connector"
og:description: "C1.ai provides identity governance for Jira Cloud. Integrate your Jira Cloud instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
The Jira Cloud connector supports [automatic account provisioning](/product/admin/account-provisioning). To provision accounts, the app's Provisioning step must include a Connector step with mappings for both required fields:

| Field | Type | Description |
| :--- | :--- | :--- |
| `email` | String | Email address of the new user. |
| `products` | String list | Jira products the user can access. Valid values: `jira-core`, `jira-servicedesk`, `jira-product-discovery`, `jira-software`. |

Both mappings are required. If the `products` mapping is missing, the Atlassian API rejects the request with an HTTP 400 error, and the connector reports this as a generic "failed to create user" error without surfacing the upstream message, so the cause may not be immediately obvious.
...
**Done.** Your Jira Cloud connector is now pulling access data into C1.ai.
</Tab>
...
stringData:
  ...
  # Optional: skip project participant sync (see note below)
...
<Warning>
**About `BATON_SKIP_PROJECT_PARTICIPANTS`**

When this flag is enabled, the connector stops syncing per-project participant data entirely. This removes the `participate` **entitlement** for every project, not just the individual grants. Any access requests or access reviews that reference a project's `participate` entitlement will have no target entitlement to operate on. Only enable this flag if you do not use project participant entitlements in access requests or reviews.
</Warning>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-jira
        image: public.ecr.aws/conductorone/baton-jira:latest
...
<Steps>
    ...
    <Step>
    Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Jira Cloud connector to. Jira Cloud data should be found on the **Entitlements** and **Accounts** tabs.
    </Step>
</Steps>
