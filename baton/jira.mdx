...
| `products` | String list | Jira products the user can access. Valid values: `jira-core`, `jira-servicedesk`, `jira-product-discovery`, `jira-software`. To create a user without product access, pass an empty list. |

Both mappings are required. If the `products` mapping is missing, the Atlassian API rejects the request with an HTTP 400 error. The connector reports this as a generic "create account failed" error without surfacing the upstream message, so the cause may not be immediately obvious.
...
- Per-project participant listing uses a page size of 1,000. On projects with more than 1,000 participants the connector pages through all of them, but each page requires a separate API call.
