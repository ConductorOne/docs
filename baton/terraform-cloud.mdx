...
description: "C1.ai provides identity governance and just-in-time provisioning for HCP Terraform. Integrate your HCP Terraform instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for HCP Terraform. Integrate your HCP Terraform instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
- Account provisioning creates an organization invitation. HCP Terraform does not accept a password through the API, so C1.ai does not set one. The invited person receives an email from HCP Terraform and sets their own password when they accept the invitation.
...
- Projects and workspaces sync for visibility and are read-only. C1.ai does not provision access to them directly.
...
<Tip>
If you run Terraform Enterprise, sign in to your own instance URL instead. You provide that URL to C1.ai as the **Address** setting.
</Tip>
</Step>
...
<Warning>
This token is used by C1.ai when automatically provisioning and deprovisioning access. **If you do not want C1.ai to perform these tasks, use an account without owner permissions and do not enable provisioning.**
</Warning>

<Tip>
If you set an expiration date, record it and rotate the token in C1.ai before it expires. An expired token causes syncs to fail.
</Tip>
</Step>
...
<Warning>
To configure a connector in C1.ai, you must have the **Connector Administrator** or **Super Administrator** role. You also need the HCP Terraform API token you created above.
</Warning>
...
**Done.** Your HCP Terraform connector is now pulling access data into C1.ai.
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Applications**. On the **Managed apps** tab, locate and click the name of the application you added the HCP Terraform connector to. HCP Terraform data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
