...
description: "C1.ai provides identity governance and just-in-time provisioning for Sigma Computing. Integrate Sigma Computing with C1.ai to review and provision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Sigma Computing. Integrate Sigma Computing with C1.ai to review and provision access."
...
- Team Admin access is sync-only. You can review Team Admin access in C1.ai, but Sigma's API does not support granting or revoking it.
...
**Done.** Your Sigma Computing connector is now pulling access data into C1.ai.
</Tab>
...
When running in service mode on Kubernetes, a self-hosted connector maintains an ongoing connection with C1.ai and uploads data at regular intervals.
...
<Step>
Click **Rotate** to generate a new C1.ai Client ID and Secret. Copy and securely store both values for Step 2.
</Step>
</Steps>
...
<Steps>
<Step>
Create a namespace for C1.ai connectors if desired, and apply the secret and deployment configuration files.
</Step>

<Step>
In C1.ai, click **Applications**. On the **Managed apps** tab, open the application with the Sigma Computing connector and confirm that Sigma data appears on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
