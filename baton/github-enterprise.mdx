...
description: "C1.ai provides identity governance and just-in-time provisioning for GitHub Enterprise. Integrate your GitHub Enterprise instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for GitHub Enterprise. Integrate your GitHub Enterprise instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
Select the following **Scopes:**
   ...
   - **repo** - select all
   - **admin:org** - select all if using C1.ai for GitHub Enterprise provisioning (see the note below), or **read:org**
   - **user** - select all
   - **admin:enterprise** - select **read:enterprise**

   <Warning>
   The **write::org** scope is used by C1.ai when automatically provisioning and deprovisioning GitHub Enterprise access on your behalf. **If you do not want C1.ai to perform these tasks for you, do not give your token this scope.**
   </Warning>
</Step>
<Step>
...
If you're using a personal access token to set up the connector:
   ...
   1. **Optional.** If you want to sync only specific organizations, enter the organizations' names in the **Organizations** field. If you do not specify specific organizations, C1.ai will sync all organizations.
...
**Done.** Your GitHub Enterprise connector is now pulling access data into C1.ai.
</Tab>
...
<Warning>
Self-hosted deployment requires **baton-github-enterprise v0.0.4 or later**. Earlier versions fail to register with C1.ai when deployed in minimal container environments (for example, Debian Bookworm slim on Kubernetes) due to missing OS info fallbacks.
</Warning>
...
- The **Connector Administrator** or **Super Administrator** role in C1.ai
- A **Client ID** and **Client Secret** from C1.ai (navigate to **Settings** > **API Credentials** to generate these)
...
<Step>
The connector connects to your C1.ai tenant and begins syncing. Verify it appears as **Connected** in **Apps** > **Connectors**.
</Step>
</Steps>
