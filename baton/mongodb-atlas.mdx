...
description: "C1.ai provides identity governance and just-in-time provisioning for MongoDB Atlas. Integrate your MongoDB Atlas instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for MongoDB Atlas. Integrate your MongoDB Atlas instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
| Organization API Keys | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> |  |
...
| :--- | :--- | :--- |
| `SCRAM-SHA` | Password-based authentication (default). A random password is generated and stored in a C1.ai [vault](/product/admin/vaults). | Alphanumeric string (special characters such as `@` are not supported; email addresses are invalid) |
...
<Step>
Give the new API key a description, such as "C1.ai integration".
</Step>
<Step>
...
<Step>
Under **API Access List**, click **Add Access List Entry**. Add the IP addresses listed on the [C1.ai Baton FAQ page](/baton/faq#which-ip-addresses-does-conductorone-use-for-connectors), then click **Save**.
...
<Step>
**Optional.** To sync databases and collections, for each project, add the IP addresses listed on the [C1.ai Baton FAQ page](/baton/faq#which-ip-addresses-does-conductorone-use-for-connectors) to **Database & Network Access** > **IP Access List**. See the **Sync databases** and **Enable Mongo driver** configuration options below.
</Step>
<Step>
...
<Step>
**Optional.** If desired, click to enable **Create invite**, which will create email invitations for new MongoDB accounts provisioned by C1.ai.
</Step>
<Step>
...
**Done.** Your MongoDB Atlas connector is now pulling access data into C1.ai.
</Tab>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-mongodb
        image: public.ecr.aws/conductorone/baton-mongodb:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the MongoDB Atlas connector to. MongoDB Atlas data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
