...
og:title: Run multiple connectors as a Windows service - C1.ai docs
...
- A C1.ai connector created for each SQL Server instance — see [Deploy self-hosted connectors](/baton/deploy)
...
<Step>
Open each connector YAML file in Notepad and add the following, replacing the values with the connection string and C1.ai credentials for that instance:
...
<Step>
In C1.ai, confirm the connectors are syncing. Navigate to each connector and verify data is appearing.
</Step>
<Step>
