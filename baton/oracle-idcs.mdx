...
description: "C1.ai provides identity governance for Oracle Identity Cloud Service (IDCS). Integrate your Oracle IDCS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance for Oracle Identity Cloud Service (IDCS). Integrate your Oracle IDCS instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
| Application roles | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>* | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>** |

The Oracle IDCS connector supports [automatic account provisioning and deprovisioning](/product/admin/account-provisioning).

The connector also supports **account lifecycle actions** (enable and disable account) and **attribute push** to update user profile fields directly from C1.ai.

*You must opt into syncing application and application role data; these are not synced by default.

**Application access and application roles that are inherited via group membership cannot be deprovisioned using C1.ai.
...
Use the **Automation** tab on the Oracle IDCS connector page in C1.ai to enable or disable user accounts:
...
The connector supports pushing updates to the following user profile fields directly from a user's profile page in C1.ai:
...
To push an attribute, navigate to an Oracle IDCS user's profile in C1.ai and use the **Push Attribute** option. Updates are applied via SCIM PATCH.
...
<Step>
Enter a name for the application, such as "C1.ai", then click **Next**.
</Step>
<Step>
...
**Done.** Your Oracle IDCS connector is now pulling access data into C1.ai.
</Tab>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-oracle-idcs
        image: public.ecr.aws/conductorone/baton-oracle-idcs:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Oracle IDCS connector to. Oracle IDCS data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
