...
og:title: Set up a Workday Accounts connector - C1.ai docs
og:description: Integrate your Workday instance with C1.ai to run user access reviews, enable just-in-time access requests, and easily provision and deprovision access.
description: C1.ai provides identity governance and just-in-time provisioning for Workday. Integrate your Workday instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests.
sidebarTitle: Workday Account
---

## Which Workday connector should I use? 

C1.ai offers two Workday connectors: Workday and Workday Accounts. How you want to work with Workday in C1.ai will determine which one you should set up.
...
| User-based security groups | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>    | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>  | 
| Account type    | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>    |   | 
| Service center  | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>    |   | 

The connector surfaces the following entitlements:

* **Security groups** and **user-based security groups**: a `Member` entitlement, granted to each account that belongs to the group.
* **Service centers**: a `Representative` entitlement, granted to each account that represents the service center.
* **Account type**: `Implementer` and `Integration User` role assignments, granted to the matching accounts.
...
<Step>
In the modal that appears, give the new API client a name, such as "C1.ai integration". 
</Step>
<Step>
...
<Step>
Create a new security group. Give it a name, such as "C1.ai WQL integration security group". 
</Step>
<Step>
...
If you want C1.ai to sync specific non-user-based security group types (such as `Role-Based Security Group (Constrained)` and `Integration System Security Group`), you'll need to gather the list of these types. 
...
<Steps>
   ...
   **Optional.** If desired, click the checkbox to **Sync user-based security groups**. 
      ...
      * If you select this option, you may enter an optional list of the user-based security groups that you want to sync in the **User-based security groups (optional)** field. 
   </Step>
   <Step>
   **Optional.** Enter the list of (non-user-based) security group types you want to sync in the **Other security group types (optional)** field. Remember that security group type names are case-sensitive. 
   </Step>
   <Step>
   **Optional.** Enter the list of (non-user-based) security groups you want to sync in the **Other security groups (optional)** field. Remember that security group names are case-sensitive.  
   </Step>
   <Step>
   Click **Save**.
   </Step>
...
**Done.** Your Workday Accounts connector is now pulling access data into C1.ai.
</Tab>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-workday-wql
        image: public.ecr.aws/conductorone/baton-workday-wql:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Workday Accounts connector to. Workday Accounts data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
