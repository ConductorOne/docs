...
description: "C1.ai provides identity governance and just-in-time provisioning for Kyriba. Integrate your Kyriba instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Kyriba. Integrate your Kyriba instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
sidebarTitle: "Kyriba"
---
...
Connector actions are custom capabilities that extend C1.ai automations with app-specific operations. Which automation step offers an action is determined by the action itself — you cannot choose:
...
|-------------|-------------------|-------------|
| `enable_user` | `user_id` (string, required) | Reactivates a previously disabled Kyriba user without affecting any other data |
| `disable_user` | `user_id` (string, required) | Deactivates a Kyriba user without deleting them — the user record, their profile, and their access assignments are preserved and can be restored with `enable_user` |

The connector passes `user_id` to Kyriba unchanged. Kyriba's `/v1/users/{ref}` accepts either the user's UUID or their `code`, and C1.ai's account selector supplies the UUID.
...
- **Kyriba URL** — the base URL of your Kyriba instance, **including the API gateway path** (the host and path are tenant-specific — for example, `https://<your-tenant-host>/api` or `https://<your-tenant-host>/gateway/api`). Confirm the exact value with your Kyriba administrator.
- **Token URL** — the OAuth2 token endpoint (for example, `https://<your-tenant-host>/gateway/oauth/token`; the host may differ from the API host, such as an `auth.` subdomain)
...
<Steps>
  ...
  <Step>
    Grant the following access permissions to enable full sync and provisioning:

    | Access permission | Used for |
    | :--- | :--- |
    ...
    These are Kyriba access permissions granted to the API user, not OAuth `scope` parameters — the connector does not send a `scope` value on the token request. For sync-only (read) access, all three are still required. Provisioning requires write access on the relevant permissions: `user-scope` for account creation/deletion, the `enable_user`/`disable_user` actions, and data permission profile assignment; `user-group-scope` for group membership changes. Note that user **update and deletion** — used for the `enable_user`/`disable_user` actions and data-permission-profile assignment (`PUT /v1/users/{ref}`), and account deprovisioning (`DELETE /v1/users/{ref}`) — require broader user-management permission than user creation. A credential granted only create + read access may sync and create users successfully but return `403 Forbidden` on updates, deletes, and the enable/disable actions; grant full user-management permission if you need deprovisioning, enable/disable, or data-permission-profile assignment.
  </Step>
...
{/* AUTO-GENERATED:START - config-params
     Generated from config_schema.json. Do not edit manually. */}
      ...
      <Step>
        ...
        - **Kyriba URL**: The base URL of your Kyriba instance, including the tenant-specific API gateway path (for example, `https://<your-tenant-host>/api` or `https://<your-tenant-host>/gateway/api`)
        - **Token URL**: The OAuth2 token endpoint URL (for example, `https://<your-tenant-host>/gateway/oauth/token`; the host may differ from the API host)
...
{/* AUTO-GENERATED:END - config-params */}
    ...
    **Done.** Your Kyriba connector is now pulling access data into C1.ai.
  </Tab>
  ...
  <Tab title="Self-hosted">
    ...
    * [GitHub repository](https://github.com/conductorone/baton-kyriba): Access the source code, report issues, or contribute to the project.
    ...
    stringData:
      ...
      # Kyriba credentials
      BATON_KYRIBA_URL: https://<your-tenant-host>/api
      BATON_TOKEN_URL: https://<your-tenant-host>/gateway/oauth/token
    ...
    <Steps>
      ...
      <Step>
        Check that the connector data uploaded correctly. In C1.ai, click **Applications**. On the **Managed apps** tab, locate and click the name of the application you added the Kyriba connector to. Kyriba data should be found on the **Entitlements** and **Accounts** tabs.
      </Step>
    </Steps>
