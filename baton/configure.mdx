...
C1 automatically pauses a connector’s sync cycle and alerts the connector owner if significant data anomalies are detected (for example, if the number of resources decreases by half between one sync and the next, or if a large number of grants are removed).

When a significant drop in resources or grants is detected during a connector sync, C1 does not immediately apply the change as a precaution against unintentionally removing access. A banner explaining that the sync has been paused is displayed. Click **Confirm changes** in the banner on the application’s page to let C1 know that the change is expected and that it’s safe to proceed.

A common way to trigger this pause: adding a setup field that narrows what a connector syncs — for example, limiting it to specific tenants, projects, orgs, or sites — to a connector that already synced without that field set. Excluding scope that was previously synced also excludes the grants and memberships tied to it, which registers as a significant drop. This is expected; confirm the change to apply the narrower sync going forward. Check the connector’s own documentation for which setup fields can narrow its sync scope.
