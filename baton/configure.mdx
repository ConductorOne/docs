...
This task requires the **Super Administrator** or **Application Administrator** role in C1.ai.
</Warning>

You can specify which attributes each connector syncs or ignores. This allows you to customize the information that is pulled into C1.ai.
...
<Steps>
    ...
    - **Blocklist option:** Select **Ignore attributes** and enter those attributes that you want C1.ai to ignore when syncing. All attributes not specifically listed will be synced.
    - **Allowlist option:** Select **Only sync attributes** and enter those attributes that you want C1.ai to sync. All attributes not specifically listed will be ignored.
    </Step>
    <Step>
    Click **Save**.
    </Step>
...
C1.ai automatically pauses a connector’s sync cycle and alerts the connector owner if significant data anomalies are detected (for example, if the number of resources decreases by half between one sync and the next).
