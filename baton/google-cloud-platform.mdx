...
<Note>
GCP IAM bindings can carry a **condition** (a CEL expression that GCP evaluates at request time, e.g. to grant access only during a time window or only for resources with a certain tag). Neither the flat grants model nor the Sparse ACL (RBAC) model evaluates these expressions — a synced grant may reflect access that, in GCP, only actually applies when its condition holds. If the precise, currently-in-effect state of a conditional binding matters, verify it directly in GCP (Console or `gcloud`).
</Note>

<Warning>
**Revoking conditional access deletes the condition.** A condition is stored on the IAM binding itself, and GCP does not allow a binding with no members. So when a revoke removes the last member of a conditional binding, GCP deletes the binding and its CEL expression along with it — the same thing `gcloud remove-iam-policy-binding` does. The connector cannot restore the expression afterwards, and it is not visible in the resulting policy.

The connector logs the full expression at `WARN` when this happens, so it can be recovered from connector logs. If a conditional binding's CEL expression is something you need to keep, revoke that access directly in GCP rather than through C1, or ensure the binding retains at least one other member.

This follows from C1 having no way to represent an IAM condition, so both provisioning operations treat access as unconditional: **granting** access always writes a plain, unconditional binding — so granting a role to someone who already holds it under a condition widens their access — and **revoking** it removes the principal from every binding for that role, conditional or not. A revoke followed by a re-grant therefore leaves the principal with unconditional access where they previously had conditional access. If conditions are part of how you control access to a scope, manage those bindings in GCP.
</Warning>
