...
description: "C1.ai provides identity governance and just-in-time provisioning for Google Cloud Platform with Google Workspace. Integrate your Google Cloud Platform with Google Workspace instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:description: "C1.ai provides identity governance and just-in-time provisioning for Google Cloud Platform with Google Workspace. Integrate your Google Cloud Platform with Google Workspace instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
<Note>
  ...
  Identity pools, C1.ai also shows condition-scoped administrative access as the
...
1. Create a dedicated GCP project for the C1.ai integration
...
We recommend creating a dedicated GCP project for the C1.ai integration. This keeps the integration's permissions and audit logs isolated from your other projects.
...
- **Service account description:** for example, "Service account for C1.ai Google Cloud Platform with Google Workspace Integration"
...
<Warning>
  **If you already had Workforce Identity Federation enabled, add
  ...
  policy instead would make C1.ai treat every pool grant it had already synced as
...
<Step>
Keep the downloaded file safe — you'll upload it when configuring the connector in C1.ai.
</Step>
</Steps>
...
- **OAuth Scopes**: Copy and paste in the relevant scopes
        ...
        - Use the following scopes to give C1.ai **READ** access (syncing access data):
       ...
       - Use the following scopes to give C1.ai **READ/WRITE** access (syncing access data and provisioning access):
...
<Note>
Enabling this option changes how IAM access data is represented in C1.ai. Flat grants on projects, folders, and organizations will no longer be synced — only the IAM Role Assignment resources will carry that data. Enable this only if your C1.ai configuration is set up to use the RBAC model.
</Note>
</Step>
<Step>
Click **Save**.
...
**Done.** Your Google Cloud Platform with Google Workspace connector is now pulling access data into C1.ai.
...
<Steps>
  <Step>
    Create a namespace in which to run C1.ai connectors (if desired), then apply
    the secret config and deployment config files.
  </Step>
  <Step>
    Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On
