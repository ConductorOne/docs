...
description: "C1.ai provides identity governance and just-in-time provisioning for LangSmith. Integrate your LangSmith instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for LangSmith. Integrate your LangSmith instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
personal access token (`lsv2_pt_...`) or workspace-scoped key. C1.ai recommends a
...
- Workspace role grants can be added and revoked from C1.ai.
- Organization role grants are read-only because LangSmith replaces the single
  ...
  grants.
- A user must already be a LangSmith organization member before C1.ai can assign
...
<Tabs>
  <Tab title="Cloud-hosted">
    ...
    **Done.** Your LangSmith connector is now pulling access data into C1.ai.
  </Tab>
  ...
  <Tab title="Self-hosted">
    ...
    <Steps>
      ...
      <Step>
        Configure C1.ai credentials and LangSmith credentials as environment
        variables:
