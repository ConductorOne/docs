...
description: "C1.ai provides identity governance and just-in-time provisioning for LDAP. Integrate your LDAP server with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1.ai provides identity governance and just-in-time provisioning for LDAP. Integrate your LDAP server with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
sidebarTitle: "LDAP"
---
...
Two requirements come from the C1.ai side rather than from LDAP:

- **`user_id` takes the C1.ai account identifier, not the LDAP DN.** C1.ai resolves the account to the connector's resource before dispatching the action. A DN fails inside C1.ai with `resource <dn> with type user was not found` and never reaches the connector, so it produces no connector log line and leaves the directory untouched.
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the LDAP connector to. LDAP data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>

**Done.** Your LDAP connector is now pulling access data into C1.ai.
</Tab>
</Tabs>