...
- SQLite
- Amazon Redshift
- IBM DB2 (available on request — [contact C1.ai's support team](mailto:support@conductorone.com) for a binary)
...
| Amazon Redshift | `postgres://user:pass@cluster.region.redshift.amazonaws.com:5439/dbname` |
| IBM DB2 | `db2://user:pass@host:50000/dbname` |

<Note>
IBM DB2 requires a separate binary bundled with IBM's client driver and isn't available from the standard download center. [Contact C1.ai's support team](mailto:support@conductorone.com) to request a DB2-enabled binary. Customers connecting to DB2 are responsible for providing their own IBM Db2 Connect license where required.
</Note>
...
The `purpose` field tells C1.ai how to interpret an entitlement:
...
Create new accounts through C1.ai:
...
|-------|----------|-------------|
| `char_set` | Yes | The set of characters to draw from |
| `min_count` | Yes | Minimum number of characters from this set that must appear in the password (must be > 0) |
...
| `--client-id` | C1.ai client ID |
| `--client-secret` | C1.ai client secret |
