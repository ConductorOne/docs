...
og:description: "C1 provides identity governance for Workday. Integrate your Workday instance with C1 for unified visibility and governance over user access."
description: "C1 provides identity governance for Workday. Integrate your Workday instance with C1 for unified visibility and governance over user access."
...
The **Custom Report with Basic Auth** option is the most popular and is often preferred by Workday Admins and HR teams. RaaS allows fine-grained control over what data is sent to C1 and lets you transform attribute formats as needed.
...
<Step>
Give the new API client a name, such as "C1 integration (report OAuth)".
</Step>
<Step>
...
If using a custom report with Basic Auth (Option 1), select **Custom Report with Basic Auth** and fill out the form:
   ...
   2. **Optional.** To include extra Workday data in the sync, in the **Additional report fields** field, enter any additional field names from the Workday JSON report that you want to include in user profiles. Press **Enter** between each field name.
      ...
      This option surfaces *extra* single-value fields from your report as [user profile attributes](/product/admin/attributes). It is separate from the built-in fields listed in [Expected custom report contents when using the JSON format](#expected-custom-report-contents-when-using-the-json-format), which the connector parses natively during the core sync. Fields that return multiple values (such as `roles` or `security_groups`) are not supported as additional report fields — but `roles` and `security_groups` are still synced as entitlements when included in your report under the expected column headings.
</Step>
<Step>
