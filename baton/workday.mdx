...
<Note>
   Duplicate `user_id` rows are reduced to a single record: the active record wins, and the record that loses is discarded whole, taking its roles and security groups with it. This applies to the JSON report only — the CSV report described below is unaffected.
</Note>
