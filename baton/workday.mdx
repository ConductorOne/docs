...
og:title: Set up a Workday connector - C1.ai docs
og:description: "C1.ai provides identity governance for Workday. Integrate your Workday instance with C1.ai for unified visibility and governance over user access."
description: "C1.ai provides identity governance for Workday. Integrate your Workday instance with C1.ai for unified visibility and governance over user access."
sidebarTitle: "Workday"
---

## Which Workday connector should I use? 

C1.ai offers two Workday connectors: Workday and Workday Accounts (WQL). How you want to work with Workday in C1.ai will determine which one you should set up.
...
<Tip>
To authenticate with Workday, you can use a Report-as-a-service (RaaS) **custom report** in CSV or JSON format (with either Basic Auth or API Client), or directly an **API Client** with OAuth.

**Which authentication method should I use?** 

The **Custom Report with Basic Auth** option is the most popular and is often preferred by Workday Admins and HR teams. RaaS allows fine-grained control over what data is sent to C1.ai and lets you transform attribute formats as needed.

The **Custom Report with API Client** option is a good choice when your security policies require OAuth-based authentication, while still providing the same fine-grained data control as Basic Auth reports. The refresh token for this option must be generated using the credentials of the Workday user who owns the report.

The **API Client** option pulls available data about Workday accounts, such as each account’s manager and manager email, which can be used as [user attributes](/product/admin/attributes). It does not require a custom report.

Follow the relevant set of instructions below to generate your credentials.
</Tip>

### Option 1: Authenticate using a custom report with Basic Auth 
...
<Tip>
   The fields needed in custom reports can vary depending on how your Workday installation is configured. Contact C1.ai support if you need help figuring out which fields to include.   
</Tip>
...
### Option 2: Authenticate using a Custom Report with API Client

This option combines a RaaS Custom Report with OAuth authentication. Follow the **Set up the custom report** steps from Option 1 above to create the report and obtain its URL, then set up an API client as described below.

<Warning>
The refresh token for this option **must be generated using the credentials of the Workday user who owns the custom report**. Tokens generated for a different user will result in authorization errors when the connector attempts to fetch the report.
</Warning>

<Note>
Unlike Option 3, this option does **not** require creating an Integration System User (ISU) or an Integration System Security Group (ISSG). The API client is tied directly to the report owner's credentials via the refresh token.
</Note>

#### Create Workday API Client Credentials for Custom Report
...
<Step>
Give the new API client a name, such as "C1.ai integration (report OAuth)".
</Step>
<Step>
In the **Scopes** box, select **Tenant Non-Configurable**. Also search for "Staffing" and select **Staffing** and **Organizations and Roles**, then click **OK**.

<Warning>
The **Tenant Non-Configurable** scope is required for custom report OAuth authentication. Without it, OAuth token requests will be rejected.
</Warning>
</Step>
<Step>
...
<Steps>
<Step>
Click the three dots icon next to the client name and navigate to **API Client** > **Manage Refresh Tokens for Integrations**.
</Step>
<Step>
Select the **Workday user who owns the custom report** (not the ISU) and click **OK**.

<Warning>
This step is critical. The refresh token must be associated with the user who created and owns the custom report, not the ISU. Using the wrong user will result in authorization errors when fetching the report.
</Warning>
</Step>
<Step>
...
### Option 3: Authenticate using API client credentials (OAuth)
...
<Step>
In the **Create Integration System User** modal, enter a **User Name**, such as "C1.ai integration". 
</Step>
<Step>
...
<Step>
Create a new security group. Give it a name, such as "C1.ai integration security group".
</Step>
<Step>
...
<Step>
Give the new API client a name, such as "C1.ai integration".
</Step>
<Step>
...
<Step>
In the **Scopes** box, search for "Staffing". Select **Staffing** and **Organizations and Roles** and click **OK**.
</Step>
<Step>
The newly created client's client ID and client secret are shown. Carefully copy and save these credentials.
...
<Step>
If using a custom report with Basic Auth (Option 1), select **Custom Report with Basic Auth** and fill out the form:
...
<Step>
If using a custom report with OAuth (Option 2), select **Custom Report with API Client** and fill out the form:

   1. Enter the **Report URL** in the relevant field.

   2. Enter the **Client ID**, **Client secret**, and **Refresh token** from the API client you registered for this report. For **Tenant Name**, enter the tenant segment from your Report URL — it's the value between `/customreport2/` and the next `/` (for example, in `https://wd2-impl-services1.workday.com/ccx/service/customreport2/acme/...`, the tenant is `acme`).

   3. **Optional.** Enter any additional field names from the Workday JSON report in the **Additional report fields** field. Press **Enter** between each field name.

      Fields that return multiple values (such as `roles` or `security_groups`) are not supported.
</Step>
<Step>
If using an API client (Option 3), select **API Client** and fill out the form:
...
**Done.** Your Workday connector is now pulling access data into C1.ai.
</Tab>
...
stringData:
  ...
  # Option 1: Custom report with Basic Auth
  ...
  # Option 2: Custom report with OAuth (refresh token must belong to the report owner)
  BATON_WORKDAY_FETCH_USERS_FROM_CUSTOM_REPORT: true
  BATON_WORKDAY_REPORT_URL: <Workday custom report URL>
  BATON_WORKDAY_CLIENT_ID: <Workday API client ID>
  BATON_WORKDAY_CLIENT_SECRET: <Workday API client secret>
  BATON_WORKDAY_REFRESH_TOKEN: <Refresh token generated for the report owner>
  BATON_TENANT_NAME: <Workday tenant name>
  BATON_WORKDAY_ADDITIONAL_REPORT_FIELDS: <List of additional field names from the Workday JSON report to include in user profiles>

  # Option 3: API client with OAuth
  BATON_WORKDAY_CLIENT_ID: <Workday API client ID>
  BATON_WORKDAY_CLIENT_SECRET: <Workday API client secret>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: baton-workday
        image: public.ecr.aws/conductorone/baton-workday:latest
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Workday connector to. Workday data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
**Done.** Your Workday connector is now pulling access data into C1.ai.
