...
description: "C1.ai provides identity governance and just-in-time provisioning for Atlassian. Integrate your Atlassian instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
og:title: "Set up an Atlassian connector"
og:description: "C1.ai provides identity governance and just-in-time provisioning for Atlassian. Integrate your Atlassian instance with C1.ai to run user access reviews (UARs) and enable just-in-time access requests."
...
|-------------|-------------------|-------------|
| enable_user | `user_id` (string, required) | Restore an Atlassian user's access to the organization |
| disable_user     | `user_id` (string, required) | Suspend an Atlassian user's access to the organization |
...
**Done.** Your Atlassian connector is now pulling access data into C1.ai.
</Tab>
...
<Steps>
    ...
    <Step>
    Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Atlassian connector to. Atlassian data should be found on the **Entitlements** and **Accounts** tabs.
    </Step>
</Steps>
