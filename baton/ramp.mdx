...
description: C1.ai provides identity governance and just-in-time provisioning for Ramp. Integrate your Ramp instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access.
og:description: C1.ai provides identity governance and just-in-time provisioning for Ramp. Integrate your Ramp instance with C1.ai to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access.
sidebarTitle: Ramp
---

<Tip>
**This is an updated and improved version of the Ramp connector!** If you're setting up Ramp with C1.ai for the first time, you're in the right place. 
</Tip>
...
Synced Ramp vendors are emitted as C1.ai vendor resources with Ramp vendor identity metadata. Vendor owner access is represented as the `owner` entitlement on each vendor. Vendor agreements are available as an opt-in resource type and use the same Ramp `vendors:read` scope as vendors.
...
<Steps>
  ...
  <Step>
    ...
    <Warning>
    The **users:write** and **vendors:write** scopes are used only when C1.ai provisions and deprovisions access. **If you do not want C1.ai to perform these tasks, do not grant these scopes.**
    </Warning>
  </Step>
  <Step>
...
**Done.** Your Ramp connector is now pulling access data into C1.ai.
</Tab>
...
The connector ships with an opt-in `vendor_agreement` resource type. When enabled in C1.ai resource-type settings, the connector syncs Ramp vendor agreements as their own resources, including term, renewal status, total contract value, and contract owners. Vendor agreements use the existing `vendors:read` scope; Ramp does not require a separate vendor-agreement scope.
...
Set `BATON_AUDIT_LOG_EVENTS=true` to expose Ramp's audit-log API as an event feed so vendor and agreement changes can flow into C1.ai incrementally between full syncs. This setting is independent from vendor-agreement resource enablement and requires Ramp audit-log API availability, such as Ramp Plus, plus the `audit_logs:read` scope. When the flag is unset (the default), the audit-log API is not called.
...
<Step>
Check that the connector data uploaded correctly. In C1.ai, click **Apps**. On the **Managed apps** tab, locate and click the name of the application you added the Ramp connector to. Ramp data should be found on the **Entitlements** and **Accounts** tabs.
</Step>
</Steps>
