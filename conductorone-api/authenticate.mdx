...
description: "To use the C1.ai API, you must authenticate your requests. This document guides you through the process of obtaining and utilizing an API key, as well as establishing an access token."
---
...
The first step in the process is to obtain your API key from C1.ai:

<Steps>
<Step>
In the C1.ai app, navigate to the User's API Settings page by clicking your username at the bottom of the screen and selecting **AI & API**.
</Step>
<Step>
...
A crucial part of interacting with the C1.ai API involves determining the appropriate endpoints for your requests. Two key components in this process are your API root and the token endpoint. These can be inferred from your Client ID.
...
#### Example requests for a basic access token

The examples below use placeholder values. Replace `<your-client-id>`, `<your-client-secret>`, and `<your-hostname>` with values from your own API credential (see Step 1) and the hostname extracted from your Client ID (see Step 2).

<CodeGroup>
```bash curl
curl -X POST "https://<your-hostname>/auth/v1/token" \
...
HOSTNAME = "<your-hostname>"  # for example, "acme.conductor.one"

response = requests.post(
    f"https://{HOSTNAME}/auth/v1/token",
    data={
        "grant_type": "client_credentials",
        "client_id": CLIENT_ID,
        "client_secret": CLIENT_SECRET,
...
Once you have successfully obtained an access token, you are ready to make authenticated requests to the C1.ai API.
...
- Generate and implement a new access token before the `expires_in` time has passed. This will ensure continuous access to the C1.ai API.
