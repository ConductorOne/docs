...
<Note>
Send the Client Secret in full, including the `secret-token:conductorone.com:` prefix. The token endpoint validates the entire string. If you strip the prefix or send only the base64url portion, the request fails with `invalid_request` and an `invalid OAuthTokenClientCredentials.ClientSecret` length error.
</Note>
